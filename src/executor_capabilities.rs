...
use crate::runtime_manifest::{
    RUNTIME_MANIFEST_SCHEMA, RUNTIME_MANIFEST_VERSION, RuntimeManifest, RuntimeManifestError,
    is_sha256_fingerprint, write_canonical_json,
};

pub const RUNTIME_TARGET_CONTRACT_SCHEMA: &str = "orbitkv.runtime-target-contract";
pub const RUNTIME_TARGET_CONTRACT_VERSION: u32 = 1;
pub const EXECUTION_SIGNATURE_SCHEMA: &str = "orbitkv.execution-signature";
pub const EXECUTION_SIGNATURE_VERSION: u32 = 1;
pub const RUNTIME_TARGET_BINDING_SCHEMA: &str = "orbitkv.runtime-target-binding";
pub const RUNTIME_TARGET_BINDING_VERSION: u32 = 1;
pub const RUNTIME_TARGET_ARTIFACT_MAX_BYTES: usize = 16 * 1024 * 1024;
...
pub struct RuntimeTargetIdentityV1 {
    pub id: String,
...
#[derive(Clone, Debug, Deserialize, Eq, PartialEq, Serialize)]
#[serde(deny_unknown_fields)]
pub struct RuntimeAdmissionProfileV1 {
    pub id: String,
    pub version: u32,
}

/// Closed topology vocabulary understood by the v1 admission implementation.
///
...
/// Versioned static contract published by an executor adapter.
#[derive(Clone, Debug, Deserialize, Eq, PartialEq, Serialize)]
#[serde(deny_unknown_fields)]
pub struct RuntimeTargetContractV1 {
    pub schema: String,
    pub version: u32,
    pub fingerprint: String,
    pub target: RuntimeTargetIdentityV1,
    pub admission_profile: RuntimeAdmissionProfileV1,
    pub page_tokens: u64,
    pub supported_topologies: Vec<ExecutionTopologyV1>,
}

#[derive(Clone, Debug, Deserialize, Eq, PartialEq, Serialize)]
#[serde(deny_unknown_fields)]
pub struct ExecutionKvHeadRangeV1 {
    pub start: u32,
...
#[serde(deny_unknown_fields)]
pub struct ExecutionBlockDomainV1 {
    pub start_block: u64,
    #[serde(skip_serializing_if = "Option::is_none")]
    pub end_block_exclusive: Option<u64>,
}

impl ExecutionBlockDomainV1 {
...
pub enum ExecutionAddressProgramV1 {
    AppendOnly,
    Pinned,
    Periodic {
        period_blocks: u64,
...
#[derive(Clone, Debug, Deserialize, Eq, PartialEq, Serialize)]
#[serde(tag = "kind", rename_all = "snake_case", deny_unknown_fields)]
pub enum ExecutionRetirementProgramV1 {
    Never,
...
#[derive(Clone, Debug, Deserialize, Eq, PartialEq, Serialize)]
#[serde(deny_unknown_fields)]
pub struct ExecutionTokenClassV1 {
    pub name: String,
    pub layers: Vec<u32>,
    #[serde(skip_serializing_if = "Option::is_none")]
    pub kv_head_range: Option<ExecutionKvHeadRangeV1>,
    pub bytes_per_token_per_layer: u64,
    pub address: ExecutionAddressProgramV1,
    pub retirement: ExecutionRetirementProgramV1,
    pub minimum_slots_per_request: Option<u64>,
    #[serde(default, skip_serializing_if = "ExecutionBlockDomainV1::is_all")]
    pub block_domain: ExecutionBlockDomainV1,
}

#[derive(Clone, Debug, Deserialize, Eq, PartialEq, Serialize)]
#[serde(deny_unknown_fields)]
pub struct ExecutionStateComponentV1 {
    pub name: String,
    pub bytes_per_token_per_layer: u64,
}
...
#[derive(Clone, Debug, Deserialize, Eq, PartialEq, Serialize)]
#[serde(tag = "kind", rename_all = "snake_case", deny_unknown_fields)]
pub enum ExecutionTokenBackendV1 {
    TokenSlots {
        storage: TokenStorageKind,
        components: Vec<ExecutionStateComponentV1>,
...
#[derive(Clone, Debug, Deserialize, Eq, PartialEq, Serialize)]
#[serde(deny_unknown_fields)]
pub struct ExecutionTokenStateV1 {
    pub name: String,
    pub layers: Vec<u32>,
    pub backend: ExecutionTokenBackendV1,
}

#[derive(Clone, Debug, Deserialize, Eq, PartialEq, Serialize)]
#[serde(tag = "kind", rename_all = "snake_case", deny_unknown_fields)]
pub enum ExecutionFixedBackendV1 {
...
#[derive(Clone, Debug, Deserialize, Eq, PartialEq, Serialize)]
#[serde(deny_unknown_fields)]
pub struct ExecutionFixedStateV1 {
    pub name: String,
    pub layers: Vec<u32>,
    pub backend: ExecutionFixedBackendV1,
}

/// Complete structural projection used for target admission.
#[derive(Clone, Debug, Deserialize, Eq, PartialEq, Serialize)]
#[serde(deny_unknown_fields)]
pub struct ExecutionSignatureV1 {
    pub schema: String,
    pub version: u32,
    pub fingerprint: String,
    ...
    pub page_tokens: u64,
    pub token_classes: Vec<ExecutionTokenClassV1>,
    pub token_states: Vec<ExecutionTokenStateV1>,
    pub fixed_states: Vec<ExecutionFixedStateV1>,
}
...
#[derive(Clone, Debug, Deserialize, Eq, PartialEq, Serialize)]
#[serde(deny_unknown_fields)]
pub struct RuntimeTargetBindingV1 {
    pub schema: String,
    pub version: u32,
    pub fingerprint: String,
    pub manifest_fingerprint: String,
    ...
    pub execution_topology: ExecutionTopologyV1,
    pub execution_signature: ExecutionSignatureV1,
}
...
pub enum TargetAdmissionError {
    ...
    #[error("target contract supported_topologies must be nonempty, sorted, and unique")]
    NonCanonicalTopologies,
    ...
    #[error("manifest topology is not one of the closed v1 execution shapes")]
    UnsupportedManifestTopology,
    #[error("target does not support manifest topology {0:?}")]
    UnsupportedTargetTopology(ExecutionTopologyV1),
...
impl RuntimeTargetContractV1 {
    /// Creates and fingerprints one strict v1 target contract.
    ///
    /// # Errors
    ///
    /// Returns an error when identities, geometry, topology order, or the
    /// derived fingerprint violate the closed v1 contract.
    pub fn new(
        target: RuntimeTargetIdentityV1,
        admission_profile: RuntimeAdmissionProfileV1,
        page_tokens: u64,
        supported_topologies: Vec<ExecutionTopologyV1>,
    ) -> Result<Self, TargetAdmissionError> {
        let mut contract = Self {
            schema: RUNTIME_TARGET_CONTRACT_SCHEMA.to_owned(),
            version: RUNTIME_TARGET_CONTRACT_VERSION,
    ...
    /// Parses and validates a serialized target contract.
    ///
    /// # Errors
    ///
    /// Returns an error for malformed, oversized, noncanonical, or tampered
    /// artifacts.
    pub fn from_json(bytes: &[u8]) -> Result<Self, TargetAdmissionError> {
        from_json(bytes, "runtime target contract")
    }
    ...
    pub fn validate(&self) -> Result<(), TargetAdmissionError> {
        validate_envelope(
            ...
            &self.schema,
            self.version,
            RUNTIME_TARGET_CONTRACT_SCHEMA,
        ...
        if self.supported_topologies.is_empty()
            || !self
                .supported_topologies
                .windows(2)
                .all(|pair| pair[0] < pair[1])
        {
            return Err(TargetAdmissionError::NonCanonicalTopologies);
        }
        validate_fingerprint("runtime target contract", &self.fingerprint, self)
    }
}

impl ExecutionSignatureV1 {
    ...
    pub fn validate(&self) -> Result<(), TargetAdmissionError> {
        ...
        if self.manifest_schema != RUNTIME_MANIFEST_SCHEMA
            || self.manifest_version != RUNTIME_MANIFEST_VERSION
...
impl RuntimeTargetBindingV1 {
    ...
    pub fn validate(&self) -> Result<(), TargetAdmissionError> {
        validate_envelope(
            ...
            &self.schema,
            self.version,
            RUNTIME_TARGET_BINDING_SCHEMA,
            RUNTIME_TARGET_BINDING_VERSION,
            &self.fingerprint,
        )?;
    ...
    /// Recomputes target admission and requires byte-meaningful typed equality.
    ///
    /// # Errors
    ///
    /// Returns an error when the binding is invalid or differs from a freshly
    /// admitted binding for the supplied manifest and target.
    pub fn validate_against(
        &self,
        manifest: &RuntimeManifest,
        target: &RuntimeTargetContractV1,
    ) -> Result<(), TargetAdmissionError> {
        self.validate()?;
        let expected = admit_runtime_manifest(manifest, target)?;
        if *self != expected {
...
pub fn derive_execution_signature(
    manifest: &RuntimeManifest,
) -> Result<ExecutionSignatureV1, TargetAdmissionError> {
    manifest.validate()?;
    let token_classes = manifest
        ...
        .map_or_else(Vec::new, |manager| {
            manager
                ...
                .classes
                .iter()
                .map(ExecutionTokenClassV1::from)
                .collect()
        });
    let mut token_states = Vec::new();
    let mut fixed_states = Vec::new();
    for state in &manifest.attention_state_plan.states {
        match state.backend {
            AttentionStateBackend::TokenSlots { .. } => {
                token_states.push(ExecutionTokenStateV1::from(state));
            }
            AttentionStateBackend::RecurrentCheckpoints { .. }
            | AttentionStateBackend::ConvolutionRing { .. } => {
                fixed_states.push(ExecutionFixedStateV1::from(state));
            }
        }
    }
    let mut signature = ExecutionSignatureV1 {
        ...
        page_tokens: manifest.attention_state_plan.page_tokens,
        token_classes,
        token_states,
        fixed_states,
    };
...
/// Structurally admits a manifest and emits a binding to the exact target
/// contract. No device, stream, tensor, or engine object is touched.
///
/// # Errors
///
/// Returns an error when either artifact is invalid, page geometry differs,
/// or the manifest topology is unsupported by the selected target.
pub fn admit_runtime_manifest(
    manifest: &RuntimeManifest,
    target: &RuntimeTargetContractV1,
) -> Result<RuntimeTargetBindingV1, TargetAdmissionError> {
    target.validate()?;
    ...
    let mut binding = RuntimeTargetBindingV1 {
        schema: RUNTIME_TARGET_BINDING_SCHEMA.to_owned(),
        version: RUNTIME_TARGET_BINDING_VERSION,
        fingerprint: String::new(),
        manifest_fingerprint: manifest.fingerprint.clone(),
...
fn validate_signature_projection(
    ...
    states.extend(
        signature
            .token_states
            .iter()
            .map(ExecutionTokenStateV1::to_input),
    );
    states.extend(
        signature
            .fixed_states
            .iter()
            .map(ExecutionFixedStateV1::to_input),
    );
    ...
    let expected_tokens = compiled
        ...
        .map(ExecutionTokenStateV1::from)
        .collect::<Vec<_>>();
    let expected_fixed = compiled
        .states
        .iter()
        .filter(|state| !matches!(state.backend, AttentionStateBackend::TokenSlots { .. }))
        .map(ExecutionFixedStateV1::from)
        .collect::<Vec<_>>();
...
fn classify_execution_signature(
    signature: &ExecutionSignatureV1,
) -> Result<ExecutionTopologyV1, TargetAdmissionError> {
    signature.validate()?;
    ...
    if fixed.is_empty() && classes.len() == 1 && tokens.len() == 1 {
        ...
        {
            return Ok(ExecutionTopologyV1::WholeDomainFullLatentKv);
        }
        if full_class(&classes[0], &tokens[0], TokenStorageKind::TokenKv)
            && covers_all_layers([tokens[0].layers.as_slice()])
        {
            return Ok(ExecutionTopologyV1::WholeDomainFullTokenKv);
        }
        if sliding_class(&classes[0], &tokens[0], signature.page_tokens)
            && covers_all_layers([tokens[0].layers.as_slice()])
        {
            return Ok(ExecutionTopologyV1::WholeDomainSlidingTokenKv);
        }
    }
    ...
    {
        return Ok(ExecutionTopologyV1::WholeDomainFullSlidingTokenKv);
    }
        ...
        && full_class(&classes[0], &tokens[0], TokenStorageKind::TokenKv)
    {
        ...
        {
            return Ok(ExecutionTopologyV1::WholeDomainFullTokenKvMamba);
        }
        if fixed.len() == 2 {
            let recurrent = fixed
                .iter()
            ...
            {
                return Ok(ExecutionTopologyV1::WholeDomainFullTokenKvGdnConvolution);
            }
        }
    }
...
fn full_class(
    class: &ExecutionTokenClassV1,
    state: &ExecutionTokenStateV1,
    storage: TokenStorageKind,
) -> bool {
    whole_domain(class)
        && class.name == state.name
        && class.layers == state.layers
        && matches!(class.address, ExecutionAddressProgramV1::AppendOnly)
        && matches!(class.retirement, ExecutionRetirementProgramV1::Never)
        && class.minimum_slots_per_request.is_none()
        && matches!(
            state.backend,
            ExecutionTokenBackendV1::TokenSlots {
                storage: actual,
...
fn sliding_class(
    ...
    let ExecutionTokenBackendV1::TokenSlots {
        storage,
        retention,
    ...
    matches!(
        class.address,
        ExecutionAddressProgramV1::Periodic { period_blocks: actual }
            if actual == period_blocks
    ) && matches!(
        class.retirement,
        ExecutionRetirementProgramV1::BlockEndPlus { offset_tokens }
            if offset_tokens == window_tokens - 1
    ) && class.minimum_slots_per_request == Some(period_blocks)
}

fn whole_domain(class: &ExecutionTokenClassV1) -> bool {
    class.kv_head_range.is_none() && class.block_domain.is_all()
}

fn recurrent_state(state: &ExecutionFixedStateV1, family: RecurrentFamily, slots: u32) -> bool {
    matches!(
        state.backend,
        ExecutionFixedBackendV1::RecurrentCheckpoints {
            family: actual,
...
fn convolution_state(state: &ExecutionFixedStateV1, slots: u32) -> bool {
    matches!(
        state.backend,
        ExecutionFixedBackendV1::ConvolutionRing {
            checkpoint_slots_per_request,
            token_relocatable: false,
            ..
...
impl From<&crate::plan::ClassLayoutProgram> for ExecutionTokenClassV1 {
...
impl From<&KvHeadRange> for ExecutionKvHeadRangeV1 {
...
impl From<&BlockDomain> for ExecutionBlockDomainV1 {
...
impl From<&AddressProgram> for ExecutionAddressProgramV1 {
...
impl From<&RetirementProgram> for ExecutionRetirementProgramV1 {
...
impl From<&CompiledAttentionState> for ExecutionTokenStateV1 {
    fn from(value: &CompiledAttentionState) -> Self {
        ...
        Self {
            name: value.name.clone(),
            layers: value.layers.clone(),
            backend: ExecutionTokenBackendV1::TokenSlots {
                storage: *storage,
                components: components
                    .iter()
                    .map(|component| ExecutionStateComponentV1 {
...
impl From<&CompiledAttentionState> for ExecutionFixedStateV1 {
    fn from(value: &CompiledAttentionState) -> Self {
...
impl ValidateArtifact for RuntimeTargetContractV1 {
    fn validate_artifact(&self) -> Result<(), TargetAdmissionError> {
        self.validate()
    }
}

impl ValidateArtifact for ExecutionSignatureV1 {
    fn validate_artifact(&self) -> Result<(), TargetAdmissionError> {
        self.validate()
    }
}

impl ValidateArtifact for RuntimeTargetBindingV1 {
    fn validate_artifact(&self) -> Result<(), TargetAdmissionError> {
        self.validate()
    }
}
...
mod tests {
    ...
    include!("executor_capabilities_tests.rs");
}
