...
//!
//! Command bindings use these regions for both Oxide-owned allocations and
...
impl<T: DeviceCopy> ReadDeviceRegion<T> {
    ...
    ///   [`crate::interop::StreamOrderedEngineAuthority`] only after Oxide has
    ///   enqueued its post-event wait. Oxide keeps the retained binding private
...
impl<T: DeviceCopy> ReadWriteDeviceRegion<T> {
    ...
    ///   [`crate::interop::StreamOrderedEngineAuthority`] returned after Oxide
    ///   enqueues a post-event wait. It may access the range only through that
    ///   same stream; it does not permit host or cross-stream access. Oxide does
    ...
    /// Recovers a Oxide-owned allocation. External regions are returned
