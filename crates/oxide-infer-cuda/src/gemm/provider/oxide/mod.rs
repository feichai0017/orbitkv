//! Oxide cuda-oxide provider for dense BF16 GEMM.
...
pub(crate) use sm90::OxideBf16DensePlan;

const H20_DEVICE_NAME: &str = "NVIDIA H20";
const H20_COMPUTE_CAPABILITY: (i32, i32) = (9, 0);
const SM90A_ARTIFACT_TARGET: &str = "sm_90a";

/// Lazily loads the experimental Oxide artifact only for explicit Oxide plans.
#[derive(Clone)]
pub(crate) struct OxideProvider {
    ...
    pub(crate) fn plan_bf16_dense(
        ...
        let provider = {
            let mut slot = self
                .sm90
                .lock()
                .map_err(|_| Bf16DenseGemmPlanError::OxideProviderLockPoisoned)?;
            if slot.is_none() {
                validate_h20_target(&self.context)?;
                *slot = Some(Sm90Provider::load(&self.context)?);
            }
            slot.as_ref()
                .expect("Oxide SM90 provider was initialized above")
                .clone()
        };
...
fn validate_h20_target(context: &CudaContext) -> Result<(), Bf16DenseGemmPlanError> {
    ...
    if device != H20_DEVICE_NAME {
        return Err(Bf16DenseGemmPlanError::OxideUnsupportedDevice { device });
    }

    let (major, minor) = context.compute_capability()?;
    if (major, minor) != H20_COMPUTE_CAPABILITY {
        return Err(Bf16DenseGemmPlanError::OxideUnsupportedComputeCapability { major, minor });
    }
    ...
    if actual.as_deref() != Some(SM90A_ARTIFACT_TARGET) {
        return Err(Bf16DenseGemmPlanError::OxideUnsupportedArtifactTarget { actual });
    }
    Ok(())
}
