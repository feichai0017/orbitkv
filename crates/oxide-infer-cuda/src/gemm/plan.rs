...
use crate::gemm::provider::{CublasLtBf16DensePlan, OxideBf16DensePlan};
...
impl Bf16DenseGemmPlan {
    ...
    pub(crate) const fn from_oxide(plan: OxideBf16DensePlan) -> Self {
        Self {
            inner: Bf16DenseGemmPlanInner::Oxide(plan),
        }
    }
    ...
    pub fn spec(&self) -> Bf16DenseGemmSpec {
        match &self.inner {
            ...
            Bf16DenseGemmPlanInner::Oxide(plan) => plan.spec(),
        }
    }
    ...
    pub fn plan_info(&self) -> Bf16DenseGemmPlanInfo {
        match &self.inner {
            ...
            Bf16DenseGemmPlanInner::Oxide(plan) => Bf16DenseGemmPlanInfo {
                provider: GemmProviderId::Oxide,
                algorithm: Bf16DenseGemmAlgorithm::OxideSm90SimtGemvM1N16K64,
                workspace_required_bytes: plan.workspace_required_bytes(),
                tensor_alignment_bytes: plan.tensor_alignment_bytes(),
                workspace_alignment_bytes: plan.workspace_alignment_bytes(),
            },
    ...
    pub fn estimated_waves_count(&self) -> Option<f32> {
        match &self.inner {
            ...
            Bf16DenseGemmPlanInner::Oxide(_) => None,
        }
    }
    ...
    pub fn enqueue_into(
        ...
        match &self.inner {
            ...
            Bf16DenseGemmPlanInner::Oxide(plan) => plan.enqueue_into(scope, operands),
        }
    }
}
...
pub enum Bf16DenseGemmPlanError {
    ...
    #[error("Oxide SM90 GEMV requires M=1, got {m}")]
    OxideMNotOne { m: usize },
    #[error("Oxide SM90 GEMV requires N to be a multiple of 16, got {n}")]
    OxideNNotMultipleOf16 { n: usize },
    #[error("Oxide SM90 GEMV requires K to be a multiple of 64, got {k}")]
    OxideKNotMultipleOf64 { k: usize },
    #[error("Oxide SM90 GEMV requires device 'NVIDIA H20', got '{device}'")]
    OxideUnsupportedDevice { device: String },
    #[error("Oxide SM90 GEMV requires compute capability 9.0, got {major}.{minor}")]
    OxideUnsupportedComputeCapability { major: i32, minor: i32 },
    #[error("Oxide SM90 GEMV requires an sm_90a artifact, got {actual:?}")]
    OxideUnsupportedArtifactTarget { actual: Option<String> },
    #[error("Oxide SM90 GEMV grid needs {blocks} blocks, exceeding the CUDA x-grid range")]
    OxideGridDimensionOutOfRange { blocks: usize },
    #[error("Oxide provider initialization lock is poisoned")]
    OxideProviderLockPoisoned,
    #[error(transparent)]
