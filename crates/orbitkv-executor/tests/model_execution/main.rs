...
fn model_harness_with_geometry(
    config_bytes: &[u8],
    maximum_requests: usize,
    maximum_operations: usize,
    maximum_step_tokens: usize,
    ...
    let manifest = compile_hf_runtime_manifest(config_bytes, retention).unwrap();
...
#[test]
#[ignore = "requires the Qwen3.8 27B FP8 checkpoint, CUDA, and provider headers"]
#[allow(clippy::too_many_lines)]
fn qwen38_fp8_bounded_prefill_decode_executes_and_drains() {
    decoder_replay::run(1);
}

#[test]
#[ignore = "requires a block-FP8 checkpoint, CUDA, provider headers and independent logits"]
fn quantized_decoder_bucket_transitions() {
    decoder_replay::run(4);
}

#[test]
#[ignore = "requires a block-FP8 checkpoint, independent reference logits, CUDA and provider headers"]
fn quantized_decoder_batched_reference_and_drain() {
    quantized_batch::run();
}
...
fn assert_reference_logits(
    ...
    eprintln!(
        "Qwen3.8 27B FP8 {phase} parity: actual_top={actual_top:?} reference_top={reference_top:?} max_abs={maximum_absolute_difference}"
    );
    ...
    let reference_margin = reference_top[0].1 - reference_top[1].1;
    let reference_token = reference_top[0].0;
    let actual_expected = actual[usize::try_from(reference_token).unwrap()];
    let actual_margin = actual_top[0].1 - actual_expected;
    if reference_margin > maximum_absolute_difference {
        assert_eq!(actual_top[0].0, reference_token);
    } else {
        assert!(
            actual_margin <= maximum_absolute_difference,
            "{phase} top-1 changed beyond the measured numerical envelope: actual winner {:?}, expected {reference_token}, margin {actual_margin}, max_abs {maximum_absolute_difference}",
            actual_top[0].0,
        );
    }
    reference_token
