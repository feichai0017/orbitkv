...
fn main() -> Result<(), Box<dyn Error>> {
    ...
    if single_trace.provider() != "loom-infer-cuda"
...
fn run_command_capacity_case(
    ...
    plan: &loom_infer_cuda::attention::Bf16PagedBatchDecodePlan,
    ...
    if !matches!(
        ...
        EngineEnqueueCause::Command(
            loom_infer_cuda::command::CommandError::CommandCapacityExceeded { capacity: 1 }
        )
...
// stream before Loom sees them. This module contains all raw-pointer setup.
#[allow(unsafe_code)]
mod raw_engine {
    ...
    use half::bf16;
    use loom_infer::{Bf16PagedBatchDecodeSpec, Bf16SingleDecodeSpec};
    ...
    // allocation lease, so dropping this value cannot invalidate Loom work.
