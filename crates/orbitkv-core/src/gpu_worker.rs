...
fn finish_gpu_transfer(
    ...
    if let Err(error) = stream.synchronize() {
        error!("Cannot establish GPU transfer completion: {error}; terminating Cache Manager");
        // Returning an error would let clients recycle pages still used by DMA.
        std::process::abort();
    }
    submitted.map_err(EngineError::Storage)
}
