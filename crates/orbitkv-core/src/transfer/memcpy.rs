...
pub(super) fn merged_ranges(mut copies: &[CopyDesc]) -> impl Iterator<Item = Merged> + '_ {
    std::iter::from_fn(move || {
        ...
        while let Some((&next, remaining)) = copies.split_first() {
            ...
            let host_contiguous = start.host.wrapping_add(size) == next.host;
