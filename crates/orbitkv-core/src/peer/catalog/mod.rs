...
use crate::peer::candidates::{CANDIDATE_CACHE_BYTES, CandidateIndex};
use log::warn;
...
impl CatalogClient {
    pub(crate) fn new(
        ...
        let cache = dram.upgrade().ok_or("cache has stopped")?;
    ...
    async fn flush_with_timeout(&self, timeout: Duration) -> Result<(), String> {
        let cache = self.dram.upgrade().ok_or("cache has stopped")?;
        let waits = self
            .streams
            .iter()
            .enumerate()
...
#[cfg(test)]
#[path = "../../../tests/unit/peer/catalog/mod.rs"]
mod tests;
