...
fn cache(journal: usize) -> Arc<DramStore> {
    Arc::new(DramStore::new(
        64 * 1024 * 1024,
        false,
        None,
...
fn insert(cache: &DramStore, key: u32) {
...
fn client(server: &TestServer, cache: &Arc<DramStore>) -> CatalogClient {
...
async fn discovery_coalesces_bounds_batches_and_reuses_only_positive_versioned_evidence() {
    ...
    for response in &responses {
        ...
        assert!(rows.iter().all(|r| r.replicas.len() == 1
            && r.replicas[0].owner.incarnation == owner.membership.owner().incarnation));
    }
...
async fn discovery_deadline_spans_batches_and_keeps_later_local_and_cached_candidates() {
    ...
    let discovery = async {
        ...
        for (batch, hashes) in hashes.chunks(DISCOVERY_MAX_KEYS).enumerate() {
            let rows = crate::planning::discover::discover(
                &destination,
                None,
                Some(&requester),
                "ns",
