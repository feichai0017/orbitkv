mod compilation;
use crate::egglog_utils::log_channel_enabled;
use crate::op::LLIROp;
pub use crate::search::unroll::{collapse_loops_to_first_iter, unroll_loops_in_llir};
use crate::search::{SearchSpace, bucket_index_combinations};
...
pub use artifact::{ScheduleBucket, SelectedSchedule, llir_program_identity};
...
pub struct CompileOptions {
    ...
    /// Initial genomes drawn from shuffled per-class coverage cycles,
    /// including the first executable seed.
    /// After that seed, rejected and duplicate programs consume this allowance too.
    /// Finding the first executable seed uses the separate initial retry policy.
    /// Clamped to the graph budget. One preserves a single initial parent.
    ...
    /// Optional explicit joint representative shapes. Only these bucket combinations
    /// are compiled; every supplied map must cover all bucketed dimensions, and
    /// at most one representative may select each combination. Callers must
    /// cover every combination their runtime input contract admits.
    pub bucket_representatives: Option<Vec<DynMap>>,
    /// Enable egglog progress logging. Quiet by default; overridden by
    /// `EGGLOG_LOG=1` or `ORBITKV_LOG=1`.
    pub egglog_log: bool,
    /// Enable automatic loop rolling and its diagnostics. Disabled by default;
    /// overridden by `ROLLING_LOG=1` or `ORBITKV_LOG=1`.
    pub rolling_log: bool,
    /// Enable search progress logging. Enabled by default; overridden by
    /// `SEARCH_LOG=0`/`1` or `ORBITKV_LOG=1`.
    pub search_log: bool,
    /// Optional structured runtime search trace. CUDA writes a new JSONL file;
    /// an existing path is rejected so a previous experiment is never replaced.
    /// This diagnostic destination is not part of a selected schedule identity.
    pub search_trace: Option<std::path::PathBuf>,
    /// Caller-supplied declarations and facts made visible to backend rewrite
    /// rules. Backends remain responsible for defining any rules that consume
    /// them. This is intended for stable compile-time contracts such as an
    /// external state manager's storage, lifetime, and arena invariants.
...
impl CompileOptions {
    ...
    pub fn search_trace(mut self, path: impl Into<std::path::PathBuf>) -> Self {
        self.search_trace = Some(path.into());
        self
    }
    ...
    /// Reserve part of the existing graph budget for broad initial coverage.
    pub fn initial_population(mut self, candidates: usize) -> Self {
        self.initial_population = candidates.max(1);
        self
    }
    ...
    /// Compile only the bucket combinations selected by these representative shapes.
    /// Unlike independent dimension representatives, these preserve correlations
    /// between dimensions without teaching core application-specific constraints.
    pub fn bucket_representatives(mut self, representatives: Vec<DynMap>) -> Self {
        self.bucket_representatives = Some(representatives);
        self
    }
    ...
    /// Add caller-owned declarations and facts to every bucket's egglog
    /// program. The text is inserted after backend declarations and before
    /// rewrite execution.
    pub fn compiler_facts(mut self, facts: impl Into<String>) -> Self {
        self.compiler_facts = facts.into();
        self
    }
...
impl Default for CompileOptions {
    fn default() -> Self {
        Self {
            ...
            bucket_representatives: None,
            egglog_log: false,
            rolling_log: false,
            search_log: true,
            search_trace: None,
            compiler_facts: String::new(),
        }
    }
}

fn joint_bucket_combinations(options: &CompileOptions) -> Vec<(DynMap, Option<DynMap>)> {
    let Some(representatives) = &options.bucket_representatives else {
        return bucket_index_combinations(&options.dim_buckets)
            .into_iter()
            .map(|indices| (indices, None))
            .collect();
    };
    assert!(
        !representatives.is_empty(),
        "joint bucket representatives must not be empty"
    );
    let mut selected = Vec::with_capacity(representatives.len());
    for representative in representatives {
        assert_eq!(
            representative.len(),
            options.dim_buckets.len(),
            "joint representatives must cover exactly the bucketed dimensions"
        );
        let indices = options
            .dim_buckets
            .iter()
            .map(|(&dim, buckets)| {
                let value = representative
                    .get(&dim)
                    .expect("joint representative missing dimension");
                let index = buckets
                    .iter()
                    .position(|bucket| bucket.contains(*value))
                    .expect("joint representative is outside dimension buckets");
                (dim, index)
            })
            .collect::<DynMap>();
        assert!(
            !selected.iter().any(|(existing, _)| *existing == indices),
            "duplicate joint bucket combination"
        );
        selected.push((indices, Some(representative.clone())));
    }
    selected
}
...
impl Graph {
    ...
    fn run_auto_loop_rolling_prepass(&mut self, options: &CompileOptions) {
        ...
        let max_regions = std::env::var("ORBITKV_MAX_ROLLED_REGIONS")
            .ok()
            .and_then(|value| value.parse::<usize>().ok())
            .unwrap_or(2);
        while rolled < max_regions && self.auto_roll_loops_prepass_with_log(log) > 0 {
            rolled += 1;
        }
        if rolled == 0 {
            println!(
    ...
    fn best_rolling_candidate(&self, max_region_size: usize) -> RollingSearchReport {
        ...
        // Source nodes, Outputs, and loop markers are region boundary, not
        // body: they feed or drain repeated windows without belonging to
        // them. Source nodes include named Inputs as well as constants/iotas.
        // Keeping a layer-local offset constant in the body makes otherwise
        // identical stateful layers hash differently; as a boundary stream it
        // is instead selected per iteration, just like a layer-local weight.
        // Markers especially must be excluded — after one roll, per-iteration
        // values arrive through LoopInput markers whose stream ids differ, and
        // letting them into windows breaks the hash match for otherwise-
        // identical bodies nested inside the roll.
        ...
        // Restrict each subsequent search to one scope as well. Merely checking
        // a completed occurrence is too late: windows built from a globally
        // interleaved topological order can skip markers yet still mix an
        // existing loop body with outside nodes, which creates foreign-marker
        // bridges when nested.
        ...
        let mut scopes = topo
            .iter()
            .map(|node| node_scope.get(node).copied())
            .collect_vec();
        ...
        // isn't rolled (correctness preserved). Tunable via ORBITKV_MAX_ROLL_BODY.
        let roll_body_cap = std::env::var("ORBITKV_MAX_ROLL_BODY")
            .ok()
        ...
        for scope in scopes {
            let scoped_topo = topo
                .iter()
                .copied()
                .filter(|node| node_scope.get(node).copied() == scope)
                .collect_vec();
            let topo_index: FxHashMap<NodeIndex, usize> = scoped_topo
                .iter()
                .enumerate()
                .map(|(i, &n)| (n, i))
                .collect();
            let max_window = max_region_size
                .min(scoped_topo.len() / 2)
                .min(roll_body_cap);
            let node_hashes = scoped_topo
                .iter()
                .map(|&node| cheap_rolling_node_hash(&self.graph, node, &self.custom_ops))
                .collect_vec();
            let rolling_hash = RollingHash64::new(&node_hashes);
            let mut scope_best: Option<RollingCandidate> = None;
            let mut scope_runs: Vec<RollingRun> = Vec::new();

            // Search all window sizes down to 1, using cheap rolling hashes
            // only as a gate for expensive canonicalization. Candidate
            // selection remains purely based on valid HLIR-op reduction.
            for window in rolling_probe_window_sizes(max_window) {
                let mut start = 0usize;
                while start + window * 2 <= scoped_topo.len() {
                    ...
                    let mut occs = vec![];
                    let mut starts = vec![];
                    let first_nodes = scoped_topo[start..start + window].to_vec();
                    ...
                    while pos + window <= scoped_topo.len() {
                        if rolling_hash.window_hash(pos, window) != first_hash {
                            break;
                        }
                        let nodes = scoped_topo[pos..pos + window].to_vec();
                    ...
                    let replace = scope_best.as_ref().is_none_or(|b| {
                        (rolling_net_savings(&candidate), candidate.occurrences.len())
                            > (rolling_net_savings(b), b.occurrences.len())
                    });
                    if replace {
                        scope_best = Some(candidate);
                    }
                    start = pos.saturating_sub(window).max(start + 1);
                }
            }
            let mut grown_best = scope_best.map(|best| {
                // `best` is already rollable (gated above). Growing only ever
                // extends occurrences, which could re-introduce a cross-
                // occurrence dependency; if it does, keep the validated seed.
                let seed = best.clone();
            ...
            for run in &scope_runs {
                let state_param_indices =
                    collect_state_params(&run.occurrences, &uses, &self.graph);
            ...
            if let Some(candidate) = grown_best {
                let replace = best_overall.as_ref().is_none_or(|best| {
                    (rolling_net_savings(&candidate), candidate.occurrences.len())
                        > (rolling_net_savings(best), best.occurrences.len())
                });
                if replace {
    ...
    /// let a = cx.tensor(3);
    /// let b_id = cx.add_op(
    ///     orbitkv_compiler::hlir::Mul { input_shapes: vec![a.shape, a.shape], ..Default::default() },
    ///     &[a.id],
    /// );
...
#[cfg(test)]
#[path = "../tests/unit/graph/joint_bucket_tests.rs"]
mod joint_bucket_tests;
