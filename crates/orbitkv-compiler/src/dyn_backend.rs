...
/// for dynamic consumers without requiring
...
/// struct layout.
pub const BACKEND_FACTORY_CAPSULE_NAME: &std::ffi::CStr = c"orbitkv.compiler.backend_factory.v2";
...
pub fn compile_backend<Rt: Runtime + 'static>(
    ...
    for (label, bytes, dtype) in &args.weights {
        if !args.device_ptrs.contains_key(label)
            && let Some(&node_id) = label_map.get(label)
        {
...
#[cfg(test)]
#[path = "../tests/unit/dyn_backend/mod.rs"]
mod tests;
