...
fn materialize_unrolled_view(
    llir: &impl MaterializeLLIRView,
    mut record_origin: impl FnMut(usize),
) -> Option<LLIRGraph> {
    use crate::hlir::{
...
pub(crate) fn materialize_unrolled_llir(llir: &LLIRGraph) -> Option<LLIRGraph> {
    materialize_unrolled_view(&StableMaterializeView::new(llir), |_| {})
}
...
pub fn unroll_packed_llir(llir: PackedLLIRGraph) -> LLIRGraph {
    ...
    let graph = materialize_unrolled_view(&llir, |_| {})
...
/// Materialize executable nodes and their extraction provenance together.
/// Loop copies retain their rolled choices; eliminated markers own no work.
pub(crate) fn unroll_with_origins(
    llir: PackedLLIRGraph,
) -> (LLIRGraph, Vec<std::sync::Arc<[super::profile::ChoiceSite]>>) {
    let mut origins = Vec::new();
    let graph = materialize_unrolled_view(&llir, |node| {
        origins.push(llir.origins[node].clone());
    })
    .expect("rolled LLIR must satisfy the loop materialization invariants");
    (graph, origins)
}
...
#[cfg(test)]
#[path = "../../tests/unit/search/unroll_provenance.rs"]
