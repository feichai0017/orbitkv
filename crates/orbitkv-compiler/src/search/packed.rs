...
pub struct PackedLLIRGraph {
    pub ops: Vec<LLIROp>,
    // Exact extraction choices for each rolled operation, excluding upstream
    // IR operands. Sidecar metadata never enters program fingerprints.
    pub(crate) origins: Vec<std::sync::Arc<[crate::search::profile::ChoiceSite]>>,
