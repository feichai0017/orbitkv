...
//! (`LLIR_DUMP_DIR`), best-candidate logs (`ORBITKV_LOG_LLIR`), and the
//! per-candidate op histogram log (`ORBITKV_CANDIDATE_OPS`).
...
/// the file named by `ORBITKV_CANDIDATE_OPS` — search-trajectory forensics
/// for "was family X ever generated, and what did it measure".
pub fn log_candidate_ops(llir: &LLIRGraph, tag: &str) {
    static PATH: std::sync::OnceLock<Option<String>> = std::sync::OnceLock::new();
    let Some(path) = PATH.get_or_init(|| std::env::var("ORBITKV_CANDIDATE_OPS").ok()) else {
        return;
    };
...
/// When `ORBITKV_LOG_LLIR=1`, print a canonical, diffable dump of a
...
pub fn log_best_llir(llir: &LLIRGraph, context: &str) {
    ...
    if !*ENABLED.get_or_init(|| std::env::var_os("ORBITKV_LOG_LLIR").is_some()) {
        return;
    }
