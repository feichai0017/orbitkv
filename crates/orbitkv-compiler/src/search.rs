...
//! - [`GeneticSearch`]: OrbitKV's genetic search as a pull-style state
...
pub use genetic::{Candidate, CandidateId, GeneticSearch, Outcome, Ranked, SamplingOrigin};
...
pub struct BucketSearchSpace {
    ...
    /// unbucketed.
    pub bucket_indices: DynMap,
    /// Optional correlated representative values supplied by the caller.
    pub representative_override: Option<DynMap>,
    ...
    pub fn bucket_contexts(&self, base_dyn_map: &DynMap) -> Vec<BucketContext<'_>> {
        self.buckets
            ...
            .map(|(index, bucket)| {
                ...
                if let Some(overrides) = &bucket.representative_override {
                    representative_dyn_map
                        .extend(overrides.iter().map(|(&dim, &value)| (dim, value)));
                }
                BucketContext {
                    space: self,
                    index,
