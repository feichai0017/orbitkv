...
//!
//! OrbitKV's contract is legality by construction: every rewrite may only add
...
//! paths). Per-event details are printed when `ORBITKV_MASK_LOG=1`. A summary
...
impl MaskEvent {
    ...
    /// Record and, under `ORBITKV_MASK_LOG=1`, print the event detail.
...
fn mask_log_enabled() -> bool {
    ...
    *ENABLED.get_or_init(|| std::env::var_os("ORBITKV_MASK_LOG").is_some())
}
