...
fn register_ranks(engine: &OrbitKVEngine, shape: &Shape, page_first: bool) -> Vec<RankBuffers> {
...
async fn run_holder(cli: &Cli, shape: &Shape, pool_bytes: usize) {
    ...
    let engine = Arc::new(
        OrbitKVEngine::new_with_config(pool_bytes, cli.use_hugepages, config)
...
async fn main() {
    let cli = Cli::parse();
    orbitkv_common::logging::init_stdout_colored("info");
