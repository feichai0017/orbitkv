use orbitkv_core::{trace_in_span, trace_root};
...
pub struct GrpcEngineService {
    ...
    shutdown: Arc<Notify>,
    hll_tracker: Arc<std::sync::Mutex<orbitkv_common::hll::MultiWindowHllTracker>>,
...
impl GrpcEngineService {
    ...
    fn build_transfer_slot_info(
        ...
        let layer_block = orbitkv_core::LayerBlock::new(raw_block);
    ...
    async fn register_context_batch(
        ...
        let result: Result<Response<RegisterContextResponse>, Status> = async {
            ...
            let transfer_mode = match req.transfer_mode() {
                ProtoTransferMode::Direct => orbitkv_core::TransferMode::Direct,
                ProtoTransferMode::Kernel => orbitkv_core::TransferMode::Kernel,
            };
        ...
        let fut = async {
            ...
            let outcome = execute_query(
                ...
                QueryInput {
                    ...
                    wait_for_full_prefix: req.wait_for_full_prefix,
            ...
            let outcome = match outcome {
                QueryOutcome::Loading => query_response::Outcome::Loading(QueryLoading {}),
                QueryOutcome::Ready {
                    num_hit_blocks,
                    lease,
                    hit_positions,
...
mod tests {
    ...
    fn validate_register_context_rejects_client_version_mismatch() {
        ...
        assert!(err.message().contains("OrbitKV version mismatch"));
