...
pub(super) fn engine_with_channel(
    ...
    let engine = ModelEngine {
        shared: Arc::new(EngineShared {
            ...
            startup_report: EngineStartupReport::default(),
...
#[test]
fn search_budget_accepts_one_candidate_and_rejects_zero() {
    let mut config = config(vec![64, 33]);
    config.search_graphs = 1;
    assert_eq!(config.validate(), Ok(()));
    config.search_graphs = 0;
    assert_eq!(config.validate(), Err(ModelEngineError::InvalidConfig));
}
