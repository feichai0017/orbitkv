...
#[derive(Parser, Debug)]
#[command(
    name = "orbitkv-metaserver",
    version,
    about = "OrbitKV MetaServer - manages block hash keys across multi-node instances"
)]
pub struct Cli {
...
pub async fn run() -> Result<(), Box<dyn Error>> {
    let cli = Cli::parse();
    orbitkv_common::logging::init_stderr(&cli.log_level);
    info!("Starting orbitkv-metaserver v{}", env!("CARGO_PKG_VERSION"));
