...
pub fn register_store_gauges(store: &Arc<BlockHashStore>) {
    ...
    STORE_GAUGES.get_or_init(|| {
        let meter = global::meter("orbitkv_metaserver");
        let entries_store = Arc::clone(&s);
        let entries = meter
            .u64_observable_gauge("orbitkv_metaserver_store_entries")
        ...
        let owners = meter
            .u64_observable_gauge("orbitkv_metaserver_block_owners")
        ...
        let nodes = meter
            .u64_observable_gauge("orbitkv_metaserver_nodes")
...
static SWEEP_METRICS: LazyLock<SweepMetrics> = LazyLock::new(|| {
    ...
    SweepMetrics {
        removed_owners: meter
            .u64_counter("orbitkv_metaserver_sweep_removed_owners")
            .with_description("Total node ownership records removed by lifecycle sweep")
            .build(),
        removed_keys: meter
            .u64_counter("orbitkv_metaserver_sweep_removed_keys")
            .with_description("Total block keys removed by lifecycle sweep")
            .build(),
        removed_nodes: meter
            .u64_counter("orbitkv_metaserver_sweep_removed_nodes")
...
impl RpcMetrics {
    fn new() -> Self {
        let meter = global::meter("orbitkv_metaserver_rpc");
        let request_count = meter
            .u64_counter("orbitkv_metaserver_rpc_requests")
            .with_description("Total RPC requests handled by orbitkv metaserver")
            .build();
        let request_duration = meter
            .f64_histogram("orbitkv_metaserver_rpc_duration")
