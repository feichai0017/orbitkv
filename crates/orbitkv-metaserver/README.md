...
A gRPC server for managing block hash keys across multi-node OrbitKV instances. The MetaServer provides a centralized registry for tracking which block hashes exist across distributed deployments.

## Overview

The MetaServer acts as a coordination service for distributed OrbitKV deployments. It maintains a global registry of block hash keys, allowing OrbitKV instances to:
...
```bash
# Build debug version
cargo build -p orbitkv-metaserver

# Build release version
cargo build -p orbitkv-metaserver --release

# Run tests
cargo test -p orbitkv-metaserver
```
...
cargo run -p orbitkv-metaserver

# Custom bind address
cargo run -p orbitkv-metaserver -- --addr 0.0.0.0:50056

# With debug logging
cargo run -p orbitkv-metaserver -- --log-level debug

# Custom node lifecycle timings
cargo run -p orbitkv-metaserver -- --node-stale-secs 30 --ttl-minutes 120 --sweep-interval-secs 600

# Configure the HTTP listener
cargo run -p orbitkv-metaserver -- --addr 0.0.0.0:50056 --http-addr 0.0.0.0:9092

# Show all options
cargo run -p orbitkv-metaserver -- --help
```
