...
impl EgglogOp for KernelGather {
    ...
    fn rewrites(&self) -> Vec<Rule> {
        ...
        let hlir_gather = orbitkv_compiler::hlir::Gather::default().sort();
...
impl EgglogOp for KernelScatter {
    ...
    fn rewrites(&self) -> Vec<Rule> {
        ...
        let hlir_scatter = orbitkv_compiler::hlir::Scatter::default().sort();
...
impl EgglogOp for KernelIota {
    ...
    fn rewrites(&self) -> Vec<Rule> {
        let (args, hlir_iota_kind) = orbitkv_compiler::hlir::Iota::default().sort().new_call();
...
impl EgglogOp for KernelConstant {
    ...
    fn rewrites(&self) -> Vec<Rule> {
        let (mut args, const_kind) = orbitkv_compiler::hlir::Constant::default()
            .sort()
            .new_call();
        let hlir_inputs = v("?__inputs");
...
impl EgglogOp for KernelCast {
    ...
    fn rewrites(&self) -> Vec<Rule> {
        ...
        let hlir_cast = orbitkv_compiler::hlir::Cast::default().sort();
