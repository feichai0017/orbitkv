...
struct CompiledKernel {
    ...
    node: NodeIndex,
    /// LLIR operations compiled into this step, including fused interiors.
...
fn cublaslt_capture_cache_capacity() -> usize {
    std::env::var("ORBITKV_CUBLASLT_CAPTURE_CACHE_CAPACITY")
        .ok()
...
struct CachedFlashInferPrepare {
    ...
    prepared: Rc<PreparedFlashInferAttention>,
...
impl RecaptureProfile {
    fn new() -> Self {
        Self {
            enabled: std::env::var_os("ORBITKV_CUDA_PROFILE_RECAPTURE").is_some(),
            ..Default::default()
        }
    }
...
impl CudaGraphOp {
    ...
    fn new(
        ...
        if !captured_host_dyn_dims.is_empty()
            && std::env::var_os("ORBITKV_CUDA_DEBUG_CAPTURED_DIMS").is_some()
        {
            eprintln!(
    ...
    fn host_device_memory_plan(
        ...
        let mut flashinfer_bytes = 0usize;
        for (idx, op) in state.flashinfer_ops.iter().enumerate() {
            ...
            if !shares_existing {
                flashinfer_bytes = flashinfer_bytes
        ...
        persistent_bytes = persistent_bytes.checked_add(flashinfer_bytes).ok_or(
            ResourceViolation::ArithmeticOverflow {
                resource: "CUDA graph prepared device memory",
            },
        )?;
        // Recapture prepares all replacement FlashInfer plans before retiring
        // the old islands. One replacement generation can coexist with every
        // resident variant; only the active variant is updated at a time.
        let mut transient_peak_bytes = flashinfer_bytes;
...
impl EgglogOp for CudaGraphOp {
    fn sort(&self) -> orbitkv_compiler::egglog_utils::api::SortDef {
        orbitkv_compiler::egglog_utils::api::sort(OP_KIND, "CudaGraphOp", &[])
    }
    ...
    fn extract<'a>(
        &'a self,
        _egraph: &'a orbitkv_compiler::egglog_utils::SerializedEGraph,
        _kind_children: &[&'a orbitkv_compiler::prelude::ENodeId],
        _input_enodes: Vec<&'a orbitkv_compiler::prelude::ENodeId>,
        _list_cache: &mut FxHashMap<&'a orbitkv_compiler::prelude::ENodeId, Vec<Expression>>,
        _expr_cache: &mut FxHashMap<&'a orbitkv_compiler::prelude::ENodeId, Expression>,
    ) -> (LLIROp, Vec<&'a orbitkv_compiler::prelude::ENodeId>) {
...
fn get_or_prepare_cublaslt(
    ...
    if std::env::var_os("ORBITKV_CUDA_DEBUG_CUBLASLT_PREPARE_CACHE").is_some() {
        eprintln!(
...
fn get_or_prepare_flashinfer(
    ...
    step: usize,
    prepare: impl FnOnce() -> anyhow::Result<PreparedFlashInferAttention>,
) -> anyhow::Result<(Rc<PreparedFlashInferAttention>, bool)> {
...
impl CudaGraphOp {
    ...
    fn reset_materialization_state_and_trim(
        ...
        // The allocations belong to a private capture stream, but their
        // kernels ran on this execution stream. Retire uses before owners.
        stream.synchronize()?;
    ...
    pub(crate) fn release_materialization(&self) {
        ...
        self.stream
            .synchronize()
            .expect("failed to retire CUDA graph executions before releasing capture resources");
        ...
        if !changed_dyn_vars.is_empty() {
            ...
            if changed_buffers.is_empty() {
                ...
                if state.body_only_dyn_fast_paths == 1
                    && std::env::var_os("ORBITKV_CUDA_DEBUG_BODY_DYN_FASTPATH").is_some()
                {
                    eprintln!(
    ...
    fn materialize_impl(
        ...
        if captured_host_shape_changed {
            self.reset_materialization_state_and_trim(&mut state, stream)
                .map_err(|error| {
                    anyhow::anyhow!("reset captured HostOp shape for dyn={dyn_map:?}: {error:#}")
                })?;
        }
        ...
        if state.cuda_graph.is_none() {
            ...
            self.build_graph(&mut state, stream, buffers, dyn_map)
                .map_err(|error| {
                    anyhow::anyhow!("build CUDA graph for dyn={dyn_map:?}: {error:#}")
                })?;
        ...
        if !changed_captured_nodes.is_empty() {
            if std::env::var_os("ORBITKV_CUDA_DEBUG_CAPTURED_REBUILD").is_some() {
        ...
        if needs_update {
            ...
            for &idx in &dirty_kernels {
                ...
                let cu_func = unsafe { kernel.function.raw_function() };
                let (kernel_name, kernel_node) = (kernel.kernel_name, kernel.node);
                let params_ptr = state.kernel_params[idx].as_cuda_params();
                let graph = state.cuda_graph.as_mut().unwrap();
                unsafe {
                    graph.set_kernel_node_params(
                    ...
                    .map_err(|error| {
                        anyhow::anyhow!(
                            "update source kernel node {graph_node:?} for {kernel_name} at {kernel_node:?}, launch={launch:?}: {error:?}",
                        )
                    })?;
                }
            }
            ...
            if recaptured_cublaslt {
                ...
                for &idx in &dirty_kernels {
                    ...
                    unsafe {
                        ...
                        .map_err(|error| {
                            anyhow::anyhow!(
                                "update executable kernel node {graph_node:?} for {kernel_name} at {kernel_node:?}, launch={launch:?}: {error:?}",
                            )
                        })?;
                    }
                }
    ...
    pub(crate) fn launch_steps(
        ...
        if profile_steps {
            while state.timing_events.len() < state.steps.len() + 1 {
                state
                    .timing_events
                    .push(create_cuda_event(stream.context())?);
            }
            crate::kernel::record_event_on_stream(
                stream.context(),
                state.timing_events[0],
                stream,
            )?;
        }
    ...
    fn recapture_cublaslt_island(
        ...
        let (child_node, active_prepared) = if let Some(cached) = cached {
            ...
            let child_node = graph
                .add_child_graph_node(upstream_nodes, &cached.graph)
                .map_err(|error| {
                    anyhow::anyhow!(
                        "add cached cuBLASLt child at {:?} with upstream={upstream_nodes:?}: {error:?}",
                        op.node,
                    )
                })?;
            if let Some(profile) = profile.as_deref_mut() {
            ...
            let timer = Instant::now();
            let child_node = graph
                .add_child_graph_node(upstream_nodes, &child_graph)
                .map_err(|error| {
                    anyhow::anyhow!(
                        "add recaptured cuBLASLt child at {:?} with upstream={upstream_nodes:?}: {error:?}",
                        op.node,
                    )
                })?;
            if let Some(profile) = profile.as_deref_mut() {
    ...
    fn build_graph(
        ...
        // Declared before `graph` so error unwinding destroys the parent
        // before child graphs and their resource owners. Existing captures
        // stay in state until the replacement executable is installed.
        let mut pending_host_captures: Vec<Option<PreparedHostCapture>> =
            (0..state.captured_host_ops.len()).map(|_| None).collect();
        ...
        let tracing_enabled = enabled!(Level::TRACE)
            ...
            && state.flashinfer_ops.is_empty();
        let step_profile = std::env::var_os("ORBITKV_CUDA_PROFILE_GRAPH_STEPS").is_some();
        ...
        for step_index in 0..n_steps {
            ...
            match step {
                CompiledStep::Kernel(idx) => {
                    ...
                    let cu_func = unsafe { kernel.function.raw_function() };
                    if std::env::var_os("ORBITKV_CUDA_DEBUG_GRAPH").is_some() {
                        eprintln!(
                    ...
                    let graph_node = unsafe {
                        graph
                            ...
                            .map_err(|error| {
                                anyhow::anyhow!(
                                    "add kernel {} at {:?}, launch={launch:?}: {error:?}",
                                    kernel.kernel_name,
                                    kernel.node,
                                )
                            })?
                ...
                CompiledStep::CuBlasLt(idx) => {
                    ...
                    let (prepared, _) = {
                        ...
                        .map_err(|error| {
                            anyhow::anyhow!(
                                "prepare cuBLASLt step {step_index} at {:?}: {error:#}",
                                op.node,
                            )
                        })?
                    };
                    ...
                    let child_node = Self::add_cublaslt_child_node(
                        &mut graph,
                        &deps,
                        &child_graph,
                        None,
                    ...
                    .map_err(|error| {
                        anyhow::anyhow!(
                            "add cuBLASLt step {step_index} with dependencies={deps:?}: {error:#}"
                        )
                    })?;
                ...
                CompiledStep::FlashInferDecode(idx) => {
                    let entry_node = graph.add_empty_node(&deps).map_err(|error| {
                        anyhow::anyhow!(
                            "add FlashInfer entry for step {step_index} with dependencies={deps:?}: {error:?}"
                        )
                    })?;
                    ...
                    .map_err(|error| {
                        anyhow::anyhow!(
                            "prepare FlashInfer step {step_index} at {:?}: {error:#}",
                            state.flashinfer_ops[idx].node,
                        )
                    })?;
                    let capture_stream = self.capture_stream()?;
                ...
                CompiledStep::CapturedHost(idx) => {
                    let capture_stream = self.capture_stream()?;
                    {
                        ...
                        op.prepare_graph_capture(&capture_stream, buffers, dyn_map)
                            .map_err(|error| {
                                anyhow::anyhow!(
                                    "prepare captured HostOp {:?} at {:?}: {error:#}",
                                    op.host_op,
                                    op.node,
                                )
                            })?;
                    ...
                    CudaGraphHandle::begin_standalone_capture(&capture_stream).map_err(
                        |error| {
                            anyhow::anyhow!(
                                "begin captured HostOp step {step_index} {:?}: {error:?}",
                    ...
                    let enqueue_result = {
                        ...
                        op.host_op
                            ...
                            .map_err(|error| {
                                anyhow::anyhow!(
                                    "enqueue captured HostOp {:?} at {:?}: {error:#}",
                                    op.host_op,
                                    op.node,
                                )
                            })
                    };
                    // Always end capture, even when the provider enqueue
                    // failed, before releasing the prepared resource owners.
                    let capture_result = CudaGraphHandle::end_standalone_capture(&capture_stream)
                        .map_err(|error| {
                            anyhow::anyhow!(
                                "end captured HostOp step {step_index} {:?}: {error:?}",
                                state.captured_host_ops[idx].host_op,
                            )
                        });
                    enqueue_result?;
                    ...
                    let child_node = graph
                        .add_child_graph_node(&deps, &child_graph)
                        .map_err(|error| {
                            anyhow::anyhow!(
                                "add captured HostOp step {step_index} {:?} with dependencies={deps:?}: {error:?}",
                                state.captured_host_ops[idx].host_op,
                            )
                        })?;
                    let op = &mut state.captured_host_ops[idx];
                    op.graph_node = Some(child_node);
                    pending_host_captures[idx] = Some(PreparedHostCapture {
                        graph: child_graph,
        ...
        let exec = if let Some(mut exec) = old_exec {
            ...
            graph.instantiate().map_err(|error| {
                anyhow::anyhow!(
                    "instantiate new CUDA graph with {} steps for dyn={dyn_map:?}: {error:?}",
                    state.steps.len(),
                )
            })?
        };
        ...
        for (op, pending) in state
            .captured_host_ops
            .iter_mut()
            .zip(pending_host_captures)
        {
            let pending = pending.expect("every captured HostOp was prepared");
            op.child_graph = Some(pending.graph);
            op.capture_resources = pending.resources;
        }
        state.cublaslt_prepare_cache = prepared_cache_plan;
        state.cublaslt_workspace_pool = workspace_pool_plan;
...
impl Drop for CudaGraphOp {
    fn drop(&mut self) {
        // Captured allocations may be owned by another stream. Finish their
        // actual executions before the graph/owner drop order below.
...
pub(crate) fn prepare_kernel_to_host_plan_with_topo_and_source_cache(
    ...
    let profile = std::env::var_os("ORBITKV_CUDA_PROFILE_PLAN").is_some();
        ...
        for unit in prepared_plan.fusion.compile_units_for(subgraph) {
            match unit {
                ...
                CompileUnit::Region(region) => {
                    ...
                    kernels[kernel_idx]
                        .source_nodes
                        .extend(region.elementwise_topo.iter().copied());
                    kernels[kernel_idx]
                        .source_nodes
                        .extend(region.fs_nodes.iter().copied());
...
#[cfg(test)]
#[path = "../../tests/unit/kernel/to_host/mod.rs"]
mod tests;
