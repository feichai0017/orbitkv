    ...
    for (int token = begin; token < end; ++token) {
        ...
        for (long long width = threadIdx.x; width < @KEY_WIDTH@; width += blockDim.x) {
            ...
            if constexpr (round_normalized_qk_to_bf16) {
                query_sum += __bfloat162float(__float2bfloat16_rn(q * q));
                key_sum += __bfloat162float(__float2bfloat16_rn(k * k));
            } else {
        ...
        const float query_inverse = round_normalized_qk_to_bf16
            ? __bfloat162float(__float2bfloat16_rn(rsqrtf(__bfloat162float(
                __float2bfloat16_rn(__bfloat162float(__float2bfloat16_rn(reduction[0]))
                    + @NORMALIZATION_EPSILON@)))))
            : rsqrtf(reduction[0] + @NORMALIZATION_EPSILON@) * query_scale;
        __syncthreads();
        ...
        const float key_inverse = round_normalized_qk_to_bf16
            ? __bfloat162float(__float2bfloat16_rn(rsqrtf(__bfloat162float(
                __float2bfloat16_rn(__bfloat162float(__float2bfloat16_rn(reduction[0]))
                    + @NORMALIZATION_EPSILON@)))))
            : rsqrtf(reduction[0] + @NORMALIZATION_EPSILON@);
        for (long long width = threadIdx.x; width < @KEY_WIDTH@; width += blockDim.x) {
            if constexpr (round_normalized_qk_to_bf16) {
        ...
        for (long long value_index = threadIdx.x; value_index < @VALUE_WIDTH@;
            ...
            for (long long width = 0; width < @KEY_WIDTH@; ++width) {
                ...
                memory = round_normalized_qk_to_bf16
                    ? __fadd_rn(memory, __fmul_rn(decayed, normalized_key[width]))
                    : fmaf(decayed, normalized_key[width], memory);
            }
            const float delta = round_normalized_qk_to_bf16
                ? __fmul_rn(__fsub_rn(value[value_base + value_index], memory), beta)
                : (value[value_base + value_index] - memory) * beta;
            float token_output = 0.0f;
            for (long long width = 0; width < @KEY_WIDTH@; ++width) {
                const long long state_index = state_base + width * @VALUE_WIDTH@ + value_index;
                const float next = round_normalized_qk_to_bf16
                    ? __fadd_rn(__fmul_rn(normalized_key[width], delta), output[state_index])
                    : fmaf(normalized_key[width], delta, output[state_index]);
                output[state_index] = next;
                token_output = round_normalized_qk_to_bf16
                    ? __fadd_rn(token_output, __fmul_rn(next, normalized_query[width]))
                    : fmaf(next, normalized_query[width], token_output);
            }
            output[value_base + value_index] = token_output;
        }
        __syncthreads();
    }
    if constexpr (@ROUND_FINAL_STATE_TO_BF16@) {
        for (long long index = threadIdx.x; index < value_head_width; index += blockDim.x) {
            output[state_base + index] = __bfloat162float(
                __float2bfloat16_rn(output[state_base + index]));
        }
    }
}
