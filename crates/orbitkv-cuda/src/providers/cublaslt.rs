mod handle;
pub(crate) use handle::try_create_cublaslt;
...
// LLIR fingerprints use Debug as the semantic representation of dialect ops.
// Cached handles include mutable stream counts, pointers and prepared shapes;
// none of those may change a program's identity after profiling or capture.
impl std::fmt::Debug for CuBlasLt {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        // Exhaustive destructuring makes every newly added field an explicit
        // choice between operation semantics and runtime-only state.
        let Self {
            m,
            n,
            k,
            ...
            b_scale_input,
            cublaslt: _,
            resource_prepare_cache: _,
        } = self;
        f.debug_struct("CuBlasLt")
            .field("m", m)
            .field("n", n)
            .field("k", k)
            .field("a_layout", a_layout)
            .field("b_layout", b_layout)
            .field("a_order", a_order)
            .field("b_order", b_order)
            .field("c_order", c_order)
            .field("d_order", d_order)
            .field("lda", lda)
            .field("ldb", ldb)
            .field("ldc", ldc)
            .field("ldd", ldd)
            .field("batch_count", batch_count)
            .field("stride_a", stride_a)
            .field("stride_b", stride_b)
            .field("stride_c", stride_c)
            .field("stride_d", stride_d)
            .field("a_dtype", a_dtype)
            .field("b_dtype", b_dtype)
            .field("c_dtype", c_dtype)
            .field("d_dtype", d_dtype)
            .field("compute_type", compute_type)
            .field("scale_dtype", scale_dtype)
            .field("alpha", alpha)
            .field("beta", beta)
            .field("epilogue", epilogue)
            .field("a_scale_input", a_scale_input)
            .field("b_scale_input", b_scale_input)
            .finish()
    }
}
...
impl EgglogOp for CuBlasLt {
    ...
    fn rewrites(&self) -> Vec<Rule> {
        vec![
            Rule::raw(include_str!("cublaslt/cublaslt_matmul_rewrite.egg")),
            Rule::raw(include_str!["cublaslt/cublaslt_RmRm_rewrite.egg"]), // row row
            Rule::raw(include_str!["cublaslt/cublaslt_RmCm_rewrite.egg"]), // row col
            Rule::raw(include_str!["cublaslt/cublaslt_CmRm_rewrite.egg"]), // col row
            Rule::raw(include_str!["cublaslt/cublaslt_CmCm_rewrite.egg"]), // col col
            Rule::raw(include_str!["cublaslt/cublaslt_fp8_rewrite.egg"]),
            Rule::raw(include_str!["cublaslt/cublaslt_output_witness.egg"]),
            Rule::raw(include_str!["cublaslt/cublaslt_scale_rewrite.egg"]),
            Rule::raw(include_str!["cublaslt/cublaslt_beta_rewrite.egg"]),
            Rule::raw(include_str!["cublaslt/cublaslt_epilogue_rewrite.egg"]),
            Rule::raw(include_str!["cublaslt/cublaslt_row_order_rewrite.egg"]),
        ]
    }
...
fn autotune_select(
    ...
    const REPS: usize = 16;
    let log = std::env::var_os("ORBITKV_CUBLASLT_AUTOTUNE_LOG").is_some_and(|v| v == "1");
...
#[cfg(test)]
#[path = "../../tests/unit/providers/cublaslt/mod.rs"]
mod tests;
