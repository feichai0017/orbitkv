...
impl EgglogOp for FlashInferAttention {
    ...
    fn rewrites(&self) -> Vec<Rule> {
        if !CAPABILITIES.supports_target(crate::device_compute_major()) {
            return vec![];
        }
    ...
    fn extract<'a>(
        ...
        crate::host::provider_source::validate_provider_identity(
            &recorded_provider,
            &current_provider,
        )
...
impl HostOp for FlashInferAttention {
    fn prepare_compilation(
        ...
        let group_size = Self::group_size(self.num_qo_heads, self.num_kv_heads)?;
        let _ = jit::ensure_compiled(self.head_dim, self.window_left >= 0, group_size);
        Ok(())
    }
...
#[cfg(test)]
#[path = "../../tests/unit/host/flashinfer/mod.rs"]
mod tests;
