...
impl<const PREQUANTIZED: bool> EgglogOp for DeepGemmImpl<PREQUANTIZED> {
    ...
    fn egglog_declarations(&self) -> Vec<String> {
        vec![BLOCK_SCALED_LINEAR_DECLARATIONS.to_owned()]
    }
    ...
    fn extract<'a>(
        ...
        crate::host::provider_source::validate_provider_identity(&provider, &current_provider)
            ...
            .map(|variant| {
                Rule::raw(format!(
                    "(rule
                        ...
                        (
                            {prepare}
                            (let ?candidate (Op ({name} ?m ?n ?k {variant} \"{provider}\")
                                (ICons {activation} (ICons ?w (ICons ?ws (INil))))))
                            (union ?reference ?candidate)
                            (set (dtype ?candidate) (Bf16))
                        )
                        :ruleset matmul_backend
                        :name \"{name} sm90 1d2d block-scaled variant {variant}\"
                    )",
                ))
            })
            .collect()
...
impl<const PREQUANTIZED: bool> HostOp for DeepGemmImpl<PREQUANTIZED> {
    fn prepare_compilation(
        ...
        let _ = jit::ensure_compiled(config)?;
        Ok(())
    }
    ...
    fn execute(
        ...
        let config = jit::Config::for_variant(stream, m, n, k, self.variant)?;
        let library = jit::ensure_compiled(config)?;
...
#[cfg(test)]
#[path = "../../tests/unit/host/deepgemm/mod.rs"]
mod tests;
