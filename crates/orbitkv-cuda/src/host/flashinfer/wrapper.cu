// FlashInfer batch decode + prefill wrapper for orbitkv_cuda.
// JIT-compiled at runtime with -DORBITKV_HEAD_DIM=N.
//
...
#ifndef ORBITKV_HEAD_DIM
#error "ORBITKV_HEAD_DIM must be defined (e.g. -DORBITKV_HEAD_DIM=128)"
#endif
#ifndef ORBITKV_GQA_GROUP_SIZE
#error "ORBITKV_GQA_GROUP_SIZE must be defined"
#endif
...
#define DISPATCH_HEAD_DIM(head_dim, HEAD_DIM, ...)  \
  {                                                  \
    constexpr size_t HEAD_DIM = ORBITKV_HEAD_DIM;    \
    __VA_ARGS__                                      \
  }
...
#define DISPATCH_GQA_GROUP_SIZE(group_size, GROUP_SIZE, ...)                \
  {                                                                         \
    if ((group_size) != ORBITKV_GQA_GROUP_SIZE) {                           \
      std::ostringstream err_msg;                                           \
      err_msg << "JIT group_size mismatch: " << (group_size) << " != "    \
              << ORBITKV_GQA_GROUP_SIZE;                                    \
      FLASHINFER_ERROR(err_msg.str());                                      \
    }                                                                       \
    constexpr size_t GROUP_SIZE = ORBITKV_GQA_GROUP_SIZE;                  \
    __VA_ARGS__                                                             \
  }
...
constexpr uint32_t HEAD_DIM = ORBITKV_HEAD_DIM;
constexpr uint32_t GQA_GROUP_SIZE = ORBITKV_GQA_GROUP_SIZE;
...
// .so when -DORBITKV_USE_SWA=1.
#ifndef ORBITKV_USE_SWA
#define ORBITKV_USE_SWA 0
#endif
constexpr bool USE_SWA = ORBITKV_USE_SWA != 0;

// dtype codes shared with the Rust side (jit.rs / mod.rs).
constexpr int ORBITKV_DTYPE_F32 = 0;
constexpr int ORBITKV_DTYPE_F16 = 1;
constexpr int ORBITKV_DTYPE_BF16 = 2;
constexpr int ORBITKV_EXCEPTION_ERROR = -1000;

thread_local char orbitkv_last_error[1024] = {};

static void orbitkv_set_last_error(const char* message) noexcept {
    const char* source = message ? message : "unknown C++ exception";
    std::strncpy(orbitkv_last_error, source, sizeof(orbitkv_last_error) - 1);
    orbitkv_last_error[sizeof(orbitkv_last_error) - 1] = '\0';
}

template <typename F>
static int orbitkv_ffi_guard(F&& operation) noexcept {
    orbitkv_last_error[0] = '\0';
    try {
        return operation();
    } catch (const std::exception& error) {
        orbitkv_set_last_error(error.what());
        return ORBITKV_EXCEPTION_ERROR;
    } catch (...) {
        orbitkv_set_last_error(nullptr);
        return ORBITKV_EXCEPTION_ERROR;
    }
}
...
#define ORBITKV_INSTANTIATE_PREFILL(T, CTA_TILE_Q)                                          \
...
ORBITKV_INSTANTIATE_PREFILL(half, 16)
ORBITKV_INSTANTIATE_PREFILL(half, 64)
ORBITKV_INSTANTIATE_PREFILL(half, 128)
ORBITKV_INSTANTIATE_PREFILL(__nv_bfloat16, 16)
ORBITKV_INSTANTIATE_PREFILL(__nv_bfloat16, 64)
ORBITKV_INSTANTIATE_PREFILL(__nv_bfloat16, 128)

#undef ORBITKV_INSTANTIATE_PREFILL
    ...
    int64_t* plan_info_out, int* plan_info_len_out)
{
    ...
    return orbitkv_ffi_guard([&]() -> int {
      switch (dtype) {
        case ORBITKV_DTYPE_F32:
#if ORBITKV_HEAD_DIM <= 256
...
#endif
        case ORBITKV_DTYPE_F16:
            return batch_decode_plan_t<half>(
                float_workspace, float_ws_size, int_workspace, int_ws_size,
                page_locked_int_workspace, indptr_h, batch_size, num_qo_heads,
                num_kv_heads, page_size, enable_cuda_graph, stream,
                plan_info_out, plan_info_len_out);
        case ORBITKV_DTYPE_BF16:
    ...
    int64_t* plan_info_out, int* plan_info_len_out)
{
    ...
    return orbitkv_ffi_guard([&]() -> int {
      if (dtype != ORBITKV_DTYPE_F16 && dtype != ORBITKV_DTYPE_BF16) {
...
// FlashInfer writes output as (batch, heads, dim) but OrbitKV expects (heads, batch, dim).
