...
use orbitkv_compiler::dyn_backend::{BackendCompileArgs, DynBackend, compile_backend};
...
pub type DefaultCudaDynBackend = CudaDynBackend<DefaultCudaOps>;

pub fn cuda_factory(
    graph: &mut Graph,
    args: BackendCompileArgs,
) -> Result<Box<dyn DynBackend>, String> {
    cuda_factory_for::<DefaultCudaOps>(graph, args, "orbitkv-cuda")
}
