...
use cudarc::driver::{
    CudaEvent, CudaFunction, CudaModule, CudaSlice, CudaStream, DevicePtr, DriverError, result, sys,
};

use half::{bf16, f16};
use itertools::Itertools;
use orbitkv_compiler::hlir::*;
...
use orbitkv_tracing::PerfettoGuard;
use orbitkv_tracing::prost::Message;
use std::{
...
pub(crate) struct CompiledBucket {
    /// Computed from the original LLIR only when stage diagnostics are enabled.
...
fn bucket_output_aliases_input(
    bucket: &CompiledBucket,
    output: NodeIndex,
    input: NodeIndex,
) -> Option<bool> {
    let mut data_node = *bucket.output_producers.get(&output)?;
    while let Some(&alias) = bucket.output_alias_map.get(&data_node) {
        data_node = alias;
    }
    Some(bucket.llir_to_hlir.get(&data_node) == Some(&input))
}
...
fn validate_required_state_aliases(
    ...
    for &(output, input) in required {
        anyhow::ensure!(
            bucket_output_aliases_input(bucket, output, input) == Some(true),
...
pub struct CudaRuntimeImpl<O> {
    ...
    pub(crate) module_artifact: Option<Arc<Mutex<ModuleArtifact>>>,
    pub(crate) selected_schedule: Option<orbitkv_compiler::graph::SelectedSchedule>,
    ...
    profile_single_row_indptrs: FxHashMap<NodeIndex, Symbol>,
    profile_input_generator: Option<Arc<ProfileInputGenerator>>,
    /// Persistent state edges that every selected bucket must implement as a
    /// true output-to-input alias. Unlike ordinary output registrations these
    /// contracts reject materializing candidates instead of adding copy-back.
    required_state_aliases: Vec<(NodeIndex, NodeIndex)>,
    ...
    last_hotspot_profile: Vec<orbitkv_compiler::search::ProfiledRegion>,
    ...
    /// Caller-shared device allocations retained by the runtime for external
    /// persistent-state bindings. Unlike `external_buffers`, these entries
    /// keep the allocation alive through every compiled bucket and capture.
    shared_external_buffers: FxHashMap<NodeIndex, Arc<CudaSlice<u8>>>,
    /// Required output paired with each caller-shared persistent input. Used
    /// to remove the durable output registration when that input is rebound.
    shared_external_outputs: FxHashMap<NodeIndex, NodeIndex>,
    /// Unforgeable per-binding identities shared with execution receipts.
    shared_external_identities: FxHashMap<NodeIndex, Arc<()>>,
...
///
/// The allocation and stream must match, both ranges must be non-empty, equal
/// in length, in bounds, and non-overlapping. Keeping the allocation in an
/// `Arc` lets callers retain it until a later stream event proves completion.
pub fn copy_shared_device_range(
    stream: &Arc<CudaStream>,
    allocation: &Arc<CudaSlice<u8>>,
    source: std::ops::Range<usize>,
    destination: std::ops::Range<usize>,
) -> Result<(), DriverError> {
    if allocation.stream().cu_stream() != stream.cu_stream()
        || source.start >= source.end
        || destination.start >= destination.end
        || source.len() != destination.len()
        || source.end > allocation.len()
        || destination.end > allocation.len()
        || !(source.end <= destination.start || destination.end <= source.start)
    {
        return Err(DriverError(sys::CUresult::CUDA_ERROR_INVALID_VALUE));
    }
    let base = allocation.device_ptr(stream).0;
    let source_ptr = base + source.start as u64;
    let destination_ptr = base + destination.start as u64;
    unsafe {
...
/// Enqueues a checked zero fill within one shared CUDA allocation.
pub fn zero_shared_device_range(
    stream: &Arc<CudaStream>,
    allocation: &Arc<CudaSlice<u8>>,
...
pub struct CapturedCudaExecution {
    ...
    stream: Arc<CudaStream>,
    shared_states: Box<[CudaSharedStateBinding]>,
}

/// Compile-time storage policy for one caller-owned persistent state edge.
#[derive(Clone, Copy, Debug, Eq, PartialEq)]
pub enum CudaSharedStatePolicy {
    /// Every retained schedule must update the input allocation directly.
    RequiredInPlace,
    /// A schedule may materialize the output; the runtime copies it back to
    /// the stable input allocation on the same stream before recording completion.
    CopyBackAllowed,
}

/// Opaque identity of one shared required-state alias installed in a runtime.
#[derive(Clone)]
pub struct CudaSharedStateBinding {
    input: NodeIndex,
    output: NodeIndex,
    allocation: Arc<CudaSlice<u8>>,
    bytes: usize,
...
/// Opaque proof that a OrbitKV CUDA execution has been enqueued and followed
/// by an event on the same stream.
pub struct CudaExecutionReceipt {
    event: CudaEvent,
    stream: Arc<CudaStream>,
    shared_states: Box<[CudaSharedStateBinding]>,
}

impl CudaExecutionReceipt {
    fn record(
        stream: &Arc<CudaStream>,
        shared_states: Box<[CudaSharedStateBinding]>,
    ) -> Result<Self, DriverError> {
        let event = stream.record_event(Some(sys::CUevent_flags::CU_EVENT_DISABLE_TIMING))?;
        Ok(Self {
            event,
    ...
    /// Waits for completion after verifying the execution stream and every
    /// expected shared required-state alias.
    ///
    /// # Errors
    ///
    /// Rejects a receipt from another stream/runtime state binding or
    /// propagates CUDA event errors.
    pub fn synchronize_required_states<'a>(
        self,
        expected_stream: &CudaStream,
        expected_states: impl IntoIterator<Item = &'a CudaSharedStateBinding>,
    ) -> Result<(), DriverError> {
        if self.stream.cu_stream() != expected_stream.cu_stream()
            || !Arc::ptr_eq(self.stream.context(), expected_stream.context())
        {
            return Err(DriverError(sys::CUresult::CUDA_ERROR_INVALID_CONTEXT));
        }
        let expected_states = expected_states.into_iter().collect::<Vec<_>>();
        if !same_shared_state_bindings(&self.shared_states, &expected_states) {
            return Err(DriverError(sys::CUresult::CUDA_ERROR_INVALID_VALUE));
        }
...
fn same_shared_state_bindings(
    observed: &[CudaSharedStateBinding],
    expected: &[&CudaSharedStateBinding],
) -> bool {
    !expected.is_empty()
        && observed.len() == expected.len()
        && observed
            .iter()
            .all(|item| expected.iter().any(|other| item.same_identity(other)))
        && expected
            .iter()
            .all(|item| observed.iter().any(|other| other.same_identity(item)))
}

impl CudaSharedStateBinding {
    fn same_identity(&self, other: &Self) -> bool {
        self.input == other.input
            && self.output == other.output
            && self.bytes == other.bytes
            && Arc::ptr_eq(&self.allocation, &other.allocation)
            && self.stream.cu_stream() == other.stream.cu_stream()
            && Arc::ptr_eq(self.stream.context(), other.stream.context())
...
impl CapturedCudaExecution {
    ...
    /// Launches the captured graph and records a completion event on its
    /// original stream.
    pub fn launch_recorded(&self) -> Result<CudaExecutionReceipt, DriverError> {
        self.launch()?;
        CudaExecutionReceipt::record(&self.stream, self.shared_states.clone())
    }

    /// Verifies required-state identities before launching and recording.
    pub fn launch_recorded_for<'a>(
        &self,
        expected_states: impl IntoIterator<Item = &'a CudaSharedStateBinding>,
    ) -> Result<CudaExecutionReceipt, DriverError> {
        let expected = expected_states.into_iter().collect::<Vec<_>>();
        if !same_shared_state_bindings(&self.shared_states, &expected) {
            return Err(DriverError(sys::CUresult::CUDA_ERROR_INVALID_VALUE));
        }
        self.launch_recorded()
    }
}

impl<O: IntoEgglogOp> CudaRuntimeImpl<O> {
    /// Keep a two-element CSR indptr consistent with one dynamic token
    /// dimension while search profiles different buckets. Real execution data
    /// installed by the caller is untouched outside the profiling path.
    pub fn register_profile_single_row_indptr(
        &mut self,
        input: impl ToId,
        tokens: impl Into<Symbol>,
    ) {
        self.profile_single_row_indptrs
            .insert(input.to_id(), tokens.into());
    }

    /// Register small synthetic integer inputs for representative search shapes.
    /// Called before candidate preparation and graph materialization; ordinary
    /// execution keeps caller-provided inputs unchanged. The generator must be
    /// deterministic and return data within the caller-reserved input capacities.
    pub fn register_profile_input_generator(
        &mut self,
        generator: impl Fn(&DynMap) -> Vec<(NodeIndex, Vec<i32>)> + Send + Sync + 'static,
    ) {
        self.profile_input_generator = Some(Arc::new(generator));
    }

    fn apply_profile_input_overrides(&mut self, dyn_map: &DynMap) {
        if let Some(generator) = &self.profile_input_generator {
            for (input, values) in generator(dyn_map) {
                self.set_data(input, values);
            }
        }
        let values = self
            .profile_single_row_indptrs
            .iter()
            .map(|(&input, &tokens)| {
                let tokens = dyn_map[&tokens];
                (
                    input,
                    i32::try_from(tokens).expect("profile token dimension exceeds i32"),
                )
            })
            .collect_vec();
        for (input, tokens) in values {
            self.set_data(input, vec![0_i32, tokens]);
        }
    }
    ...
    /// Executes the selected bucket and records an event after all enqueued
    /// work on the runtime stream.
    pub fn execute_recorded(
        &mut self,
        dyn_map: &DynMap,
    ) -> Result<CudaExecutionReceipt, DriverError> {
        <Self as Runtime>::execute(self, dyn_map);
        CudaExecutionReceipt::record(&self.cuda_stream, self.shared_state_bindings())
    }

    /// Verifies required-state identities before executing and recording.
    pub fn execute_recorded_for<'a>(
        &mut self,
        dyn_map: &DynMap,
        expected_states: impl IntoIterator<Item = &'a CudaSharedStateBinding>,
    ) -> Result<CudaExecutionReceipt, DriverError> {
        let observed = self.shared_state_bindings();
        let expected = expected_states.into_iter().collect::<Vec<_>>();
        if !same_shared_state_bindings(&observed, &expected) {
            return Err(DriverError(sys::CUresult::CUDA_ERROR_INVALID_VALUE));
        }
        <Self as Runtime>::execute(self, dyn_map);
        CudaExecutionReceipt::record(&self.cuda_stream, observed)
    }
    ...
    pub fn capture_execution(
        ...
        Ok(CapturedCudaExecution {
            executable,
            stream,
            shared_states: self.shared_state_bindings(),
        })
    }
    ...
    pub fn prepare_captured_execution(
        ...
        let _modules = ModuleArtifactGuard::enter(self.module_artifact.clone());
    ...
    pub(crate) fn module_artifact_context(&self) -> &Arc<cudarc::driver::CudaContext> {
        self.cuda_stream.context()
    }
    ...
    fn set_data_bytes(&mut self, id: NodeIndex, bytes: Vec<u8>, keep_host_mirror: bool) {
            ...
            && bytes.len() <= buf.len()
        {
            ...
            *len = bytes.len();
            ...
            self.remove_shared_external_binding(id);
            if keep_host_mirror {
    ...
    /// Register one caller-shared allocation as both a persistent input and a
    /// persistent output. The runtime retains an `Arc`, so compiled buckets
    /// and CUDA graphs cannot outlive the backing allocation.
    ///
    /// # Errors
    ///
    /// Rejects an empty or out-of-bounds range, a foreign CUDA context, or a
    /// stream that differs from the runtime execution stream.
    pub fn bind_shared_state(
        &mut self,
        input: impl ToId,
        output: impl ToId,
        allocation: Arc<CudaSlice<u8>>,
        bytes: usize,
        policy: CudaSharedStatePolicy,
    ) -> Result<CudaSharedStateBinding, DriverError> {
        let input = input.to_id();
        let output = output.to_id();
        if bytes == 0
            || bytes > allocation.len()
            || !Arc::ptr_eq(allocation.context(), self.cuda_stream.context())
            || allocation.stream().cu_stream() != self.cuda_stream.cu_stream()
        {
            return Err(DriverError(sys::CUresult::CUDA_ERROR_INVALID_VALUE));
        }
        self.remove_shared_external_binding(input);
        let pointer = allocation.device_ptr(&self.cuda_stream).0;
        unsafe {
            self.set_device_ptr(input, pointer, bytes);
            self.set_output_device_ptr(output, pointer, bytes);
        }
        if policy == CudaSharedStatePolicy::RequiredInPlace {
            self.required_state_aliases.push((output, input));
        }
        let binding = CudaSharedStateBinding {
            input,
            output,
        ...
        self.shared_external_buffers.insert(input, allocation);
        self.shared_external_outputs.insert(input, output);
        self.shared_external_identities
            .insert(input, Arc::clone(&binding.identity));
        Ok(binding)
    }

    /// Registers caller-owned state and requires direct in-place mutation in
    /// every selected bucket.
    pub fn alias_shared_state_required(
        &mut self,
        input: impl ToId,
        output: impl ToId,
        allocation: Arc<CudaSlice<u8>>,
    ...
    fn shared_state_bindings(&self) -> Box<[CudaSharedStateBinding]> {
        let mut inputs = self
            .shared_external_buffers
            .keys()
            .copied()
            .collect::<Vec<_>>();
        ...
        inputs
            .into_iter()
            .filter_map(|input| {
                let allocation = self.shared_external_buffers.get(&input)?;
                let &output = self.shared_external_outputs.get(&input)?;
                let identity = self.shared_external_identities.get(&input)?;
                let (_, bytes) = self.input_allocation(input)?;
                Some(CudaSharedStateBinding {
                    input,
                    output,
                    allocation: Arc::clone(allocation),
                    bytes,
    ...
    fn remove_shared_external_binding(&mut self, input: NodeIndex) {
        self.required_state_aliases
            .retain(|&(_, existing_input)| existing_input != input);
        self.shared_external_identities.remove(&input);
        if self.shared_external_buffers.remove(&input).is_some()
            && let Some(output) = self.shared_external_outputs.remove(&input)
            && self.output_ptr_registrations.remove(&output).is_some()
        {
            self.dirty_output_ptr_registrations.insert(output);
        }
    }
    ...
    /// Allocate persistent state and require every selected bucket to mutate
    /// it in place. Candidate search and stored-schedule loading both fail
    /// closed when the output does not resolve to the input.
    pub fn alias_state_required(
        &mut self,
        input: impl ToId,
        output: impl ToId,
        bytes: usize,
        ...
        let input = input.to_id();
        let output = output.to_id();
        let allocation = self.alias_state(input, output, bytes);
        // Binding an input clears its old alias contract.
        self.required_state_aliases.push((output, input));
        allocation
    }
    ...
    pub fn output_aliases_input_in_all_buckets(&self, output: impl ToId, input: impl ToId) -> bool {
        ...
        !self.compiled_buckets.is_empty()
            && (0..self.compiled_buckets.len()).all(|bucket| {
                self.output_aliases_input_in_bucket(output, input, bucket) == Some(true)
            })
    }

    /// Whether one retained dynamic-shape bucket resolves an output directly
    /// into the named input allocation.
    ///
    /// Returns `None` for an unknown bucket or output, allowing callers to
    /// distinguish missing compiler metadata from a materializing schedule.
    pub fn output_aliases_input_in_bucket(
        &self,
        output: impl ToId,
        input: impl ToId,
        bucket_index: usize,
        ...
        let output = output.to_id();
        let input = input.to_id();
        bucket_output_aliases_input(self.compiled_buckets.get(bucket_index)?, output, input)
    }
    ...
    fn initialize_fixed_intermediate_buffer_plan(bucket: &mut CompiledBucket, dyn_dims: &DynMap) {
        let profile = std::env::var_os("ORBITKV_CUDA_ARENA_PROFILE").is_some();
    ...
    fn plan_intermediate_buffers(bucket: &mut CompiledBucket, dyn_dims: &DynMap) {
        ...
        if bucket.preserve_intermediate_buffers_for_debug {
            ...
            if std::env::var_os("ORBITKV_CUDA_MEMORY_DEBUG").is_some() {
                eprintln!(
    ...
    fn prepare_bucket_buffers(&mut self, bucket_idx: usize, dyn_map: &DynMap) {
        let profile_prepare = std::env::var_os("ORBITKV_CUDA_PROFILE_EXEC").is_some()
            || std::env::var_os("ORBITKV_CUDA_PROFILE_RECAPTURE").is_some();
        ...
        if let Some((node, previous, current)) = changed_resource_input
            && std::env::var_os("ORBITKV_CUDA_DEBUG_RESOURCE_INPUT_CHANGE").is_some()
        {
            eprintln!(
    ...
    /// was executing. Gated on ORBITKV_SEARCH_DUMP_LAST_LLIR.
    pub(crate) fn dump_candidate_llir_for_postmortem(llir_graph: &LLIRGraph, dyn_map: &DynMap) {
        if std::env::var_os("ORBITKV_SEARCH_DUMP_LAST_LLIR").is_none() {
            return;
        }
        let summary = llir_graph
            ...
            .join("\n");
        let _ = std::fs::write(
            "/tmp/orbitkv_search_last_candidate_llir.txt",
    ...
    fn materialize_bucket_cuda_graphs(
        ...
        let _modules = ModuleArtifactGuard::enter(self.module_artifact.clone());
        let _stage = tracing::info_span!(target: "orbitkv::stage", "cuda.graph.materialize", bucket = bucket_idx,
            fully_dirty = self.compiled_buckets[bucket_idx].materialization_fully_dirty).entered();
    ...
    fn candidate_allocation_dyn_map(
        dyn_map: &DynMap,
        ctx: &orbitkv_compiler::search::BucketContext<'_>,
    ) -> DynMap {
        if ctx.is_bucketed() {
...
impl<O: IntoEgglogOp> CudaRuntimeImpl<O> {
    ...
    pub(crate) fn cancel_search_profile(&mut self) {
        ...
        self.hotspot_profiling = false;
        self.last_hotspot_profile.clear();
    }

    /// Diagnostic pass after the ordinary fitness trials. Its events never
    /// enter the ranked metric or the installed serving CUDA graph.
    pub(crate) fn profile_loaded_hotspots(
        &mut self,
        dyn_map: &DynMap,
    ) -> Vec<orbitkv_compiler::search::ProfiledRegion> {
        self.profiling = true;
        self.profile_cuda_graphs = false;
        self.hotspot_profiling = true;
        self.last_hotspot_profile.clear();
        self.assume_dyn_dims_stale();
        self.execute(dyn_map);
        let profile = std::mem::take(&mut self.last_hotspot_profile);
    ...
    fn profile_loaded_llir_inner(
        ...
        self.apply_profile_input_overrides(dyn_map);
        self.profiling = true;
        ...
        for _ in 0..trials.max(1) {
            ...
            if early_stop.is_some_and(|(best, factor)| {
                ...
                orbitkv_compiler::op::early_stop_exceeded(mean, best, factor)
            }) {
                break;
            }
        }
        ...
        let display = if std::env::var_os("ORBITKV_SEARCH_OP_NAMES").is_some() {
    ...
    fn try_load_llir(&mut self, llir_graph: &LLIRGraph) -> anyhow::Result<()> {
        validate_static_llir_semantics(llir_graph, &self.required_state_aliases)
        ...
        validate_required_state_aliases(&self.required_state_aliases, &bucket)?;
    ...
    fn install_validated_bucket_set_inner(
        ...
        {
            self.apply_profile_input_overrides(representative_dyn_map);
    ...
    fn compile_and_validate_bucket_set_with_allocation_maps(
        ...
        for bucket in bucket_llirs {
            validate_static_llir_semantics(bucket.llir, &self.required_state_aliases)
    ...
    /// either `validate_static_llir_semantics` or `prepare_static_llir_resources`
    /// with this runtime's required state aliases for every supplied graph.
    fn compile_and_validate_prevalidated_bucket_set_with_allocation_maps(
        ...
        let _stage = tracing::info_span!(target: "orbitkv::stage", "cuda.buckets.compile_and_validate", buckets = bucket_llirs.len()).entered();
        anyhow::ensure!(
            bucket_llirs.len() == allocation_dyn_maps.len(),
            "every CUDA LLIR bucket must have an allocation dimension map"
        );
        ...
        let resource_stage =
            tracing::info_span!(target: "orbitkv::stage", "cuda.resources.retained").entered();
        let (hlir_buffer_lengths, input_lengths_complete) =
        ...
        drop(resource_stage);
        let providers_stage =
            tracing::info_span!(target: "orbitkv::stage", "cuda.providers.prepare").entered();
        for (bucket, representative_dyn_map) in
            compiled_buckets.iter().zip(&representative_dyn_maps)
        {
            for executable in bucket.exec_graph.node_weights() {
                executable
                    .internal
    ...
    fn compile_and_validate_profile_candidate_inner(
        ...
        if enforce_search_planning_limit && let Some(limit) = self.search_candidate_node_limit {
            ...
            if required > limit {
                ...
                orbitkv_compiler::mask_events::RESOURCE_REJECT
                    .record_with(|| violation.to_string());
        ...
        let static_plan =
            match tracing::info_span!(target: "orbitkv::stage", "cuda.resources.static_prepare")
                .in_scope(|| {
        ...
        if let Err(violation) = validate_resource_plan(
            ...
            orbitkv_compiler::mask_events::RESOURCE_REJECT.record_with(|| violation.to_string());
            return Err(format!("resource reject: {violation}"));
        }
        ...
        // aliases, required persistent arenas, and fusion-region contracts for
        // this exact LLIR. Reuse its
...
impl<O: IntoEgglogOp> Runtime for CudaRuntimeImpl<O> {
    ...
    fn late_egglog_passes(
        ops: &[Arc<Box<dyn orbitkv_compiler::op::EgglogOp>>],
        _options: &CompileOptions,
        _dyn_map: &DynMap,
    ) -> Vec<orbitkv_compiler::egglog_utils::LateEgglogPass> {
    ...
    fn initialize(stream: Self::CompileArg) -> Self {
        ...
        Self {
            _ops: PhantomData,
            module_artifact: current_module_artifact_session(),
            selected_schedule: None,
            hlir_buffers: FxHashMap::default(),
            hlir_host_mirrors: FxHashMap::default(),
            profile_single_row_indptrs: FxHashMap::default(),
            profile_input_generator: None,
            required_state_aliases: Vec::new(),
            ...
            shared_external_buffers: FxHashMap::default(),
            shared_external_outputs: FxHashMap::default(),
            shared_external_identities: FxHashMap::default(),
        }
    }

    fn compile(
        &mut self,
        space: &orbitkv_compiler::search::SearchSpace,
        dyn_map: &DynMap,
    ...
    fn execute(&mut self, dyn_map: &DynMap) -> Self::ExecReturn {
        ...
        let execution_id = self.next_execution_id;
        self.next_execution_id = self.next_execution_id.wrapping_add(1);
        let execution_stage = tracing::info_span!(target: "orbitkv::stage", "cuda.execute",
            execution = execution_id, profiling = self.profiling, external_graph = self.external_cuda_graph,
            previous_bucket = self.active_bucket, bucket = tracing::field::Empty,
            program = tracing::field::Empty);
        let _execution_entered = execution_stage.enter();
        ...
        let profile_runtime = std::env::var_os("ORBITKV_CUDA_PROFILE_EXEC").is_some()
            || std::env::var_os("ORBITKV_CUDA_PROFILE_RECAPTURE").is_some();
        ...
        let profile_ops_file = std::env::var_os("ORBITKV_CUDA_PROFILE_OPS_FILE");
        let profile_op_details = std::env::var_os("ORBITKV_CUDA_PROFILE_OP_DETAILS").is_some();
        let profile_ops = !self.profiling
            && (std::env::var_os("ORBITKV_CUDA_PROFILE_OPS").is_some()
                || profile_ops_file
                    .as_ref()
                    .is_some_and(|path| std::path::Path::new(path).exists()));
        let sync_each_exec_op = std::env::var_os("ORBITKV_CUDA_SYNC_EACH_EXEC_OP").is_some();
        ...
        execution_stage.record("bucket", self.active_bucket);
        if let Some(program) = &self.compiled_buckets[self.active_bucket].program_identity {
            execution_stage.record("program", program.as_str());
        }
        ...
        let timer = std::time::Instant::now();
        let buffers_stage =
            tracing::info_span!(target: "orbitkv::stage", "cuda.inputs.prepare").entered();
        ...
        // prepared resources while capturing instead of touching OrbitKV's
        // private CUDA graph.
        let timer = std::time::Instant::now();
        ...
        for &exec_node in &bucket.exec_order {
            ...
            let profile_host =
                self.hotspot_profiling && !exec_op.internal.as_any().is::<CudaGraphOp>();
            let op_timing = (profile_ops || profile_host).then(|| {
            ...
            if let Some(cuda_graph) = exec_op.internal.as_any().downcast_ref::<CudaGraphOp>() {
                ...
                let result = if self.profiling && !self.profile_cuda_graphs {
                    ...
                    cuda_graph.launch_steps(
                        &exec_op.stream,
                        &buffers,
                        dyn_map,
                        execution_id,
                        self.hotspot_profiling,
                    )
                    ...
                    cuda_graph.launch_steps(&exec_op.stream, &buffers, dyn_map, execution_id, false)
                } else {
            ...
            if !self.profiling
                && std::env::var_os("ORBITKV_CUDA_CHECK_NONFINITE_INTERNAL").is_some()
            {
        ...
        if self.hotspot_profiling {
            // Search may be driven by an embedded runtime whose normal calls
            // are asynchronous. Feedback must only read completed events.
            if !self.synchronize_stream {
                self.cuda_stream.synchronize().unwrap();
            }
            for &exec_node in &bucket.exec_order {
                ...
                if let Some(graph) = exec_op.internal.as_any().downcast_ref::<CudaGraphOp>() {
                    self.last_hotspot_profile.extend(
                        graph
                            .measured_regions()
                            .expect("failed to collect hotspot timings"),
                    );
                }
            }
            for (exec_index, _, _, start, end) in &op_events {
                let exec_op = &bucket.exec_graph[NodeIndex::new(*exec_index)];
                if exec_op.internal.as_any().is::<CudaGraphOp>() {
                    continue;
                }
                self.last_hotspot_profile
                    .push(orbitkv_compiler::search::ProfiledRegion {
                        nodes: vec![exec_op.output],
                        cost: f64::from(
                            start
                                .elapsed_ms(end)
                                .expect("failed to measure host region"),
                        ) / 1_000.0,
                    });
            }
        }
        if std::env::var_os("ORBITKV_CUDA_PROFILE_GRAPH_STEPS").is_some() {
            for &exec_node in &bucket.exec_order {
                let exec_op = &bucket.exec_graph[exec_node];
                if let Some(cuda_graph) = exec_op.internal.as_any().downcast_ref::<CudaGraphOp>() {
                    cuda_graph.print_step_profile(dyn_map, exec_node.index());
                }
            }
        }
...
impl<O: IntoEgglogOp> CudaRuntimeImpl<O> {
    ...
    #[cfg(test)]
    pub(crate) fn debug_materialized_bucket_indices(&self) -> Vec<usize> {
        self.materialized_bucket_indices()
    }
    ...
    fn compile_bucket_with_prepared(
        ...
        let _modules = ModuleArtifactGuard::enter(self.module_artifact.clone());
        let _stage = tracing::info_span!(target: "orbitkv::stage", "cuda.bucket.compile", prepared = prepared_kernel.is_some()).entered();
    ...
    fn compile_bucket_with_current_source_limit(
        ...
        if tracing::enabled!(target: "orbitkv::stage", tracing::Level::INFO) {
            bucket.program_identity =
                Some(orbitkv_compiler::graph::llir_program_identity(llir_graph));
        }
    ...
    pub fn record_cuda_perfetto_trace(&mut self, mut perfetto_guard: PerfettoGuard) {
        ...
        let mut trace = orbitkv_tracing::schema::Trace::decode(data.as_slice()).unwrap();
...
#[cfg(test)]
#[path = "../tests/unit/runtime/arena_plan/mod.rs"]
mod arena_plan_tests;

#[cfg(test)]
#[path = "../tests/unit/runtime/required_state_aliases.rs"]
