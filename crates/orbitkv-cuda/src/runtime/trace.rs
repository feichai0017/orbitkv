//! Attach measured CUDA Graph timings to a Perfetto trace.

use crate::kernel::CudaGraphTiming;
use orbitkv_tracing::schema::{
...
fn build_interned_strings(trace: &schema::Trace) -> std::collections::HashMap<(u32, u64), String> {
    use orbitkv_tracing::schema::trace_packet;
...
fn annotation_matches_id(
    ...
    match &a.value {
        Some(orbitkv_tracing::schema::debug_annotation::Value::StringValue(v)) => {
            *v == format!("{id}")
        }
        _ => false,
    }
...
pub(super) fn record_cuda_graph_timings(
    trace: &schema::Trace,
