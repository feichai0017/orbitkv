...
fn rms_norm_rule_fires_on_mini_layer() {
    // The KernelRMSNorm rewrite must offer fused candidates for the two 2-D
    // bf16 norm sandwiches. Per-head 3-D views remain decomposed unless
    // their dense base layout can be proven.
    ...
    // Two plain norms are expected in one layer.
    assert!(
        found >= 2,
        "expected >=2 KernelRMSNorm candidates, found {found}"
    );
}
...
fn rms_norm_dense_stride_guard_is_part_of_rewrite() {
    ...
    assert!(text.contains("(= ?sq_b ?sq_a)"));
    assert!(!text.contains("ECons ?d0"));
}
...
fn dump_rotary_post_egglog() {
    ...
    fn render(
        ...
        class: &orbitkv_compiler::egglog_utils::ClassId,
        depth: usize,
    ) -> String {
        if depth == 0 {
            return "…".to_string();
