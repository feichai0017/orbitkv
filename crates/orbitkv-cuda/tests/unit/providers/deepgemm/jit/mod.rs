...
fn upstream_heuristic_is_first_but_not_the_only_candidate() {
    ...
    assert!(choices.len() >= super::super::selection::SEARCH_VARIANTS);
    assert_eq!((choices[0].block_m, choices[0].block_n), (16, 80));
    assert!(
        choices[..super::super::selection::SEARCH_VARIANTS]
            .windows(2)
...
fn source_records_exact_provider_identity_and_tile() {
    ...
    assert!(source.contains(&ProviderId::DeepGemm.source().unwrap().revision));
