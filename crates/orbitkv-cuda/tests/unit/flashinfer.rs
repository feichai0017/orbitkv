...
//!    compared against a orbitkv-compiled reference attention graph.
//!
...
use orbitkv_compiler::egglog_utils::{
...
fn ops_contains_sort(name: &str) -> bool {
    let ops = <CudaRuntime as orbitkv_compiler::op::Runtime>::Ops::into_vec();
    ops.iter().any(|op| {
...
fn flashinfer_egg_rule_parses() {
    ...
    // Capability rules accompany the semantic and structural provider rules.
    let s = format!("{rewrites:?}");
    assert!(
...
fn test_indptr_to_request_idx(
    ...
    let ge = indptr_2d
        .le(indices)
        .cast(orbitkv_compiler::dtype::DType::Int);
    ge.sum(1).cast(orbitkv_compiler::dtype::DType::Int) - 1
}
...
fn test_compute_attn_mask(
    ...
    let allowed = same.cast(orbitkv_compiler::dtype::DType::F32)
        * causal.cast(orbitkv_compiler::dtype::DType::F32);
    allowed * 1e10 - 1e10
}
...
fn test_compute_triu_gather_mask(
    ...
    let s = q_pos.dims1();
    let causal_square = graph.triu(c, 1).cast(orbitkv_compiler::dtype::DType::F32) * -1e10;
...
fn test_compute_direct_causal_mask(
    ...
    q_pos.lt(col).cast(orbitkv_compiler::dtype::DType::F32) * -1e10
}
...
fn cache_dest_with_provenance(
    ...
    match provenance {
        ...
        TestCacheProvenance::LoopInput { loop_id, stream_id } => {
            ...
            let id = graph.add_op(
                orbitkv_compiler::hlir::LoopInput {
                    loop_id,
                    stream_id,
                    dtype: cache.dtype,
                },
...
fn build_paged_attention_graph_with_token_dim(
    ...
    let scatter_idx = cx
        ...
        .as_dtype(orbitkv_compiler::dtype::DType::Int);
    let gather_idx = cx
...
fn saturate_and_has_flashinfer_inner(
    ...
    let egraph = if let Some((s_min, s_max)) = s_interval {
        let mut intervals = orbitkv_compiler::prelude::FxHashMap::default();
        intervals.insert(
            Symbol::from('s'),
            orbitkv_compiler::shape::DimInterval::new(s_min, s_max),
        );
        let interval_facts =
            orbitkv_compiler::egglog_utils::base::interval_facts_egglog(&intervals, []);
...
fn flashinfer_dump_paged_attn_egglog() {
    ...
    let ops_vec = <CudaRuntime as orbitkv_compiler::op::Runtime>::Ops::into_vec();
...
fn flashinfer_ir_nodes(egraph: &orbitkv_compiler::egglog_utils::SerializedEGraph) -> Vec<&ENodeId> {
...
fn extract_forced_flashinfer_llir(cx: &mut Graph, case_name: &str) -> LLIRGraph {
    ...
    for (idx, flashinfer_node) in flashinfer_nodes.iter().enumerate() {
        for background in 0..64u64 {
            ...
            let mut choices =
                orbitkv_compiler::egglog_utils::random_initial_choice(egraph, &mut rng);
            let flashinfer_class = &egraph.node_to_class[*flashinfer_node];
            choices.insert(flashinfer_class, flashinfer_node);

            if let Err(err) =
                orbitkv_compiler::egglog_utils::validate_choice_set(egraph, &choices, ops)
            {
                last_error = Some(err);
                continue;
            }
            ...
            let llir = orbitkv_compiler::egglog_utils::egglog_to_llir(
                egraph,
                choices,
                ops,
                &cx.custom_ops,
...
fn run_flashinfer_bf16(
    ...
    let fi = FlashInferAttention {
        ...
        page_size: 1,
        query_tokens: Expression::from('s'),
        requests: kv_indptr.map_or(1, |indptr| indptr.len() - 1).into(),
        context_dim: Expression::from('c'),
        dtype: orbitkv_compiler::dtype::DType::Bf16,
        sm_scale: 0.0,
        window_left: -1,
        plan_info: Mutex::new(Vec::new()),
    };
    ...
    let inputs: Vec<NodeIndex> = if let Some(kv_indptr) = kv_indptr {
        ...
        let derived_qo_indptr = qo_indptr
            .is_none()
            .then(|| (0..=batch_size as i32).collect::<Vec<_>>());
        let qo_indptr = qo_indptr.unwrap_or_else(|| {
            derived_qo_indptr
                .as_deref()
                .expect("missing derived query indptr")
        });
...
fn flashinfer_bf16_prefill_causal() {
    ...
    let result = run_flashinfer_bf16(&stream, &q, &k, &v, None, None, &kv_indices, s);
    assert_close(&result, &expected, BF16_RTOL, BF16_ATOL);
}

#[test]
fn flashinfer_bf16_explicit_csr_prefill_causal() {
    if !crate::tests::utilities::gpu_supports_flashinfer() {
        return;
    }
    let Some(stream) = get_cuda_stream() else {
    ...
    let s = 4usize;
    let q = round_to_bf16(&deterministic_f32(s * HIDDEN, 0.013, 0.1));
    let k = round_to_bf16(&deterministic_f32(s * KV_DIM, 0.023, 0.1));
    let v = round_to_bf16(&deterministic_f32(s * KV_DIM, 0.033, 0.1));
    let mut expected = Vec::with_capacity(s * HIDDEN);
    for row in 0..s {
        expected.extend(run_reference_attention(
            &stream,
            &q[row * HIDDEN..(row + 1) * HIDDEN],
            &k[..(row + 1) * KV_DIM],
            &v[..(row + 1) * KV_DIM],
            1,
            row + 1,
    ...
    let qo_indptr = [0, s as i32];
    let kv_indptr = [0, s as i32];
    let kv_indices = (0..s as i32).collect::<Vec<_>>();
    let result = run_flashinfer_bf16(
        &stream,
        &q,
        &k,
...
fn gemma_mini_paged_attention(
    ...
    use orbitkv_ops::{gather_rows, scatter_rows};
    let cx = q_rope.graph();
