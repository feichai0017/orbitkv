...
fn test_bucket_dispatch_simple() {
    ...
    assert_eq!(rt.output_aliases_input_in_bucket(b, a, 0), Some(false));
    assert_eq!(rt.output_aliases_input_in_bucket(b, a, 1), Some(false));
    assert_eq!(
        rt.output_aliases_input_in_bucket(NodeIndex::new(usize::MAX), a, 0),
        None
    );
    assert_eq!(rt.output_aliases_input_in_bucket(b, a, 2), None);
...
#[test]
fn test_single_materialized_graph_bucket_switch_preserves_weights() {
    // Tests that bounded graph residency preserves weights while switching buckets.
    let Some(stream) = get_cuda_stream() else {
        return;
    };
    ...
    rt.set_max_materialized_buckets(Some(1));
    assert_eq!(
    ...
    rt.set_data(b_tensor, b_data.clone());
    rt.execute(&cx.dyn_map);
    assert_eq!(rt.debug_materialized_bucket_indices(), vec![0]);
    assert_eq!(
        rt.debug_bucket_indices_bound_to_shared_arena(),
        vec![0, 1],
    ...
    rt.set_data(b_tensor, b_data.clone());
    rt.execute(&cx.dyn_map);
    assert_eq!(rt.debug_materialized_bucket_indices(), vec![1]);
    assert_eq!(
        rt.debug_bucket_indices_bound_to_shared_arena(),
        vec![0, 1],
