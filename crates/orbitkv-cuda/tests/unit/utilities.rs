...
/// Bridges the compiler's runtime types with candle's tensor types.
pub trait TestDType:
    Clone + Sized + WithDType + PartialEq + Copy + std::fmt::Debug + 'static
where
    Vec<Self>: ToCudaInput,
{
    /// The corresponding compiler DType
    const DTYPE: orbitkv_compiler::dtype::DType;
...
impl TestDType for f32 {
    const DTYPE: orbitkv_compiler::dtype::DType = orbitkv_compiler::dtype::DType::F32;
...
impl TestDType for f16 {
    const DTYPE: orbitkv_compiler::dtype::DType = orbitkv_compiler::dtype::DType::F16;
...
impl TestDType for bf16 {
    const DTYPE: orbitkv_compiler::dtype::DType = orbitkv_compiler::dtype::DType::Bf16;
...
impl TestDType for i32 {
    const DTYPE: orbitkv_compiler::dtype::DType = orbitkv_compiler::dtype::DType::Int;
...
fn run_choice_outputs<'a>(
    ...
    if std::env::var_os("ORBITKV_FUZZ_DUMP_LAST_LLIR").is_some() {
        let _ = std::fs::write("/tmp/orbitkv_fuzz_last_candidate_llir.txt", &llir_summary);
    }
    ...
    if let Some(report) = rt.first_nonfinite_f32_buffer_in_nodes(topo_order) {
        let dump_path = "/tmp/orbitkv_fuzz_bad_candidate_llir.txt";
        ...
        for (i, (&a, &b)) in actual.iter().zip(expected.iter()).enumerate() {
            ...
            if abs > spec.atol + spec.rtol * b.abs() {
                ...
                if let Some(reference_llir_summary) = reference_llir_summary {
                    let _ = std::fs::write(
                        "/tmp/orbitkv_fuzz_bad_reference_llir.txt",
...
pub fn gpu_supports_dtype(dtype: orbitkv_compiler::dtype::DType) -> bool {
    let Some((major, minor)) = gpu_compute_cap() else {
        return false;
    };
    match dtype {
        orbitkv_compiler::dtype::DType::Bf16 => major >= 8, // Ampere (sm_80+)
        orbitkv_compiler::dtype::DType::F8E4M3 | orbitkv_compiler::dtype::DType::F8E5M2 => {
            major > 8 || (major == 8 && minor >= 9)
        } // Ada/Hopper (sm_89+)
        orbitkv_compiler::dtype::DType::F4E2M1 | orbitkv_compiler::dtype::DType::F8UE8M0 => {
            major >= 10
        } // Blackwell (sm_100+)
        _ => true,
    }
...
pub fn dtype_epsilon(dtype: orbitkv_compiler::dtype::DType) -> f32 {
    match dtype {
        orbitkv_compiler::dtype::DType::F32 => 1.19e-7, // 2^-23
        orbitkv_compiler::dtype::DType::F16 => 9.77e-4, // 2^-10
        orbitkv_compiler::dtype::DType::Bf16 => 7.81e-3, // 2^-7
        orbitkv_compiler::dtype::DType::Int => 0.0,
        orbitkv_compiler::dtype::DType::Bool => 0.0,
        other => todo!("dtype_epsilon not implemented for {other}"),
    }
}

/// Map a compiler DType to the corresponding candle DType.
pub fn to_candle_dtype(dtype: orbitkv_compiler::dtype::DType) -> candle_core::DType {
    match dtype {
        orbitkv_compiler::dtype::DType::F32 => candle_core::DType::F32,
        orbitkv_compiler::dtype::DType::F16 => candle_core::DType::F16,
        orbitkv_compiler::dtype::DType::Bf16 => candle_core::DType::BF16,
        orbitkv_compiler::dtype::DType::Int => candle_core::DType::I32,
        orbitkv_compiler::dtype::DType::Bool => candle_core::DType::U8,
