use as_any::Downcast;
use orbitkv_compiler::egglog_utils::run_egglog;
use orbitkv_compiler::egglog_utils::{
    ClassId, SerializedEGraph, egglog_to_llir, random_initial_choice,
};
use orbitkv_compiler::op::{IntoEgglogOp, LLIROp};
...
fn static_planning_accepts_split_and_fused_regions_but_rejects_cycle() {
    ...
    let input = split.add_node(LLIROp::new::<orbitkv_compiler::hlir::Input>(Box::default()));
    ...
    let input = fused.add_node(LLIROp::new::<orbitkv_compiler::hlir::Input>(Box::default()));
...
/// `cargo test -p orbitkv-cuda -- --ignored bench_fused_vs_unfused_sqrt_recip --nocapture`.
#[test]
#[ignore]
...
///
/// Ignored by default — run with
/// `cargo test -p orbitkv-cuda -- --ignored bench_fused_region_vs_unfused_3op --nocapture`.
#[test]
#[ignore]
...
fn test_cast_after_unary_fuses() {
    ...
    let mut cx = Graph::new();
    let a = cx.tensor(16);
    let _b = a.sin().cast(orbitkv_compiler::dtype::DType::Bf16).output();
...
fn test_cast_producer_absorbed_into_region() {
    ...
    let mut cx = Graph::new();
    let a = cx.tensor(16).as_dtype(orbitkv_compiler::dtype::DType::Bf16);
    let _b = a.cast(orbitkv_compiler::dtype::DType::F32).sin().output();
...
fn test_cast_boundary_is_destructively_absorbed() {
    ...
    let a = cx.tensor(16);
    a.sin()
        .cast(orbitkv_compiler::dtype::DType::Bf16)
        .sqrt()
        .output();
...
fn test_cast_fusion_preserves_output() {
    ...
    let tol = dtype_epsilon(orbitkv_compiler::dtype::DType::Bf16) * TOLERANCE_SAFETY_FACTOR;
    test_binary_cuda::<f32>(
        16,
        16,
        |a, b| {
            (a + b)
                .sin()
                .cast(orbitkv_compiler::dtype::DType::Bf16)
                .cast(orbitkv_compiler::dtype::DType::F32)
        },
        |a, b| {
            (a + b)
                .unwrap()
