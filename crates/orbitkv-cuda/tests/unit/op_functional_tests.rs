...
// run `cargo test -p orbitkv-cuda -- --ignored` for the broader sweeps.

proptest! {
    #![proptest_config(ProptestConfig::with_cases(5))]

    #[ignore = "expensive CUDA op proptest sweep; run with cargo test -p orbitkv-cuda -- --ignored"]
    ...
    fn test_matmul(
        (m, n, k, a_col_major, b_col_major, m_slice, k_slice, n_slice, dtype) in
            (1usize..128, 1usize..128, 1usize..128, any::<bool>(), any::<bool>(),
             ...
             prop::sample::select(&[orbitkv_compiler::dtype::DType::F32, orbitkv_compiler::dtype::DType::F16, orbitkv_compiler::dtype::DType::Bf16]))
        ...
        let orbitkv_op = move |a: GraphTensor, b: GraphTensor| {
            let a = a.cast(dtype);
            let b = b.cast(dtype);
            ...
            a.matmul(b).cast(orbitkv_compiler::dtype::DType::F32)
        };
        ...
        let gen_lambda = |n, s| random_f32_vec(n, s, -0.5, 0.5);
        test_binary_cuda(a_shape, b_shape, orbitkv_op, candle_op, gen_lambda, gen_lambda, seed, rtol, atol);
    }
    ...
    fn test_less_than(x in 1usize..100, y in 1usize..5, seed in any::<u64>()) {
        ...
        test_binary_cuda(x, x, |a, b| a.lt(b).cast(orbitkv_compiler::dtype::DType::F32), |a, b| a.lt(&b).unwrap().to_dtype(candle_core::DType::F32).unwrap(), gen_lambda, gen_lambda, seed, 0.0, 0.0);
        test_binary_cuda((y, x), (y, x), |a, b| a.lt(b).cast(orbitkv_compiler::dtype::DType::F32), |a, b| a.lt(&b).unwrap().to_dtype(candle_core::DType::F32).unwrap(), gen_lambda, gen_lambda, seed, 0.0, 0.0);
    }
}
...
proptest! {
    ...
    #[ignore = "expensive CUDA genome fuzzing; run with cargo test -p orbitkv-cuda -- --ignored"]
    #[test]
