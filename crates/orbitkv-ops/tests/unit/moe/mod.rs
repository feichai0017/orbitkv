...
fn dump_glu_moe_egglog() {
    ...
    use orbitkv_compiler::egglog_utils::hlir_to_egglog;
    ...
    let k_expr = orbitkv_compiler::shape::Expression::from(top_k);
    ...
    let row_offsets = cx.iota(
        orbitkv_compiler::shape::Expression::from('z') / k_expr * e_dim,
        top_k_indices.dims(),
    );
...
fn gather_experts_test(
    ...
    let within = graph_source
        .graph()
        .iota(orbitkv_compiler::shape::Expression::from('z'), (d1, d2))
        .cast(DType::F32);
