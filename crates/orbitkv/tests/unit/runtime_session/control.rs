...
fn committed_attach_cancel_replays_tombstone_until_finalize() {
    ...
    assert_eq!(session.cancel_pending_attach(identity), Ok(expected));
    let after_cancel = session.stats();
    assert_eq!(after_cancel.active_requests, after_attach.active_requests);
    assert_eq!(
        after_cancel.active_snapshots + 1,
        after_attach.active_snapshots
    );
    assert_eq!(
    ...
    assert_eq!(
        ...
        Err(RuntimeSessionError::Manager(
            KvManagerError::ArenaExhausted("request")
        ))
    );
    ...
    assert_eq!(
        session.stats().active_requests + 1,
        after_attach.active_requests
    );
    session
...
fn pending_attach_cancel_expectation_mismatch_is_failure_atomic() {
    ...
    for mismatched in [
        ...
        EnginePendingAttachCancel {
            resident_count: exact.resident_count + 1,
            ..exact
        },
    ] {
        assert_eq!(
    ...
    for sequence in 0..3 {
        ...
        if sequence == 0 {
            assert_eq!(
                ...
                Err(RuntimeSessionError::Manager(
                    KvManagerError::ArenaExhausted("control")
                ))
            );
        }
        session
...
fn control_id_exhaustion_preserves_finalized_cancel_replay() {
    ...
    assert_eq!(session.finalize_pending_attach_cancel(exact), Ok(finalized));
}
