...
pub(super) type ArenaCounts = (u16, u32, u32, u64, u64, u64, u64, u64, u64, u64);

pub(super) fn sliding_plan(window_tokens: u64, page_tokens: u64) -> CompiledKvPlan {
...
pub(super) fn full_plan(page_tokens: u64) -> CompiledKvPlan {
    compile_plan(KvPlanInput {
        page_tokens,
        classes: vec![KvClassSpec {
            name: "full".into(),
...
pub(super) fn chunked_plan(chunk_tokens: u64) -> CompiledKvPlan {
    let divisor = i64::try_from(chunk_tokens).expect("test chunk size fits i64");
    ...
    .expect("test chunked plan compiles")
}

pub(super) fn hybrid_plan(window_tokens: u64) -> CompiledKvPlan {
    compile_plan(KvPlanInput {
        page_tokens: CANONICAL_PAGE_TOKENS,
        classes: vec![
            KvClassSpec {
...
pub(super) fn chunked_manager(
    ...
    manager_for_plan(
        &chunked_plan(chunk_tokens),
        &[backend(0, 211, page_count, 13_000)],
        maximum_step_tokens,
        maximum_reclamations,
    )
}

pub(super) fn operation_entries(
...
pub(super) fn share_snapshot_for_cow(
...
pub(super) fn root_node_addresses(
    root: Option<&Arc<RootTreeNode>>,
...
pub(super) fn published_entries(
    manager: &CanonicalKvManager,
    request: RequestLease,
) -> Vec<DeviceKvEntry> {
    let snapshot = manager
...
pub(super) fn assert_materialization_matches_snapshot(
    manager: &CanonicalKvManager,
...
pub(super) fn assert_canonical_prepared_spans(prepared: &PreparedStep) {
...
pub(super) fn copy_receipts(prepared: &PreparedStep) -> Vec<BackendCopyReceipt> {
    assert_canonical_prepared_spans(prepared);
    prepared
        .class_lowerings
        .iter()
...
pub(super) fn submit(manager: &mut CanonicalKvManager, prepared: &PreparedStep) -> SubmittedStep {
...
#[derive(Clone, Debug)]
pub(super) struct TestCompletion {
    pub(super) step: StepCompletion,
    pub(super) retirements: Box<[ReclamationCertificate]>,
}
...
pub(super) fn assert_snapshot_transition_identities(
    manager: &CanonicalKvManager,
    prepared: &PreparedStep,
    submitted: &SubmittedStep,
...
pub(super) fn reclamation_receipts(
    certificates: &[ReclamationCertificate],
) -> Vec<ReclamationReceipt> {
    certificates
        .iter()
...
pub(super) fn arena_counts(manager: &CanonicalKvManager) -> Vec<ArenaCounts> {
    manager
        .arena_stats()
        .iter()
        .map(|stats| {
...
pub(super) fn assert_incremental_census_matches_full_scan(manager: &CanonicalKvManager) {
...
pub(super) fn assert_reference_census_matches_full_scan(manager: &CanonicalKvManager) {
...
pub(super) fn staged_prefix_attach_fixture() -> (
    CanonicalKvManager,
    Box<[RequestLease]>,
...
pub(super) fn partial_fork_fixture(
...
pub(super) fn state_image(manager: &CanonicalKvManager) -> String {
    format!("{manager:#?}")
}

pub(super) fn complete_initial_18(
    manager: &mut CanonicalKvManager,
    request: RequestLease,
) -> TestCompletion {
    let prepared = manager
    ...
    let completion_value = manager.completion_high_water.get(&7).map_or(9, |value| {
        value.checked_add(1).expect("test completion value")
    });
    complete(manager, &submitted, 7, completion_value)
}

pub(super) fn batch_submit_items(
    manager: &CanonicalKvManager,
    prepared: &[PreparedStep],
) -> (
    Vec<SubmitBatchItem>,
