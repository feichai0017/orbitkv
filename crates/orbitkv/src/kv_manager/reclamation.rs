...
impl CanonicalKvManager {
    pub(super) fn clamped_page_token_span(
        &self,
        logical_ordinal: u64,
        boundary: u64,
        begin_overflow: &'static str,
        empty_span: &'static str,
    ) -> Result<(u64, u64), KvManagerError> {
        let token_begin = logical_ordinal
            .checked_mul(self.page_tokens)
            .ok_or(KvManagerError::ArithmeticOverflow(begin_overflow))?;
        let remaining = boundary
            .checked_sub(token_begin)
            .filter(|remaining| *remaining != 0)
            .ok_or(KvManagerError::Invariant(empty_span))?;
        let token_count = remaining.min(self.page_tokens);
        let token_end_exclusive = token_begin
            .checked_add(token_count)
            .expect("span clamped to a representable boundary");
    ...
    fn detached_binding_span(
        ...
        self.clamped_page_token_span(
            entry.logical_ordinal,
            resident_boundary,
            "detached binding token begin",
            "empty detached binding span",
        )
    }
    ...
    pub(super) fn certificate_for_root(
        ...
        let (token_begin, token_end_exclusive) = self.clamped_page_token_span(
            entry.logical_ordinal,
            target_boundary,
            "reclamation token begin",
            "empty reclamation token span",
        )?;
    ...
    pub(super) fn insert_completion_candidate(
        ...
        let (_, token_end) = self.clamped_page_token_span(
            entry.logical_ordinal,
            resident_boundary,
            "candidate token begin",
            "empty candidate resident span",
        )?;
