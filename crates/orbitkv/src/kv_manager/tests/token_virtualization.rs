...
fn full_evacuation_relocates_exact_tokens_and_reclaims_source_pages() {
    ...
    for (domain, value, expected) in [
        ...
        (5, 0, KvManagerError::InvalidCompletionFrontier),
        (
            17,
            48,
...
fn hybrid_full_relocation_keeps_swa_class_specific_placement_and_appends() {
    ...
    assert_eq!(cow_prepared.class_lowerings[0].previous_layout_boundary, 25);
    assert_eq!(cow_prepared.class_lowerings[0].target_layout_boundary, 26);
    assert_eq!(cow_prepared.class_lowerings[1].previous_layout_boundary, 49);
    assert_eq!(cow_prepared.class_lowerings[1].target_layout_boundary, 50);
...
fn append_step(
    ...
    let completion_value = manager
        .completion_high_water
        .get(&17)
        .map_or(target_boundary, |value| {
            target_boundary.max(value.checked_add(1).expect("test completion value"))
        });
