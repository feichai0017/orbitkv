...
#[test]
fn dense_cow_preserves_explicit_disposition_and_moves_dead_source_location() {
    let plan = full_plan(CANONICAL_PAGE_TOKENS);
    let mut manager = manager_for_plan(&plan, &[backend(0, 230, 8, 145_000)], 64, 16);
    let requests = manager
        .acquire_request_leases_for_test(2)
        .expect("dense shared requests");
    let initial = complete_initial_18(&mut manager, requests[0]);
    let marked = manager
        .mark_token_dispositions_batch(&[TokenDispositionBatchItem {
            request: requests[0],
            ...
            updates: vec![ClassTokenDispositionUpdate {
                class_id: 0,
                token_id: 16,
                disposition: TokenDisposition::policy_evicted(92, 3, 102),
            }]
            .into_boxed_slice(),
        }])
        .expect("mark dense-tail token dead")[0];
    let before = manager
        .token_views_batch(&[TokenViewQuery {
            request: requests[0],
            expected_snapshot: marked.snapshot,
        ...
        .unwrap()[0]
        .clone();
    let old_location = before.placements[16].location.unwrap();
    assert!(!before.placements[16].disposition.retained());

    manager
        .fork_requests_batch(&[RequestForkItem {
            source_request: requests[0],
            expected_source_head: marked.snapshot,
            target_empty_request: requests[1],
            expected_target_head: manager.request(requests[1]).unwrap().head,
        }])
        .expect("fork dense marked tail");
    let prepared = manager
        .prepare_batch(&[PrepareBatchItem {
            request: requests[0],
            expected_head: marked.snapshot,
            target_boundary: 19,
        }])
        .expect("dense COW prepare")[0]
        .clone();
    assert_eq!(prepared.tail_actions[0].kind, TailActionKind::CopyOnWrite);
    assert_eq!(prepared.tail_actions[0].valid_token_count, 2);
    let submitted = submit(&mut manager, &prepared);
    let completed = complete(&mut manager, &submitted, 31, 37);
    let after = manager
        .token_views_batch(&[TokenViewQuery {
            request: requests[0],
            expected_snapshot: completed.publication.snapshot,
            class_id: 0,
        }])
        .unwrap()[0]
        .clone();
    assert_eq!(
        after.placements[16].token_id,
        before.placements[16].token_id
    );
    assert_eq!(
        after.placements[16].disposition,
        before.placements[16].disposition
    );
    let dead_location = after.placements[16].location.unwrap();
    assert_eq!(dead_location.page, prepared.copy_intents[0].destination);
    assert_eq!(
        dead_location.backend_index,
        prepared.copy_intents[0].destination_backend_index
    );
    assert_eq!(dead_location.offset, old_location.offset);
    assert_eq!(
        after.placements[17].token_id,
        before.placements[17].token_id
    );
    assert_eq!(
        after.placements[17].disposition,
        before.placements[17].disposition
    );
    let retained_location = after.placements[17].location.unwrap();
    assert_eq!(retained_location.page, prepared.copy_intents[0].destination);
    assert_eq!(
        retained_location.backend_index,
        prepared.copy_intents[0].destination_backend_index
    );
    assert_eq!(retained_location.offset, 1);
    assert_eq!(old_location.offset, 0);
    assert_reference_census_matches_full_scan(&manager);
}

#[test]
fn cow_copy_intent_preflight_rebuilds_every_semantic_field_without_mutation() {
    let plan = full_plan(CANONICAL_PAGE_TOKENS);
    let mut manager = manager_for_plan(&plan, &[backend(0, 232, 8, 155_000)], 64, 16);
    let requests = manager
        .acquire_request_leases_for_test(2)
        .expect("copy-intent requests");
    ...
    let prepared = manager
        ...
        .expect("COW prepare")[0]
        .clone();
    assert_eq!(prepared.copy_intents.len(), 1);
        ...
        .expect("prepared operation")
    {
        ...
        OperationState::Submitted(_) => panic!("operation must still be prepared"),
    };

    let corruptions: [fn(&mut CopyIntent); 7] = [
        |intent| intent.token_count += 1,
        |intent| intent.source_token_offset = 1,
        |intent| intent.destination_token_offset = 1,
        |intent| intent.reserved = 1,
        |intent| intent.backend_domain += 1,
        |intent| intent.source_backend_index += 1,
        |intent| intent.destination_backend_index += 1,
    ];
    for corrupt in corruptions {
        let mut forged = state.clone();
        let delta = Arc::make_mut(&mut forged.delta);
        let mut classes = delta.classes.to_vec();
        let mut intent = classes[0].copy_intent.expect("copy intent");
        corrupt(&mut intent);
        classes[0].copy_intent = Some(intent);
        delta.classes = classes.into_boxed_slice();
        let before = state_image(&manager);
        assert_eq!(
            manager.preflight_prepared_delta(&forged, prepared.step),
            Err(KvManagerError::Invariant("copy intent shape"))
        );
        assert_eq!(state_image(&manager), before);
    }
    assert_reference_census_matches_full_scan(&manager);
...
#[test]
#[allow(clippy::too_many_lines)]
fn packed_partial_b4_cow_rewrites_exact_old_locations_and_certifies_last_owners() {
    let (mut manager, source, targets, source_view) = packed_fork_fixture(24);
    let marked = manager
        .mark_token_dispositions_batch(&[TokenDispositionBatchItem {
            request: source,
            expected_snapshot: source_view.snapshot,
            updates: vec![ClassTokenDispositionUpdate {
                class_id: 0,
                token_id: 35,
                disposition: TokenDisposition::policy_evicted(91, 2, 101),
            }]
            .into_boxed_slice(),
        }])
        .expect("mark one packed-tail token dead")[0];
    let old_entries = snapshot_entries(&manager, source);
    let old_tail = old_entries[1];
    let source_tokens = manager
        .token_views_batch(&[TokenViewQuery {
            request: source,
            expected_snapshot: marked.snapshot,
        ...
        .expect("packed source token view")[0]
        .clone();
    assert_eq!(
        source_tokens.placements[35].location.unwrap().page,
        old_tail.page
    );
    assert!(!source_tokens.placements[35].disposition.retained());

    let forked = manager
        .fork_requests_batch(&fork_items(&manager, source, &targets))
        .expect("packed partial B4 fork");
    assert_eq!(forked.len(), 4);
    assert!(forked.iter().all(|item| {
        item.target.view.boundary == 48
            && item.target.view.resident_count == 2
            && item.target.pages[1].valid_token_count == 8
    }));
    assert!(
        old_entries
            .iter()
            .all(|entry| manager.page(entry.page.page_id).unwrap().request_refs == 5)
    );
    assert_reference_census_matches_full_scan(&manager);
    assert_incremental_census_matches_full_scan(&manager);

    // Packed Prefix remains deliberately unsupported even though packed request
    // fork and shared-tail COW are legal.
    let before_prefix = state_image(&manager);
    assert!(matches!(
        manager.publish_prefix_batch(&[PrefixPublishItem {
            request: source,
            expected_head: marked.snapshot,
    ...
    assert_eq!(state_image(&manager), before_prefix);

    let prepared = manager
        .prepare_batch(
            &forked
                .iter()
                .map(|item| PrepareBatchItem {
                    request: item.target.view.request,
                    expected_head: item.target.view.snapshot,
                    target_boundary: 49,
                })
                .collect::<Vec<_>>(),
        )
        .expect("packed partial B4 COW prepare");
    assert_eq!(prepared.len(), 4);
    assert!(prepared.iter().all(|item| {
        item.class_lowerings.len() == 1
            && item.class_lowerings[0].flags == CLASS_LOWERING_PACKED
            && item.tail_actions.len() == 1
            && item.tail_actions[0].kind == TailActionKind::CopyOnWrite
            && item.tail_actions[0].valid_token_count == 8
            && item.tail_actions[0].source == old_tail.page
            && item.copy_intents.len() == 1
            && item.copy_intents[0].token_count == 8
            && item.copy_intents[0].source_token_offset == 0
            && item.copy_intents[0].destination_token_offset == 0
            && item.copy_intents[0].source == old_tail.page
            && item.copy_intents[0].source_backend_index == old_tail.backend_index
            && item.write_intents.is_empty()
    }));
    assert_eq!(
        prepared
            .iter()
            .map(|item| item.copy_intents[0].destination)
            .collect::<BTreeSet<_>>()
            .len(),
        4
    );
    ...
    malformed[1].copy_receipt_offset += 1;
    let before_bad_receipt = state_image(&manager);
    assert_eq!(
        manager.submit_batch(&malformed, &receipts, &copies),
        Err(KvManagerError::InvalidBatchRange)
    );
    assert_eq!(state_image(&manager), before_bad_receipt);

    let submitted = manager
        .submit_batch(&items, &receipts, &copies)
        .expect("packed partial B4 COW submit");
    assert_incremental_census_matches_full_scan(&manager);
    let submissions = submitted
        .iter()
        .map(|item| item.submission)
        .collect::<Vec<_>>();
    let before_bad_completion = state_image(&manager);
    assert_eq!(
    ...
    assert_eq!(state_image(&manager), before_bad_completion);
    ...
    let completed = manager
        ...
        .expect("packed partial B4 COW complete");
    assert!(completed.retirements.is_empty());
    assert!(completed.completions.iter().all(|item| {
        item.detached.len() == 1
            && item.detached[0].old == old_tail.page
            && item.detached[0].logical_ordinal == 1
            && item.detached[0].token_begin == 16
            && item.detached[0].token_end_exclusive == 24
            && item.detached[0].reason == DetachedReason::CopyOnWrite
    }));

    for ((prepared, completion), target) in prepared
        .iter()
        .zip(completed.completions.iter())
        .zip(targets.iter().copied())
    {
        let copy = prepared.copy_intents[0];
        let target_tokens = manager
            .token_views_batch(&[TokenViewQuery {
                request: target,
            ...
            .expect("COW target token view")[0]
            .clone();
        for (before, after) in source_tokens
            .placements
            .iter()
            .zip(target_tokens.placements.iter())
        {
            ...
            if before.location.is_some_and(|location| {
                location.page == old_tail.page && location.backend_index == old_tail.backend_index
            }) {
        ...
        let appended = target_tokens.placements[48];
        assert!(appended.disposition.retained());
        assert_eq!(appended.location.unwrap().page, copy.destination);
        assert_eq!(appended.location.unwrap().offset, 8);
    }
    let source_after_cow = manager
        .token_views_batch(&[TokenViewQuery {
            request: source,
            expected_snapshot: marked.snapshot,
            class_id: 0,
        }])
        .expect("source remains readable")[0]
        .clone();
    assert_eq!(source_after_cow.placements, source_tokens.placements);
    assert_eq!(manager.page(old_tail.page.page_id).unwrap().request_refs, 1);
    assert_eq!(
        manager.page(old_tail.page.page_id).unwrap().phase,
        PagePhase::Live
    );
    assert_reference_census_matches_full_scan(&manager);
    assert_incremental_census_matches_full_scan(&manager);

    // Fork one completed COW snapshot back onto the now-empty original source
    // request, then COW the same packed partial tail again. This proves that
    // dead-but-physically-resident slots still form the required contiguous
    // packed offset coverage after the first COW.
    let source_empty = manager
        .release_current_for_test(&[source])
        .expect("release original packed source for refork");
    assert_eq!(source_empty.retirements.len(), 1);
    assert_eq!(source_empty.retirements[0].page, old_tail.page);
    manager
        .acknowledge_reclamations_batch(&reclamation_receipts(&source_empty.retirements))
        .expect("ack original packed tail");
    manager.recycle_requests_batch(&[source]).unwrap();
    let reacquired = manager
        .acquire_request_leases_for_test(1)
        .expect("reacquire empty request")[0];
    assert_eq!(reacquired.slot, source.slot);
    let reforked = manager
        .fork_requests_batch(&[RequestForkItem {
            source_request: targets[1],
            expected_source_head: completed.completions[1].publication.snapshot,
            target_empty_request: reacquired,
            expected_target_head: manager.request(reacquired).unwrap().head,
        }])
        .expect("fork completed packed COW snapshot");
    let repeated = manager
        .prepare_batch(&[PrepareBatchItem {
            request: reacquired,
            expected_head: reforked[0].target.view.snapshot,
            target_boundary: 50,
        }])
        .expect("second packed COW prepare")[0]
        .clone();
    assert_eq!(repeated.tail_actions[0].kind, TailActionKind::CopyOnWrite);
    assert_eq!(repeated.tail_actions[0].valid_token_count, 9);
    assert_eq!(repeated.copy_intents.len(), 1);
    assert_eq!(repeated.copy_intents[0].token_count, 9);
    let repeated_submitted = submit(&mut manager, &repeated);
    let repeated_completed = complete(&mut manager, &repeated_submitted, 43, 45);
    assert!(repeated_completed.retirements.is_empty());
    let repeated_tokens = manager
        .token_views_batch(&[TokenViewQuery {
            request: reacquired,
            expected_snapshot: repeated_completed.publication.snapshot,
            class_id: 0,
        }])
        .expect("second packed COW token view")[0]
        .clone();
    assert_eq!(
        repeated_tokens.placements[35].disposition,
        source_tokens.placements[35].disposition
    );
    let repeated_dead = repeated_tokens.placements[35].location.unwrap();
    assert_eq!(repeated_dead.page, repeated.copy_intents[0].destination);
    assert_eq!(
        repeated_dead.backend_index,
        repeated.copy_intents[0].destination_backend_index
    );
    assert_eq!(
        repeated_dead.offset,
        source_tokens.placements[35].location.unwrap().offset
    );
    assert_eq!(repeated_tokens.placements[49].location.unwrap().offset, 9);
    assert_reference_census_matches_full_scan(&manager);
    assert_incremental_census_matches_full_scan(&manager);

    let first_target = targets[0];
    let first_publication = completed.completions[0].publication;
    let second_prepared = manager
        .prepare_batch(&[PrepareBatchItem {
            request: first_target,
            expected_head: first_publication.snapshot,
            target_boundary: 50,
        }])
        .expect("append again after packed COW");
    assert_eq!(
        second_prepared[0].tail_actions[0].kind,
        TailActionKind::InPlace
    );
    assert_eq!(second_prepared[0].tail_actions[0].valid_token_count, 9);
    assert!(second_prepared[0].copy_intents.is_empty());
    let second_submitted = submit(&mut manager, &second_prepared[0]);
    let second_completed = complete(&mut manager, &second_submitted, 43, 47);
    assert!(second_completed.retirements.is_empty());
    let second_tokens = manager
        .token_views_batch(&[TokenViewQuery {
            request: first_target,
            expected_snapshot: second_completed.publication.snapshot,
            class_id: 0,
        }])
        .expect("second append token view")[0]
        .clone();
    assert_eq!(second_tokens.placements[49].location.unwrap().offset, 9);
    assert_eq!(
        second_tokens.placements[49].location.unwrap().page,
        prepared[0].copy_intents[0].destination
    );

    let sibling_release = manager
        .release_current_for_test(&targets[1..])
        .expect("release COW siblings");
    assert_eq!(sibling_release.retirements.len(), 3);
    assert!(sibling_release.retirements.iter().all(|certificate| {
        certificate.logical_ordinal == 1
            && certificate.token_begin == 16
            && certificate.token_end_exclusive == 25
    }));
    let refork_source_certificate = sibling_release
        .retirements
        .iter()
        .find(|certificate| certificate.page == repeated.copy_intents[0].source)
        .expect("refork source certificate");
    assert_eq!(refork_source_certificate.completion_domain, 43);
    assert_eq!(refork_source_certificate.completion_value, 45);
    assert!(sibling_release.retirements.iter().all(|certificate| {
        certificate.page == repeated.copy_intents[0].source
            || certificate.completion_domain == 41 && certificate.completion_value == 43
    }));
    manager
        .acknowledge_reclamations_batch(&reclamation_receipts(&sibling_release.retirements))
        .expect("ack sibling COW tails");
    manager.recycle_requests_batch(&targets[1..]).unwrap();
    let repeated_release = manager
        .release_current_for_test(&[reacquired])
        .expect("release second COW request");
    assert_eq!(repeated_release.retirements.len(), 1);
    assert_eq!(repeated_release.retirements[0].logical_ordinal, 1);
    assert_eq!(repeated_release.retirements[0].token_begin, 16);
    assert_eq!(repeated_release.retirements[0].token_end_exclusive, 26);
    manager
        .acknowledge_reclamations_batch(&reclamation_receipts(&repeated_release.retirements))
        .expect("ack second COW pages");
    manager.recycle_requests_batch(&[reacquired]).unwrap();

    let final_release = manager
        .release_current_for_test(&targets[..1])
        .expect("release final extended owner");
    assert_eq!(final_release.retirements.len(), 2);
    assert_eq!(
        final_release
            .retirements
            .iter()
    ...
    manager
        .acknowledge_reclamations_batch(&reclamation_receipts(&final_release.retirements))
        .expect("ack final packed pages");
    manager.recycle_requests_batch(&targets[..1]).unwrap();
    let stats = manager.stats();
    assert_eq!(stats.free_pages, 16);
    assert_eq!(stats.active_requests, 0);
