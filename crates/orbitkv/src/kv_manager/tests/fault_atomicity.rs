...
#[test]
fn final_representable_page_span_clamps_without_hiding_begin_overflow() {
    let manager = manager_for_plan(
        &full_plan(CANONICAL_PAGE_TOKENS),
        &[backend(0, 191, 1, 9_000)],
        16,
        1,
    );
    let final_ordinal = u64::MAX / CANONICAL_PAGE_TOKENS;
    let token_begin = u64::MAX - (CANONICAL_PAGE_TOKENS - 1);
    assert_eq!(
        manager.clamped_page_token_span(
            final_ordinal,
            u64::MAX,
            "test token begin",
            "empty test span",
        ),
        Ok((token_begin, u64::MAX))
    );
    assert_eq!(
        manager.clamped_page_token_span(
            final_ordinal,
        ...
        Err(KvManagerError::Invariant("empty test span"))
    );
    assert_eq!(
        manager.clamped_page_token_span(
            final_ordinal + 1,
        ...
        Err(KvManagerError::ArithmeticOverflow("test token begin"))
    );
}

#[test]
fn final_representable_page_materialization_and_reclamation_are_clamped() {
    let manager = manager_for_plan(
        &full_plan(CANONICAL_PAGE_TOKENS),
        &[backend(0, 190, 1, 8_000)],
        16,
        1,
    );
    ...
    let entry = manager
        .root_entry_for_page(
            ...
            PageLease {
                ...
                generation: 0,
                page_id: class.first_page_id,
                pool_id: class.backend.pool_id,
            },
        )
        .expect("final representable root entry");
    ...
    assert_eq!(materialized[0].logical_ordinal, final_ordinal);
    assert_eq!(materialized[0].valid_token_count, 15);
    assert_eq!(materialized[0].visible_token_offset, 0);
    assert_eq!(materialized[0].visible_token_count, 15);

    let detached = manager
        .clear_detached_binding(entry, u64::MAX, DetachedReason::RequestRelease)
        .expect("final partial page detaches");
    assert_eq!(
    ...
    let certificate = manager
        .certificate_for_root(entry, u64::MAX, (0, 0), 23, 29)
        .expect("final partial page is certifiable");
    assert_eq!(
        (certificate.token_begin, certificate.token_end_exclusive),
        (token_begin, u64::MAX)
    );
    assert_eq!(
        (certificate.completion_domain, certificate.completion_value),
        (23, 29)
    );

    let mut candidates = BTreeMap::new();
    manager
        .insert_completion_candidate(&mut candidates, entry, u64::MAX)
        .expect("final partial page is a completion candidate");
    manager
        .insert_completion_candidate(&mut candidates, entry, u64::MAX)
        .expect("matching shared candidate remains valid");
    assert_eq!(candidates.get(&entry.page), Some(&(entry, u64::MAX)));
    assert_eq!(
        manager.insert_completion_candidate(&mut candidates, entry, u64::MAX - 1),
        Err(KvManagerError::Invariant(
...
#[test]
fn invalid_or_nonadvancing_completion_frontiers_are_zero_mutation_and_retryable() {
    let mut manager = manager_with(18, 4, 64, 8);
    let requests = manager
        .acquire_request_leases_for_test(2)
        .expect("requests");
    ...
    complete(&mut manager, &first, 23, 7);
    ...
    for (domain, value, expected) in [
        (0, 1, KvManagerError::InvalidCompletionFrontier),
        (23, 0, KvManagerError::InvalidCompletionFrontier),
        (
            23,
            7,
    ...
    assert_eq!(
        complete(&mut manager, &second, 23, 8)
            .publication
            .view_version,
        ViewVersion(1)
    );
