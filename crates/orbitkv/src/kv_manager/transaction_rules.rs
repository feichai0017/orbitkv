...
impl CanonicalKvManager {
    ...
    fn validate_epoch_delta(
        class: RuntimeClass,
        delta: &ClassDelta,
        previous_boundary: u64,
        target_boundary: u64,
    ) -> Result<(), KvManagerError> {
        if target_boundary <= previous_boundary {
            return Err(KvManagerError::Invariant("delta boundary ordering"));
        }
        let expected_reset = class.is_epoch_reset(previous_boundary, target_boundary);
        if delta.epoch_reset != expected_reset {
            return Err(KvManagerError::Invariant("delta epoch reset"));
        }
        if class.retention != RetentionKind::Chunked {
            return Ok(());
        }
        let chunk_tokens = class
            .chunk_tokens
            .ok_or(KvManagerError::Invariant("chunk size"))?;
        let blocks_per_epoch = class
            .blocks_per_epoch
            .ok_or(KvManagerError::Invariant("chunk block count"))?;
        if class.append_crosses_epoch(previous_boundary, target_boundary) {
            return Err(KvManagerError::ChunkBoundaryCrossed {
            ...
            || delta.target_layout_boundary != target_boundary
        {
            return Err(KvManagerError::Invariant(
                "chunked layout boundary must stay absolute",
            ));
        }
        if expected_reset
            && (delta.tail_action != TailActionKind::None
                || delta.tail_source.is_some()
                || delta.tail_destination.is_some()
                || delta.copy_intent.is_some())
        {
            return Err(KvManagerError::Invariant("chunked reset tail"));
        }
        let append_epoch = previous_boundary / chunk_tokens;
        let first_epoch_ordinal = append_epoch
            .checked_mul(blocks_per_epoch)
            .ok_or(KvManagerError::ArithmeticOverflow("chunk epoch ordinal"))?;
        let end_epoch_ordinal = first_epoch_ordinal
            .checked_add(blocks_per_epoch)
            .ok_or(KvManagerError::ArithmeticOverflow("chunk epoch ordinal"))?;
        if delta.writes.iter().any(|entry| {
            entry.logical_ordinal < first_epoch_ordinal
                || entry.logical_ordinal >= end_epoch_ordinal
        }) {
            return Err(KvManagerError::Invariant("chunked write epoch"));
        }
        Ok(())
    }
