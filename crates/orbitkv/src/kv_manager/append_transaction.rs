...
impl CanonicalKvManager {
    ...
    pub fn prepare_batch(
        ...
        for item in items {
            self.preflight_prepare_item(item)?;
        }
            ...
            .zip(planned_snapshots.iter().copied())
        {
            ...
            let class_boundaries = self
                ...
                .map(|class| {
                    ...
                    Ok((
                        ...
                        class.is_epoch_reset(previous_boundary, target_layout_boundary),
                    ))
                })
            ...
            let tails = self
                ...
                .map(|(class, (previous_layout_boundary, _, epoch_reset))| {
                    let root = snapshot
                        .roots
                        .get(usize::from(class.class_id))
                        .ok_or(KvManagerError::Invariant("snapshot class cardinality"))?;
            ...
            for (
                (class, previous_tail),
                (previous_layout_boundary, target_layout_boundary, epoch_reset),
            ) in self
                .classes
                .iter()
                .copied()
                ...
                .zip(class_boundaries.iter().copied())
            {
                ...
                let previous_tail_ordinal = (!epoch_reset
                    && previous_layout_boundary % self.page_tokens != 0)
                    .then_some(previous_layout_boundary / self.page_tokens);
                ...
                let epoch_start = class.retention == RetentionKind::Chunked
                    && previous_layout_boundary.is_multiple_of(
                        class
                            .chunk_tokens
                ...
                if epoch_start {
                    flags |= CLASS_LOWERING_EPOCH_START;
                }
    ...
    fn preflight_prepare_item(&self, item: &PrepareBatchItem) -> Result<(), KvManagerError> {
        ...
        for class in self.classes.iter().copied() {
            if class.retention == RetentionKind::Chunked
                && class.append_crosses_epoch(snapshot.boundary, item.target_boundary)
            {
