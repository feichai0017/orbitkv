...
pub enum KvManagerError {
    ...
    #[error("append [{previous}, {target}) crosses chunk boundary for chunk size {chunk_tokens}")]
    ...
    #[error("completion domain and value must both be nonzero")]
    InvalidCompletionFrontier,
    #[error(
        "completion value {received} did not advance domain {completion_domain} beyond {previous}"
    )]
    CompletionFrontierDidNotAdvance {
