...
#[derive(Clone, Copy, Debug, Eq, PartialEq, Serialize)]
pub struct CancelAttachedRequestItem {
    pub request: RequestLease,
...
pub struct ClassLowering {
    ...
    pub write_count: u32,
    pub reserved: u32,
    /// Absolute boundary in this class's physical layout before this append.
    pub previous_layout_boundary: u64,
    /// Absolute boundary in this class's physical layout after this append.
...
const _: [(); 48] = [(); std::mem::size_of::<ClassLowering>()];
const _: [(); 8] = [(); std::mem::align_of::<ClassLowering>()];
const _: [(); 32] = [(); std::mem::offset_of!(ClassLowering, previous_layout_boundary)];
const _: [(); 40] = [(); std::mem::offset_of!(ClassLowering, target_layout_boundary)];
