...
fn same_chunk_relation_synthesizes_resettable_arena() {
    ...
    assert_eq!(
        plan.continuation_blocks(15).unwrap()["chunked"],
        vec![0, 1, 2, 3]
    );
    assert_eq!(plan.capacity_at(15).unwrap()[0].semantic_live_tokens, 15);
    assert!(plan.continuation_blocks(16).unwrap()["chunked"].is_empty());
    assert_eq!(plan.capacity_at(16).unwrap()[0].semantic_live_tokens, 0);
    assert_eq!(plan.continuation_blocks(17).unwrap()["chunked"], vec![4]);
    assert_eq!(plan.capacity_at(17).unwrap()[0].semantic_live_tokens, 1);
