...
use half::bf16;
use oxide_infer::{Bf16SingleDecodeSpec, Bf16SingleDecodeSplitKSpec};
use oxide_infer_cuda::attention::{
    Bf16SingleDecodeArgs, Bf16SingleDecodeSplitKArgs, DecodeProvider,
};
use oxide_infer_cuda::command::{CheckedBindings, CommandQueue};
use serde_json::json;
use std::env;
use std::error::Error;
use std::sync::Arc;
...
impl BenchConfig {
    fn from_env() -> Result<Self, Box<dyn Error>> {
        Ok(Self {
            warmup_calls: env_usize("OXIDE_BENCH_WARMUP", 50)?,
            calls_per_sample: env_usize("OXIDE_BENCH_LAUNCHES", 100)?,
            samples: env_usize("OXIDE_BENCH_SAMPLES", 20)?,
        })
    }
}
...
fn write_record(
    ...
    println!(
        ...
        serde_json::to_string(&json!({
            "schema_version": 1,
            "source_commit": env::var("OXIDE_SOURCE_COMMIT").unwrap_or_else(|_| "unrecorded".into()),
            "source_state": env::var("OXIDE_SOURCE_STATE").unwrap_or_else(|_| "working_tree".into()),
