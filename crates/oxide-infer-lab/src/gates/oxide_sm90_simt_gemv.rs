...
const OXIDE_TENSOR_ALIGNMENT_BYTES: u64 = 4;
...
fn check_plan_metadata(
    ...
    if plan.spec() != expected_spec
        || info.provider() != GemmProviderId::Oxide
        || info.algorithm() != Bf16DenseGemmAlgorithm::OxideSm90SimtGemvM1N16K64
        || info.workspace_required_bytes() != 0
        || plan.workspace_required_bytes() != 0
        || info.tensor_alignment_bytes() != OXIDE_TENSOR_ALIGNMENT_BYTES
        || plan.tensor_alignment_bytes() != OXIDE_TENSOR_ALIGNMENT_BYTES
        || info.workspace_alignment_bytes() != OXIDE_WORKSPACE_ALIGNMENT_BYTES
        || plan.workspace_alignment_bytes() != OXIDE_WORKSPACE_ALIGNMENT_BYTES
        || plan.estimated_waves_count().is_some()
        || planner.workspace_limit_bytes(Bf16DenseGemmSelection::Oxide) != 0
    {
        return Err("Oxide GEMV plan metadata does not match the frozen algorithm contract".into());
    }
    match planner.provider_version(GemmProviderId::Oxide) {
        GemmProviderVersion::Oxide(version) if !version.is_empty() => Ok(()),
        other => Err(format!("Oxide GEMV reported an invalid provider version: {other:?}").into()),
    }
}
...
fn check_eager_reuse(
    ...
    for output in [first_output_handle.write(), second_output_handle.write()] {
        ...
        if completion.submitted() != 1 {
            return Err("Oxide GEMV eager completion covered the wrong command count".into());
        }
    ...
    if !workspace.is_empty() {
        return Err("Oxide GEMV plan used a nonempty caller workspace".into());
    }
    let first_actual = first_output.to_host_vec(&stream)?;
    let second_actual = second_output.to_host_vec(&stream)?;
    let first_comparison = check_guarded_output("first eager Oxide GEMV", &first_actual, expected)?;
    let second_comparison =
        check_guarded_output("second eager Oxide GEMV", &second_actual, expected)?;
    println!(
        ...
        GateCase::new("oxide_sm90_simt_gemv_h20", "eager_shape"),
        spec.m(),
        spec.n(),
        spec.k(),
...
fn check_graph_replay(
    ...
    if !expected.iter().any(|value| value.to_bits() != 0) {
        return Err("Oxide GEMV graph fixture has an all-zero expected output".into());
    }
    ...
    if captured.commands() != 3 {
        return Err("Oxide GEMV graph captured the wrong command count".into());
    }
    drop(plan);
    ...
    for expected_launch in 1..=2 {
        ...
        if completion.launch_index() != expected_launch {
            return Err("Oxide GEMV graph completion reported the wrong replay index".into());
        }
    ...
    if exec.launches() != 2 || exec.commands() != 3 {
        return Err("Oxide GEMV graph accounting changed across replay".into());
    }
    ...
    drop(bindings);
    if !workspace.is_empty() {
        return Err("Oxide GEMV graph used a nonempty caller workspace".into());
    }
    ...
    let zero_comparison = check_guarded_output(
        "replayed Oxide GEMV zero node",
        &zero_actual,
        &zero_expected,
    )?;
    let expected_comparison = check_guarded_output(
        "replayed Oxide GEMV expected node",
        &expected_actual,
        expected,
    )?;
    println!(
        ...
        GateCase::new("oxide_sm90_simt_gemv_h20", "graph_shape"),
        spec.m(),
        spec.n(),
        spec.k(),
...
fn check_general_numerics(
    ...
    let (comparison, max_rel) =
        check_guarded_output_with_tolerance("general-numerics Oxide GEMV", &actual, &expected)?;
    println!(
        ...
        GateCase::new("oxide_sm90_simt_gemv_h20", "general_numerics"),
        spec.m(),
        spec.n(),
        spec.k(),
...
fn check_shape_rejections(planner: &GemmPlanner) -> Result<(), Box<dyn Error>> {
    ...
    for (name, spec) in cases {
        let result = planner.plan_bf16_dense(spec, Bf16DenseGemmSelection::Oxide);
        let matched = match (name, result) {
            ("m", Err(Bf16DenseGemmPlanError::OxideMNotOne { m: 2 }))
            | ("n", Err(Bf16DenseGemmPlanError::OxideNNotMultipleOf16 { n: 15 }))
            | ("k", Err(Bf16DenseGemmPlanError::OxideKNotMultipleOf64 { k: 63 })) => true,
            (_, _) => false,
        };
        if !matched {
            return Err(format!("Oxide GEMV returned the wrong {name} admission result").into());
        }
    }
    println!(
        ...
        GateCase::new("oxide_sm90_simt_gemv_h20", "shape_contract"),
    );
    Ok(())
}
...
fn check_buffer_lengths(
    ...
    println!(
        ...
        GateCase::new("oxide_sm90_simt_gemv_h20", "buffer_lengths"),
    );
    Ok(())
}
...
fn expect_alignment_rejection(
    ...
    match result {
        Err(Bf16DenseGemmEnqueueError::MisalignedBuffer {
            operand,
            alignment: OXIDE_TENSOR_ALIGNMENT_BYTES,
            ..
        }) if operand == expected_operand => {}
        other => {
            return Err(format!(
...
fn check_alignment(
    ...
    println!(
        ...
        GateCase::new("oxide_sm90_simt_gemv_h20", "alignment"),
    );
    Ok(())
}
...
fn check_command_capacity(
    ...
    match plan.enqueue_into(&mut scope, operands) {
        ...
        other => return Err(format!("second Oxide GEMV command returned {other:?}").into()),
    }
    let completion = scope.finish();
    if completion.submitted() != 1 {
    ...
    drop(bindings);
    let actual = output.to_host_vec(stream)?;
    let comparison = check_guarded_output("capacity-admitted Oxide GEMV", &actual, &expected)?;
    println!(
        "{} capacity=1 first_submitted=true second_rejected_before_ffi=true \
         submitted=1 output_sentinels=preserved digest={:016x}",
        GateCase::new("oxide_sm90_simt_gemv_h20", "command_capacity"),
        comparison.digest,
    );
    Ok(())
}
...
pub fn run() -> Result<(), Box<dyn Error>> {
    ...
    let minimum_plan = planner.plan_bf16_dense(minimum_spec, Bf16DenseGemmSelection::Oxide)?;
    check_plan_metadata(&planner, &minimum_plan, minimum_spec)?;
    let GemmProviderVersion::Oxide(provider_version) =
        planner.provider_version(GemmProviderId::Oxide)
    else {
        return Err("Oxide GEMV provider reported the wrong version identity".into());
    };
    println!(
        "{} provider=Oxide provider_version={} \
         algorithm=OxideSm90SimtGemvM1N16K64 workspace_required=0 \
        ...
        GateCase::new("oxide_sm90_simt_gemv_h20", "plan"),
        provider_version,
    );
    ...
    println!(
        "gate=oxide_sm90_simt_gemv_h20 suite=all status=pass \
