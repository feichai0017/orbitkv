//! Unified logging configuration for OrbitKV components.
//!
...
pub struct LoggingConfig {
    /// Log level filter (e.g., "info", "debug", "info,orbitkv_core=debug").
...
/// keep debug output focused on OrbitKV components.
fn init(config: LoggingConfig) {
    INIT.call_once(|| {
        ...
        // using orbitkv-core as a library) typically installed its own global
        // logger before constructing the engine. Losing orbitkv's preferred
        ...
        if let Err(err) = builder.try_apply() {
            eprintln!("orbitkv logging setup skipped (global logger already installed): {err}");
        }
    });
}
