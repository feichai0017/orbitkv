//! SM90a SIMT kernels for the Loom GEMM provider.
...
mod kernels {
    ...
    pub fn loom_sm90_simt_gemv_m1_n16_k64_bf16(
        n: usize,
        k: usize,
        activation: &[bf16],
        weight: &[bf16],
...
impl Sm90Provider {
    ...
    pub(super) fn plan_bf16_dense(
        ...
        let launch =
            self.module
                .prepare_loom_sm90_simt_gemv_m1_n16_k64_bf16(LaunchConfig1D::new(
                    blocks,
                    BLOCK_THREADS,
                    0,
                ))?;
        Ok(LoomBf16DensePlan {
            spec,
...
/// One immutable prepared Loom SM90a GEMV launch.
#[derive(Clone)]
pub(crate) struct LoomBf16DensePlan {
    spec: Bf16DenseGemmSpec,
    module: kernels::LoadedModule,
    launch: PreparedLaunch<kernels::__loom_sm90_simt_gemv_m1_n16_k64_bf16_CudaKernel>,
}

impl LoomBf16DensePlan {
    ...
    pub(crate) fn enqueue_into(
        ...
        let launch_result = {
            ...
            let operation = self.module.loom_sm90_simt_gemv_m1_n16_k64_bf16_async(
                &self.launch,
                self.spec.n(),
                self.spec.k(),
                resolved.first,
...
pub(super) fn validate_spec(spec: Bf16DenseGemmSpec) -> Result<(), Bf16DenseGemmPlanError> {
    if spec.m() != 1 {
        return Err(Bf16DenseGemmPlanError::LoomMNotOne { m: spec.m() });
    }
    if !spec.n().is_multiple_of(N_MULTIPLE) {
        return Err(Bf16DenseGemmPlanError::LoomNNotMultipleOf16 { n: spec.n() });
    }
    if !spec.k().is_multiple_of(K_MULTIPLE) {
        return Err(Bf16DenseGemmPlanError::LoomKNotMultipleOf64 { k: spec.k() });
    }
...
fn checked_grid_blocks(n: usize) -> Result<u32, Bf16DenseGemmPlanError> {
    ...
    u32::try_from(blocks)
        .map_err(|_| Bf16DenseGemmPlanError::LoomGridDimensionOutOfRange { blocks })
}
...
mod tests {
    ...
    fn admission_rejects_non_m1() {
        assert!(matches!(
            ...
            Err(Bf16DenseGemmPlanError::LoomMNotOne { m: 2 })
        ));
    }
    ...
    fn admission_rejects_n_outside_tile_contract() {
        assert!(matches!(
            ...
            Err(Bf16DenseGemmPlanError::LoomNNotMultipleOf16 { n: 17 })
        ));
    }
    ...
    fn admission_rejects_k_outside_tile_contract() {
        assert!(matches!(
            ...
            Err(Bf16DenseGemmPlanError::LoomKNotMultipleOf64 { k: 65 })
        ));
    }
    ...
    fn grid_gate_rejects_more_than_u32_blocks() {
        ...
        assert!(matches!(
            ...
            Err(Bf16DenseGemmPlanError::LoomGridDimensionOutOfRange {
                blocks: actual
            }) if actual == blocks
        ));
    }
