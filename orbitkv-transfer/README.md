...
RDMA-based inter-node memory transfer engine for OrbitKV. Provides one-sided RDMA READ/WRITE operations between nodes with automatic session management, NUMA-aware topology detection, and a Mooncake-compatible API.

## Overview

OrbitKV Transfer enables high-bandwidth, low-latency memory transfers between nodes over InfiniBand/RoCE RDMA fabrics. It handles the full RDMA lifecycle — device discovery, queue pair management, connection establishment, and memory registration — behind a simple synchronous API.
...
```bash
# Build the library
cargo build -p orbitkv-transfer

# Build release
cargo build -p orbitkv-transfer --release

# Run tests
cargo test -p orbitkv-transfer
```
...
```rust
use orbitkv_transfer::{MooncakeTransferEngine, DomainAddress};
...
```bash
cargo run --bin orbitkv_topo_cli
```

Example output:

```
=== OrbitKV System Topology ===
...
cargo run --release --bin orbitkv_cpu_bench
...
# Filter by NUMA node
cargo run --release --bin orbitkv_cpu_bench -- --numa 0
```
...
```bash
# Set the NIC to use
export ORBITKV_TRANSFER_IT_NIC=mlx5_0

# Optional configuration
export ORBITKV_TRANSFER_IT_BASE_PORT=56050   # default: 56050
export ORBITKV_TRANSFER_IT_BYTES=1073741824  # default: 1 GB
export ORBITKV_TRANSFER_IT_WARMUP=1          # default: 1
export ORBITKV_TRANSFER_IT_ITERS=20          # default: 20

# Run (ignored by default — requires hardware)
cargo test -p orbitkv-transfer -- --ignored
```
...
## Integration with OrbitKV

This crate is consumed by the OrbitKV Python bindings (`python/src/transfer.rs`), which expose it as `orbitkv.TransferEngine` for use in vLLM P/D disaggregated serving. The typical flow:
...
3. Workers exchange `DomainAddress` session IDs via the OrbitKV gRPC server
...
- `RUST_LOG`: Control logging level (default: `info,orbitkv_transfer=debug`)

## License

Part of the OrbitKV project. Apache-2.0.
