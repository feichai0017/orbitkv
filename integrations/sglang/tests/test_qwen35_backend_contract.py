...
def _fixed_state_config() -> ManagerPlanConfig:
...
def test_qualification_profile_mutation_cannot_change_gdn_runtime_policy(
    ...
    assert qualification.GDN_FIXED_STATE_BACKEND_PROFILE == runtime_profile
    assert qualification.QWEN35_BACKEND_PROFILE == runtime_profile
    assert qualification.GDN_FIXED_STATE_BACKEND_PROFILE is not runtime_profile
    assert qualification.QWEN35_BACKEND_PROFILE is not runtime_profile
    assert (
        qualification.QWEN35_BACKEND_PROFILE
        is not qualification.GDN_FIXED_STATE_BACKEND_PROFILE
    )
    ...
    assert qualification.GDN_FIXED_STATE_BACKEND_PROFILE[
        "linear_attn_backend"
    ] == "triton"
    monkeypatch.setitem(
        qualification.GDN_FIXED_STATE_BACKEND_PROFILE,
        "linear_attn_backend",
        "flashinfer",
    )
...
def test_gdn_backend_contract_does_not_narrow_non_gdn_profiles():
