...
class VLLMServer:
    """Context manager for vLLM server lifecycle with OrbitKV connector."""
    ...
    def __enter__(self):
        """Start the vLLM server with OrbitKV connector."""
        env = os.environ.copy()
        ...
        print(f"\n[OrbitKV] Starting vLLM server on port {self.port}")
        print(f"[OrbitKV] Torch profiler dir: {self.torch_profile_dir}")

        # Redirect output to log file
        if self.log_file:
            print(f"[OrbitKV] Server log: {self.log_file}")
    ...
    def __exit__(self, exc_type, exc_val, exc_tb):
        ...
        if self.process:
            print("\n[OrbitKV] Stopping vLLM server...")
            self.process.send_signal(signal.SIGTERM)
            try:
                self.process.wait(timeout=10)
            except subprocess.TimeoutExpired:
                print("[OrbitKV] Server didn't stop gracefully, forcing...")
                self.process.kill()
                self.process.wait()
    ...
    def _wait_for_ready(self, timeout: int = 180):
        ...
        print("[OrbitKV] Waiting for server to be ready...")
            ...
            for endpoint in self.health_endpoints:
                ...
                try:
                    ...
                    if response.status_code == 200:
                        print(f"[OrbitKV] Server is ready! (checked {endpoint})\n")
                        time.sleep(2)
                        return
    ...
    def start_profile(self):
        ...
        try:
            ...
            if response.status_code == 200:
                print("[OrbitKV] Torch profiling started")
            else:
                print(f"[OrbitKV] Failed to start profiling: {response.status_code}")
        except requests.exceptions.RequestException as e:
            print(f"[OrbitKV] Failed to start profiling: {e}")
    ...
    def stop_profile(self):
        ...
        try:
            response = requests.post(url, timeout=5)
            if response.status_code == 200:
                print("[OrbitKV] Torch profiling stopped")
            else:
                print(f"[OrbitKV] Failed to stop profiling: {response.status_code}")
        except requests.exceptions.RequestException as e:
            print(f"[OrbitKV] Failed to stop profiling: {e}")
