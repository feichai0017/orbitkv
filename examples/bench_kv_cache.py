#!/usr/bin/env python3
"""
Benchmark script to compare LMCache performance against OrbitKV's KV cache connector.
...
class VLLMServer:
    ...
    def __enter__(self):
        ...
        if self.cross_layer_blocks:
            env["ORBITKV_CROSS_LAYER_BLOCKS"] = "1"
        ...
        if self.use_orbitkv:
            # Add OrbitKV KV connector configuration using JSON format
            kv_config = {
...
def print_comparison(results: dict, args):
    ...
    if args.with_lmcache:
        print("LMCACHE vs ORBITKV KV CACHE BENCHMARK SUMMARY (TTFT FOCUSED)")
    else:
        print("ORBITKV KV CACHE BENCHMARK SUMMARY (TTFT FOCUSED)")
    print("=" * 80)
    ...
    configs.extend(
        [
            ("orbitkv_cold", "OrbitKV (Cold)"),
            ("orbitkv_warm", "OrbitKV (Warm)"),
        ]
    )
...
def main():
    parser = argparse.ArgumentParser(
        description="Benchmark LMCache local CPU vs OrbitKV KV cache connector"
    )
    parser.add_argument(
        "--model",
        type=str,
    ...
    parser.add_argument(
        ...
        type=int,
        default=8001,
        help="Port for OrbitKV vLLM server (default: 8001)",
    )
    parser.add_argument(
        "--tp-size",
        dest="tp_size",
    ...
    parser.add_argument(
        ...
        "By default, only OrbitKV is tested (faster for development).",
    )
    parser.add_argument(
        "--profile",
        action="store_true",
    ...
    parser.add_argument(
        ...
        default=True,
        help="Enable cross-layer block layout for OrbitKV (default: enabled). "
    ...
    print("\n" + "=" * 70)
    benchmark_title = (
        "ORBITKV vs LMCACHE KV CACHE BENCHMARK"
        if args.with_lmcache
        else "ORBITKV KV CACHE BENCHMARK"
    )
    ...
    print(f"OrbitKV Port:   {args.orbitkv_port}")
    ...
    # Phase: OrbitKV vLLM (with KV connector)
    print("\n" + "=" * 70)
    print(f"PHASE {phase_counter}: ORBITKV vLLM (KV Cache Connector Enabled)")
    print("=" * 70)

    orbitkv_log = run_dir / "orbitkv_server.log"
    profile_path = run_dir / "orbitkv" if args.profile else None
    ...
    with VLLMServer(
        ...
        if args.torch_profile:
            ...
            requests.post(f"http://localhost:{args.orbitkv_port}/start_profile")

        result_file = run_dir / "orbitkv_cold.json"
        all_results["orbitkv_cold"] = run_benchmark(
            args.model,
            args.orbitkv_port,
            args.num_prompts,
            args.input_len,
        ...
        if args.torch_profile:
            ...
            requests.post(f"http://localhost:{args.orbitkv_port}/stop_profile")
        ...
        result_file = run_dir / "orbitkv_warm.json"
        all_results["orbitkv_warm"] = run_benchmark(
            args.model,
            args.orbitkv_port,
            args.num_prompts,
            args.input_len,
