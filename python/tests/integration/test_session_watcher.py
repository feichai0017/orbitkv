...
from tests.support.paths import PYTHON_ROOT, TESTS_ROOT

pytestmark = [pytest.mark.integration, pytest.mark.gpu]

HELPER = TESTS_ROOT / "support/session_crash_helper.py"
READY_TIMEOUT_SEC = 30.0
CLEANUP_LOG_DEADLINE_SEC = 5.0


def _worker_env() -> dict[str, str]:
    """Match orbitkv_server fixture's environment so the helper finds the
    same libpython / orbitkv module the server sees."""
    env = os.environ.copy()
...
def test_crashed_client_releases_ipc(channel_server, tmp_path):
    ...
    while time.monotonic() < deadline:
        if target in orbitkv_server.read_logs():
            return
        time.sleep(0.05)

    tail = orbitkv_server.read_logs()[-2000:]
