"""Real vLLM PP4 + OrbitKV P2P correctness probe.

This test is intentionally hardware-gated. It starts:

1. one source OrbitKV server with RDMA enabled and a PP4 producer vLLM,
...
Run on an 8-GPU RDMA node:
    ...
    ORBITKV_NICS=mlx5_... ORBITKV_HOST_IP=172.31.13.39 \
      uv run python -m pytest -m e2e python/tests/e2e/test_vllm_p2p_pp4_e2e.py \
      --model /data/models/Qwen3-8B --orbitkv-pool-size 60gb -s -v
"""
...
class CacheManager(ManagedProcess):
    def __init__(
        ...
        if os.environ.get("ORBITKV_USE_HUGEPAGES", "0") == "1":
            cmd.append("--use-hugepages")

        super().__init__(
            f"orbitkv-{label}",
            cmd,
            log_dir / f"orbitkv-{label}.log",
    ...
    def __init__(
        ...
        kv_config = {
            ...
            "kv_connector_extra_config": {
                ...
                "orbitkv.port": orbitkv_server.grpc_port,
        ...
        env["ORBITKV_PORT"] = str(orbitkv_server.grpc_port)
        ...
        cmd = [
            ...
            os.environ.get(
                "ORBITKV_GPU_MEMORY_UTILIZATION",
        ...
        cmd.extend(shlex.split(os.environ.get("ORBITKV_VLLM_EXTRA_ARGS", "")))
...
def _wait_for_metrics(
    ...
    current = fetch_orbitkv_metrics(metrics_port)
    while time.time() < deadline:
        current = fetch_orbitkv_metrics(metrics_port)
...
def test_pp4_p2p_matches_local_cache_load(
    ...
    nics = os.environ.get("ORBITKV_NICS", "")
    if not nics:
        pytest.skip("ORBITKV_NICS is required for PP4 P2P E2E")

    server_host = os.environ.get("ORBITKV_HOST_IP", "127.0.0.1")
    producer_devices = os.environ.get("ORBITKV_PRODUCER_GPUS", "0,1,2,3")
    consumer_devices = os.environ.get("ORBITKV_CONSUMER_GPUS", "4,5,6,7")
    source_devices = os.environ.get(
        "ORBITKV_SOURCE_SERVER_GPUS",
        f"{producer_devices},{consumer_devices}",
    )
    pool_size = os.environ.get("ORBITKV_POOL_SIZE", orbitkv_pool_size or DEFAULT_POOL_SIZE)
    request_max_tokens = int(os.environ.get("ORBITKV_REQUEST_MAX_TOKENS", "8"))
    server_max_model_len = int(
        os.environ.get("ORBITKV_MAX_MODEL_LEN", max_model_len or DEFAULT_MAX_MODEL_LEN)
    )
    ...
    with ExitStack() as stack:
        ...
        producer_after = _wait_for_metrics(
            ...
            (
                ...
                "orbitkv_save_bytes",
                "orbitkv_cache_block_insertions_total",
                "orbitkv_cache_block_insertions",
            ),
        )
        ...
        with VllmReplica(
            ...
            local_after = _wait_for_metrics(
                ...
                (
                    ...
                    "orbitkv_load_bytes",
                    "orbitkv_cache_block_hits_total",
