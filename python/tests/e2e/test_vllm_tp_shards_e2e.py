"""E2E coverage for one TP replica split across two OrbitKV servers."""
...
@pytest.mark.e2e
@pytest.mark.gpu
def test_vllm_tp_replica_uses_every_orbitkv_shard(
    model: str,
    base_port: int,
    ...
    server_binary = REPO_ROOT / "target/release/orbitkv-cache-manager"
    ...
    with ExitStack() as stack:
        ...
        metrics_before = [fetch_orbitkv_metrics(server.metrics_port) for server in servers]
        ...
        with VLLMServer(
            ...
            server_label="OrbitKV TP-shard cold",
        ):
            cold_outputs, cold_seconds, cold_p50, cold_p95 = _run_traffic(base_port, model)
        metrics_after_save = [fetch_orbitkv_metrics(server.metrics_port) for server in servers]
        ...
        with VLLMServer(
            ...
            tensor_parallel_size=tensor_parallel_size,
            max_model_len=max_model_len,
            server_label="OrbitKV TP-shard warm",
        ):
            warm_outputs, warm_seconds, warm_p50, warm_p95 = _run_traffic(base_port, model)
        metrics_after_load = [fetch_orbitkv_metrics(server.metrics_port) for server in servers]
        ...
        for index, server in enumerate(servers):
            ...
            assert save_activity > 0, f"OrbitKV shard {index} saved no KV blocks"
            assert load_activity > 0, f"OrbitKV shard {index} loaded no KV blocks"
            failures = fetch_orbitkv_rpc_failures(server.metrics_port)
            assert not failures, f"OrbitKV shard {index} RPC failures: {failures}"
