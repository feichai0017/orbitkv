...
This command must not start vLLM, `orbitkv-server`, or any GPU runtime. It still
...
- `test_local_control.py` proves Python-to-sidecar iceoryx2 ping, epoch fencing,
  UDS/memfd bootstrap, local publish, cold and warm `QueryBundle`, local lease
  release, and shutdown across a real process boundary;
- `test_engine_client.py`
- `test_session_watcher.py`

Requirements:
- built Python extension, for example `uv run maturin develop -r`
- a discoverable `orbitkv-server` binary from the installed package or Cargo target
...
This is the main correctness E2E. It starts baseline vLLM and OrbitKV-enabled vLLM with vLLM's neutral generation defaults, compares deterministic completions across one execution plan, and checks that OrbitKV metrics show save/load/hit activity.
...
Minimum checks:
- `orbitkv-server --help`
- minimal installed `orbitkv-server` startup and `/health` 200
