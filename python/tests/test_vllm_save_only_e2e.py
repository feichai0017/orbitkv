"""E2E coverage for OrbitKV save_only behind an external full-hit connector.
...
decode-side connector that claims a full prompt hit. The expected ownership is:
    ...
    MultiConnector child 1: OrbitKVConnector(save_only) -> no read, save only

Then a second vLLM instance with normal OrbitKVConnector should load the blocks
...
def _wait_for_any_metric_delta(
    ...
    *,
    ...
    while time.time() < deadline:
        latest = fetch_orbitkv_metrics(metrics_port)
...
def _save_only_multi_config(orbitkv_port: int) -> dict[str, Any]:
    return {
...
def _orbitkv_read_write_config(orbitkv_port: int) -> dict[str, Any]:
    return {
        "kv_connector": "OrbitKVConnector",
        "kv_role": "kv_both",
        "kv_connector_module_path": "orbitkv.vllm",
...
def test_save_only_external_full_hit_is_saved_for_later_orbitkv_read(
    model: str,
    base_port: int,
    tmp_path: Path,
    max_model_len: int | None,
):
    """save_only must not query OrbitKV, but must save external-hit blocks."""
    ...
    with CacheManager(
        ...
        with VLLMServer(
            ...
            save_metrics_before = fetch_orbitkv_metrics(orbitkv_server.metrics_port)
        ...
        with VLLMServer(
            ...
            read_metrics_before = fetch_orbitkv_metrics(orbitkv_server.metrics_port)
    ...
    save_bytes = _metric_delta(save_metrics_before, save_metrics_after, "orbitkv_save_bytes_total")
    ...
    assert save_bytes > 0 or insertions > 0, (
        f"save_only phase produced no OrbitKV save activity: "
    ...
    assert save_hits == 0 and save_load_bytes == 0, (
        f"save_only phase unexpectedly read from OrbitKV: "
