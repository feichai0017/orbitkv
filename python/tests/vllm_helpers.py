...
class VLLMServer:
    ...
    def __enter__(self):
        ...
        if self.orbitkv_port is not None:
            env["ORBITKV_PORT"] = str(self.orbitkv_port)
...
def check_orbitkv_server(metrics_port: int) -> bool:
    """Check if OrbitKV server is running and metrics endpoint is available."""
    try:
        fetch_orbitkv_metrics(metrics_port)
        return True
    except requests.exceptions.RequestException:
        return False
