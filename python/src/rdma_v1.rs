...
fn rdma_v1_error(context: &str, err: impl std::fmt::Display) -> PyErr {
    OrbitKVError::new_err(format!("{context}: {err}"))
}
...
impl OrbitKVRdmaV1Handshake {
    ...
    fn __repr__(&self) -> String {
        format!(
            "OrbitKVRdmaV1Handshake(status={:?}, has_metadata={})",
...
struct PendingRead {
    receivers: Vec<Option<mea::oneshot::Receiver<orbitkv_transfer::Result<usize>>>>,
    bytes_done: usize,
}
...
#[pyclass]
struct OrbitKVRdmaV1Engine {
    engine: Arc<TransferEngine>,
    state: Mutex<EngineState>,
}

#[pymethods]
impl OrbitKVRdmaV1Engine {
    #[new]
    ...
    fn register_memory(&self, py: Python<'_>, regions: Vec<Py<PyDict>>) -> PyResult<()> {
        // Register local memory regions that OrbitKV RDMA v1 may use as READ
    ...
    fn prepare_handshake(
        ...
        py: Python<'_>,
        remote_addr: String,
    ) -> PyResult<OrbitKVRdmaV1Handshake> {
    ...
    fn accept_handshake<'py>(
        ...
        // Responder-side equivalent of OrbitKVEngine::rdma_accept_handshake. The
    ...
    fn abort_handshake(&self, py: Python<'_>, remote_addr: String) -> PyResult<()> {
        ...
        let local = self
            .state
            .lock()
            .map_err(|_| PyRuntimeError::new_err("OrbitKVRdmaV1Engine state mutex poisoned"))?
            .pending_handshakes
            .remove(&remote_addr);
    ...
    fn read_async_indices(
        ...
        let prepared = {
            ...
            for (local_idx, remote_idx) in local_desc_ids.into_iter().zip(remote_desc_ids) {
                ...
                // descriptors for the OrbitKV v1 transfer engine.
    ...
    fn check_read(&self, handle: u64) -> PyResult<String> {
        ...
        for slot in &mut read.receivers {
            ...
            match rx.try_recv() {
                ...
                Err(mea::oneshot::TryRecvError::Disconnected) => {
                    return Err(OrbitKVError::new_err("RDMA READ completion channel closed"));
                }
            }
        }
        ...
        log::debug!("[OrbitKVRdmaV1Engine] RDMA READ done handle={handle} bytes={total}");
        Ok("done".to_string())
    }
...
impl OrbitKVRdmaV1Engine {
    ...
    fn prepare_handshake_inner(
        ...
        Ok(match status {
            ConnectionStatus::Existing => OrbitKVRdmaV1Handshake {
                status: "existing".to_string(),
                has_metadata: false,
                metadata: None,
            },
            ConnectionStatus::Connecting => OrbitKVRdmaV1Handshake {
            ...
            ConnectionStatus::Prepared(metadata) => {
                self.state
                    .lock()
                    .map_err(|_| {
                        PyRuntimeError::new_err("OrbitKVRdmaV1Engine state mutex poisoned")
                    })?
                    .pending_handshakes
                    .insert(remote_addr.to_string(), metadata.clone());
                OrbitKVRdmaV1Handshake {
    ...
    fn accept_handshake_inner(
        ...
        // Match the server-side OrbitKVEngine RDMA handshake behavior: a peer
...
pub(crate) fn add_classes(m: &Bound<'_, PyModule>) -> PyResult<()> {
    m.add_class::<OrbitKVRdmaV1Engine>()?;
    m.add_class::<OrbitKVRdmaV1Handshake>()?;
    Ok(())
}
