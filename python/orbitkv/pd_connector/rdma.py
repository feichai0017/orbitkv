...
def build_rdma_port(
    ...
    *,
    ...
    enabled = _extra(config, "orbitkv.pd.rdma.enabled", _MISSING)
    if enabled is not _MISSING and not _as_bool(enabled):
        raise RuntimeError("PdConnector requires native RDMA; orbitkv.pd.rdma.enabled=false")

    try:
        from orbitkv.orbitkv import PdRdmaEngine
    except ImportError as exc:
        raise RuntimeError("PdConnector requires native RDMA extension orbitkv.orbitkv") from exc
    except AttributeError as exc:
        raise RuntimeError("orbitkv.orbitkv does not expose PdRdmaEngine") from exc

    _reject_legacy_rank_config(config)
    device = _extra(config, "orbitkv.pd.rdma.device", "cuda")
...
def _reject_legacy_rank_config(config: Any) -> None:
    legacy_keys = (
        "orbitkv.pd.rdma.cuda_device",
        "orbitkv.pd.rdma.numa_node",
        "orbitkv.pd.rdma.domains",
        "orbitkv.pd.rdma.pin_worker_cpu",
    )
    present = [key for key in legacy_keys if _extra(config, key, _MISSING) is not _MISSING]
    if present:
        raise RuntimeError(
            "PdConnector RDMA requires orbitkv.pd.rdma.rank_map; remove legacy keys: "
            + ", ".join(present)
        )
...
def _rank_rdma_config(
    ...
    *,
    ...
    rank_map = _extra(config, "orbitkv.pd.rdma.rank_map", _MISSING)
    if not isinstance(rank_map, dict):
        raise RuntimeError("PdConnector RDMA requires orbitkv.pd.rdma.rank_map")
...
def _validate_rank_map_cpus(config: Any, rank_map: dict[Any, Any]) -> None:
    reserved_floor = int(_extra(config, "orbitkv.pd.rdma.reserved_cpu_floor", 16))
    seen: dict[int, str] = {}
