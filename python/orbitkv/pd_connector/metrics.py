...
from orbitkv.connector.connector_metrics import build_buckets
...
class PdPromMetrics(KVConnectorPromMetrics):
    def __init__(
        ...
        self.counter_load_failure = self._counter("vllm:orbitkv_pd_load_failure_total", labelnames)
        self.counter_prefill_push_success = self._counter(
            "vllm:orbitkv_pd_prefill_push_success_total", labelnames
        )
        self.counter_prefill_push_failure = self._counter(
            "vllm:orbitkv_pd_prefill_push_failure_total", labelnames
        )
        self.counter_decode_abort = self._counter("vllm:orbitkv_pd_decode_abort_total", labelnames)
        self.counter_prefill_release = self._counter(
            "vllm:orbitkv_pd_prefill_release_total", labelnames
        )
        self.counter_prefill_skipped_push = self._counter(
            "vllm:orbitkv_pd_prefill_skipped_push_total", labelnames
        )
