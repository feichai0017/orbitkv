...
def render_proxy_metrics(config: ProxyConfig) -> bytes:
    ...
    lines = [
        "# TYPE orbitkv_pd_proxy_requests_total counter",
        f"orbitkv_pd_proxy_requests_total {metrics['request_count']}",
        "# TYPE orbitkv_pd_proxy_stream_requests_total counter",
        f"orbitkv_pd_proxy_stream_requests_total {metrics['stream_request_count']}",
        "# TYPE orbitkv_pd_proxy_inflight_requests gauge",
        f"orbitkv_pd_proxy_inflight_requests {metrics['inflight_requests']}",
        "# TYPE orbitkv_pd_proxy_errors_total counter",
        f"orbitkv_pd_proxy_errors_total {metrics['error_count']}",
        "# TYPE orbitkv_pd_proxy_stream_chunks_total counter",
        f"orbitkv_pd_proxy_stream_chunks_total {metrics['stream_chunks']}",
        "# TYPE orbitkv_pd_proxy_stream_bytes_total counter",
        f"orbitkv_pd_proxy_stream_bytes_total {metrics['stream_bytes']}",
    ]
    _extend_summary(
        lines,
        "orbitkv_pd_proxy_decode_request_duration_seconds",
        metrics["decode_request_durations_s"],
    )
    _extend_summary(
        lines,
        "orbitkv_pd_proxy_first_chunk_duration_seconds",
        metrics["first_chunk_durations_s"],
    )
    _extend_summary(
        lines,
        "orbitkv_pd_proxy_stream_duration_seconds",
    ...
    lines.append("# TYPE orbitkv_pd_proxy_route_total counter")
    for (prefill_instance, decode_instance), count in sorted(route_counts.items()):
        lines.append(
            "orbitkv_pd_proxy_route_total{"
...
def main(argv: list[str] | None = None) -> None:
    parser = argparse.ArgumentParser(description="OrbitKV P/D local proxy")
