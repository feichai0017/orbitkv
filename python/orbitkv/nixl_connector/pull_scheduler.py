...
class OrbitKVNixlPullConnectorScheduler(NixlPullConnectorScheduler):
    """Fold OrbitKV RDMA v1 accept metadata into NIXL's GET_META response."""
    ...
    def __init__(
        ...
        self._orbitkv_rdma_config = OrbitKVRdmaV1Config.from_extra_config(extra_config)
        self._orbitkv_accept_broker_endpoint = make_accept_broker_endpoint_from_config(
            self.engine_id,
            self.vllm_config,
        )
        self._orbitkv_accept_broker_stop = threading.Event()
        self._orbitkv_accept_broker_thread: threading.Thread | None = None

    def set_xfer_handshake_metadata(self, metadata):
        """Advertise one scheduler-owned broker endpoint to all remote workers."""
        self._start_orbitkv_rdma_accept_broker()
        for payload in metadata.values():
            if not isinstance(payload, NixlHandshakePayload):
                continue
            extensions = dict(payload.extensions or {})
            extensions[ORBITKV_RDMA_V1_ACCEPT_ENDPOINT] = self._orbitkv_accept_broker_endpoint
            payload.extensions = extensions
        super().set_xfer_handshake_metadata(metadata)

    def _start_orbitkv_rdma_accept_broker(self) -> None:
        """Start the scheduler-owned ROUTER that workers register with."""
        if self._orbitkv_accept_broker_thread is not None:
            return
        if self._orbitkv_accept_broker_endpoint.startswith("ipc://"):
            with suppress(FileNotFoundError):
                os.unlink(self._orbitkv_accept_broker_endpoint[len("ipc://") :])
        ready_event = threading.Event()
        thread = threading.Thread(
            target=self._orbitkv_rdma_accept_broker_loop,
            args=(
                self._orbitkv_accept_broker_endpoint,
                ready_event,
                self._orbitkv_accept_broker_stop,
            ),
            daemon=True,
            name=f"orbitkv-rdma-v1-accept-broker-{self.engine_id}",
        )
        thread.start()
        if not ready_event.wait(timeout=self._orbitkv_rdma_config.handshake_timeout_s):
            self._orbitkv_accept_broker_stop.set()
            raise RuntimeError(
                f"OrbitKV RDMA v1 accept broker failed to bind " f"{self._orbitkv_accept_broker_endpoint}"
            )
        self._orbitkv_accept_broker_thread = thread

    def _orbitkv_rdma_accept_broker_loop(
        self,
        endpoint: str,
        ready_event: threading.Event,
        ...
        state = OrbitKVRdmaV1BrokerState(self._orbitkv_rdma_config.handshake_timeout_s)
            ...
            while not stop_event.is_set():
                ...
                for client_reply_prefix, request_id in state.pop_timed_out(now):
                    response = {
                        ...
                        "ok": False,
                        "error": "OrbitKV RDMA v1 worker accept timed out",
                    }
                ...
                try:
                    ...
                    logger.debug("OrbitKV RDMA v1 broker got malformed frames: %s", frames)
                    continue
                try:
                    ...
                    if kind == ORBITKV_RDMA_V1_ACCEPT_REGISTER:
                        ...
                        if client_reply_prefix is None:
                            logger.warning(
                                "OrbitKV RDMA v1 broker got response for unknown request %s",
                                request_id,
                            )
                            continue
                        sock.send_multipart((*client_reply_prefix, msgspec.msgpack.encode(payload)))
                    elif kind == ORBITKV_RDMA_V1_ACCEPT_REQUEST:
                    ...
                    logger.debug("OrbitKV RDMA v1 broker request failed", exc_info=True)
    ...
    def _handle_handshake_extensions(
        ...
        rdma_request = request_extensions.get(ORBITKV_RDMA_V1_EXTENSION)
        if rdma_request is None:
            return payload
        if not isinstance(rdma_request, dict):
            raise ValueError("OrbitKV RDMA v1 request extension must be a dict")
        ...
        if not isinstance(peer_key, str) or not isinstance(metadata, bytes):
            raise ValueError("OrbitKV RDMA v1 request extension missing peer_key/metadata")

        endpoint = (payload.extensions or {}).get(ORBITKV_RDMA_V1_ACCEPT_ENDPOINT)
        if not isinstance(endpoint, str):
            raise RuntimeError(
                "OrbitKV RDMA v1 accept endpoint missing from target rank handshake payload"
            )

        timeout_ms = int(self._orbitkv_rdma_config.handshake_timeout_s * 1000)
        ...
        try:
            ...
            response_extensions[ORBITKV_RDMA_V1_EXTENSION] = {
                "metadata": response_metadata,
            }
        except Exception as exc:
            logger.warning(
                "Failed to accept OrbitKV RDMA v1 handshake target_rank=%s peer=%s: %s",
                target_tp_rank,
                peer_key,
                exc,
            )
        ...
        logger.debug(
            "Accepted OrbitKV RDMA v1 handshake target_rank=%s peer=%s",
            target_tp_rank,
            peer_key,
        )
    ...
    def shutdown(self):
        """Stop the OrbitKV RDMA broker before the base scheduler listener exits."""
        self._orbitkv_accept_broker_stop.set()
        if self._orbitkv_accept_broker_thread is not None:
            self._orbitkv_accept_broker_thread.join(timeout=2.0)
            self._orbitkv_accept_broker_thread = None
        if self._orbitkv_accept_broker_endpoint.startswith("ipc://"):
            with suppress(FileNotFoundError):
                os.unlink(self._orbitkv_accept_broker_endpoint[len("ipc://") :])
        super().shutdown()
