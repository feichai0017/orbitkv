...
"""OrbitKV RDMA v1 helpers for the NIXL pull connector."""
...
from orbitkv.nixl_connector.utils import zmq_ctx
from orbitkv.orbitkv import OrbitKVRdmaV1Engine

if TYPE_CHECKING:
    from vllm.config import VllmConfig


ORBITKV_RDMA_V1_EXTENSION = "orbitkv_rdma_v1"
ORBITKV_RDMA_V1_ACCEPT_ENDPOINT = "orbitkv_rdma_v1_accept_endpoint"
ORBITKV_RDMA_V1_ACCEPT_REGISTER = "register"
ORBITKV_RDMA_V1_ACCEPT_REQUEST = "accept"
ORBITKV_RDMA_V1_ACCEPT_RESPONSE = "accept_response"
ORBITKV_RDMA_V1_ACCEPT_ACK = "registered"


@dataclass(frozen=True)
class OrbitKVRdmaV1Config:
    nics: list[str]
    qps_per_peer: int
    ...
    def from_extra_config(
        ...
        *,
        ...
        """Parse OrbitKV RDMA v1 settings from vLLM's connector extra config."""
        nics = _parse_rank_nics(
            extra_config.get("orbitkv.nixl.rdma_v1.nics_by_rank"),
            tp_rank,
        )
        if nics is None:
            raw_nics = extra_config.get("orbitkv.nixl.rdma_v1.nics")
            if raw_nics is None:
                raw_nics = extra_config.get("orbitkv.pd.rdma.nics")
            nics = _parse_nics(raw_nics)
        if not nics:
            raise ValueError(
                "OrbitKVNixlPullConnector requires kv_connector_extra_config "
                "'orbitkv.nixl.rdma_v1.nics' or "
                "'orbitkv.nixl.rdma_v1.nics_by_rank' with non-bond RDMA NIC names"
            )

        qps_per_peer = int(extra_config.get("orbitkv.nixl.rdma_v1.qps_per_peer", 4))
        if qps_per_peer <= 0:
            raise ValueError("orbitkv.nixl.rdma_v1.qps_per_peer must be positive")

        handshake_timeout_s = float(
            extra_config.get("orbitkv.nixl.rdma_v1.handshake_timeout_s", 5.0)
        )
        if handshake_timeout_s <= 0:
            raise ValueError("orbitkv.nixl.rdma_v1.handshake_timeout_s must be positive")

        perf_enabled = _config_bool(
            extra_config.get("orbitkv.nixl.rdma_v1.perf"),
            default=os.getenv("ORBITKV_NIXL_RDMA_V1_PERF", "").lower() in {"1", "true", "yes"},
        )
        perf_log_every = int(
            extra_config.get(
                "orbitkv.nixl.rdma_v1.perf_log_every",
                os.getenv("ORBITKV_NIXL_RDMA_V1_PERF_LOG_EVERY", 64),
            )
        )
        if perf_log_every <= 0:
            raise ValueError("orbitkv.nixl.rdma_v1.perf_log_every must be positive")
        return cls(
            nics=nics,
...
@dataclass(frozen=True)
class OrbitKVRdmaV1BrokerRequest:
...
class OrbitKVRdmaV1BrokerState:
    ...
    def __init__(self, timeout_s: float):
        ...
        self._pending: dict[int, OrbitKVRdmaV1BrokerRequest] = {}
    ...
    def add_request(
        ...
        *,
        ...
        self._pending[request_id] = OrbitKVRdmaV1BrokerRequest(
...
class OrbitKVRdmaV1Perf:
    ...
    def log_summary(self) -> None:
        ...
        logging.getLogger(self.logger_name).warning(
            "OrbitKV RDMA v1 perf summary %s", "; ".join(parts)
        )
    ...
    def log_final_summary(self) -> None:
        ...
        logging.getLogger(self.logger_name).warning("OrbitKV RDMA v1 perf final summary follows")
        self.log_summary()
...
def _parse_rank_nics(value: Any, tp_rank: int | None) -> list[str] | None:
    ...
    if tp_rank is None:
        if isinstance(value, dict):
            ...
            raise ValueError("orbitkv.nixl.rdma_v1.nics_by_rank must be a dict or list")
        nics: list[str] = []
    ...
    if not nics:
        raise ValueError(
            "orbitkv.nixl.rdma_v1.nics_by_rank missing non-empty NIC list "
...
def peer_key_source(peer_key: str) -> tuple[str, int]:
    """Return the source endpoint of a directional OrbitKV RDMA peer key."""
...
def worker_identity(engine_id: str, tp_rank: int) -> bytes:
    """Return the stable broker identity for one OrbitKV RDMA worker rank."""
    return f"{engine_id}:{tp_rank}".encode()


def make_accept_broker_endpoint(engine_id: str, side_channel_port: int) -> str:
    """Build the scheduler-owned local IPC endpoint for OrbitKV RDMA accepts."""
    base_path = Path(envs.VLLM_RPC_BASE_PATH)
    base_path.mkdir(parents=True, exist_ok=True)
    filename = f"orbitkv-nixl-rdma-v1-{engine_id}-{side_channel_port}.sock"
    return f"ipc://{base_path / filename}"


def make_accept_broker_endpoint_from_config(engine_id: str, vllm_config: VllmConfig) -> str:
    """Build the OrbitKV RDMA accept broker endpoint using vLLM's NIXL port rule."""
    side_channel_port = (
...
def make_accept_control_endpoint(engine_id: str, tp_rank: int, token: int) -> str:
    ...
    return f"inproc://orbitkv-nixl-rdma-v1-accept-stop-{engine_id}-{tp_rank}-{token}"
...
def build_memory_regions(blocks_data: list[tuple[int, int, int]]) -> list[dict[str, int]]:
    ...
    block entries.  OrbitKV RDMA v1 only needs each base address registered once,
...
def parse_handshake_response_extension(
    ...
    if response_extensions is None:
        return None
    payload = response_extensions.get(ORBITKV_RDMA_V1_EXTENSION)
    if payload is None:
        return None
    if not isinstance(payload, dict):
        raise ValueError("OrbitKV RDMA v1 response extension must be a dict")
    error = payload.get("error")
    if error is not None:
        raise RuntimeError(f"OrbitKV RDMA v1 handshake failed: {error}")
    metadata = payload.get("metadata")
    if not isinstance(metadata, bytes):
        raise ValueError("OrbitKV RDMA v1 response extension missing metadata bytes")
    return metadata
...
def accept_handshake_via_zmq(
    ...
    request = msgspec.msgpack.encode(
        {
            "kind": ORBITKV_RDMA_V1_ACCEPT_REQUEST,
            "target_tp_rank": target_tp_rank,
            "peer_key": peer_key,
            "metadata": metadata,
        }
    ...
    if not isinstance(response, dict):
        raise ValueError("OrbitKV RDMA v1 accept response must be a dict")
    if response.get("kind") != ORBITKV_RDMA_V1_ACCEPT_RESPONSE:
        raise ValueError(f"OrbitKV RDMA v1 accept got unexpected response {response.get('kind')!r}")
    if not response.get("ok"):
        error = response.get("error")
        raise RuntimeError(f"OrbitKV RDMA v1 accept failed: {error}")
    response_metadata = response.get("metadata")
    if not isinstance(response_metadata, bytes):
        raise ValueError("OrbitKV RDMA v1 accept response missing metadata bytes")
    return response_metadata


def create_rdma_engine(config: OrbitKVRdmaV1Config) -> OrbitKVRdmaV1Engine:
    """Create the native OrbitKV RDMA v1 engine from parsed connector config."""
    return OrbitKVRdmaV1Engine(nics=config.nics, qps_per_peer=config.qps_per_peer)
