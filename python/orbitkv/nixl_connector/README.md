...
Files changed after import carry a `Modified by OrbitKV contributors` notice.
This directory is expected to diverge from upstream as OrbitKV integrates its
own RDMA transport while keeping the vLLM connector provenance auditable.

## License Notes

vLLM is licensed under Apache License 2.0. OrbitKV is also licensed under
Apache License 2.0, so copying and modifying this connector is generally
compatible with OrbitKV's open-source distribution and internal production
...
- Mark modified files clearly when OrbitKV changes the imported connector.
