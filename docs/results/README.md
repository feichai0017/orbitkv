...
| --- | --- | --- |
| Current ABI12 source and native-wheel boundary | [all-local MoE movement](h20-moe-movement-20260801.json) · [expert-parallel movement](h20-moe-movement-ep-20260801.json) · [vLLM engine admission](h20-vllm-engine-moe-movement-20260801.json) · [clean-install H20 gate](h20-native-wheel-clean-install-abi12-20260801.json) | Revision `d4c13e2` packages all 21 semantic operators and exactly two Loom `.so` files. Source-built and fresh repository-free wheel environments each pass 359 tests with both vLLM minors plus 245 applicable tests on PyTorch 2.10. Direct movement and synthetic engine admission pass while grouped GEMM remains unchanged; the `1.0205x` synthetic ratio is not a production-model speedup claim. The wheel is qualified but not published. |
| Historical ABI11 cross-matrix wheel | [pre-MoE clean-install H20 gate](h20-native-wheel-clean-install-abi11-20260801.json) | Revision `afc54c4` packages the nineteen-operator predecessor and exactly two Loom `.so` files, then passes 342 tests with each vLLM minor plus 231 applicable tests on PyTorch 2.10. |
...
| Optional-residual RMSNorm→INT8 | [ABI10 source and Qwen W8A8 admission](h20-vllm-int8-quant-admission-20260729.json) | Rust/CUDA/PyTorch plus the real compiler route are implemented, Cutlass GEMM is unchanged, and the real-layer shadow has one one-LSB INT8 difference across 688,128 elements with exact scales/residuals. The 32-prompt gate matches `29/32` top-1 tokens and dual-order latency crosses parity, so the route remains explicit opt-in with no default or speedup claim; distribution is qualified in the current ABI12 wheel |
...
| SiLU-and-Mul→dynamic INT8 | [ABI11 source and Qwen W8A8 admission](h20-vllm-silu-int8-admission-20260801.json) · [ABI11 wheel](h20-native-wheel-clean-install-abi11-20260801.json) | Exact compiled-boundary outputs/scales, unchanged Cutlass GEMM, real Loom path hits, exact 32-prompt quality, and repository-free distribution pass. CUDA Graph ratios are `0.518-0.986x` and engine latency is not order-stable, so the route is explicit-only with no speedup/default claim |
...
| Synthetic vLLM engine admission | [Qwen2-MoE Cutlass gate](h20-vllm-engine-moe-movement-20260801.json) | Exact tokens and `48/48` movement hits with caller-owned outputs and unchanged vendor grouped GEMM; the random tiny checkpoint proves admission, not production value. |
| ABI12 binary distribution | [fresh cross-matrix wheel gate](h20-native-wheel-clean-install-abi12-20260801.json) | The same two-library wheel passes 359 tests on each vLLM minor plus 245 applicable tests on PyTorch 2.10 without a repository checkout. It remains unpublished. |

The direct artifacts qualify the permutation/combine boundary. The separate
synthetic engine artifact proves vLLM path hits and an unchanged vendor grouped
GEMM call site, while the ABI12 wheel closes distribution. None proves a
production-model or serving-latency improvement.
