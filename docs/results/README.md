...
| --- | --- | --- |
| Native Python ABI6 matrix wheel | [current clean-install H20 gate](h20-native-wheel-clean-install-abi6-20260727.json) | One exact `py3-none-linux_x86_64` wheel contains the two Loom `.so` files and passes 277 tests with each vLLM minor plus 186 applicable tests on PyTorch 2.10. It is qualified but not published. |
