...
| Boundary | Result set | Current conclusion |
| --- | --- | --- |
| Seeded categorical-sampling admission | [Source-pinned vLLM 0.24 H20 gate](h20-vllm-seeded-sampling-admission-20260727.json) | ABI8-A is admitted for implementation. At 8 rows, all-seeded sampling-only is `3.00–3.01x` the unseeded control with 10 versus 3 kernels and `4.87 MB` peak incremental storage; the isolated 32-row result is `4.82x`, 34 kernels, and `19.45 MB`. No Loom implementation or performance claim exists yet. |
