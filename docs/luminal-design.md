# Luminal and the CUDA Lite backend

This describes the checked-in fork, not an upstream feature promise.
`luminal_cuda_lite` is OrbitKV's active CUDA compiler/runtime backend. The name
identifies the crate; it does not describe a separate small inference engine or
a guarantee that compilation is cheap. There is no second `luminal_cuda` crate
in this checkout.
...
| Luminal core | Tensor graph, symbolic shapes/strides, HLIR, e-graph construction and saturation, reusable search/extraction utilities | `third_party/luminal/src/{graph,shape,hlir,search}` |
| CUDA Lite | Legal CUDA implementations, code generation, candidate compilation/device measurement, resource planning, artifacts and execution | `third_party/luminal/crates/luminal_cuda_lite/src/` |
...
This inference fork retains four packages: core, NN, CUDA Lite and tracing.
Training, Metal, Python bridge and standalone application/benchmark crates are
removed. NN supplies graph helpers and the attention/FP8 linear semantic APIs;
its contracts have no CUDA dependency. The [checkpoint import boundary](checkpoint-import.md)
...
flowchart TD
    ...
    B --> C[Luminal core: saturated e-graph for each shape bucket]
    C --> D[CUDA Lite: extract legal candidate programs]
...
Core ends at construction of the search space. CUDA Lite's `search.rs` drives
