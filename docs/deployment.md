...
handles P-to-D transfer; OrbitKV uses `read_write` on P and `save_only` on D
to reuse KV produced by earlier requests without competing with NIXL loads.

Run one OrbitKV server beside each vLLM instance, backed by the same metaserver,
...
connectors at a single shared OrbitKV server — each vLLM instance registers
...
- `vllm:orbitkv_load_failure_total`, `vllm:orbitkv_save_failure_total`, and non-OK
  OrbitKV RPC deltas stay at zero.
