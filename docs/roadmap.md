...
| Storage representations | Optional bounded Rust LZ4 SSD encoding, checksum-checked decode and raw fallback. Engine KV dtype remains separate; SGLang external scale files enter cache identity. | Qualify FP8 KV and compression per model/engine; measure size and latency before default enablement. GPU codecs and lossy storage formats remain later work. See [storage formats](storage-formats.md). |
