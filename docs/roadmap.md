...
| --- | --- | --- |
| Automatic SSD selection | Native cuFile initialization with io_uring fallback, restore leases, complete-group GPU writes and bounded staging. | Capability selection is the default; native performance and workload-based tuning require matched measurements. Direct engine-page I/O, multi-writer GPU assembly and read priority remain future work. See [GPU storage](gds.md). |
