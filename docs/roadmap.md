...
| Storage representations | GPU ANS lossless, FP8 and 3/4-bit TurboQuant, with encoded DRAM/SSD/peer payloads, checksums and raw fallback. Storage precision and SGLang external scale files enter cache identity. | Qualify model quality and end-to-end latency before enablement; direct compressed GDS and broader workload validation remain open. See [storage formats](storage-formats.md). |
