...
while FlashInfer remains `1.081x` lower-latency on mixed MQA.

The first long-context revision uses sixteen-warp token partitioning for MQA
and eight warps for GQA4. Both pass H20 correctness and sanitizer gates. The
matched eager result puts FlashInfer `1.430x` lower-latency on long MQA and
`11.233x` lower-latency on long GQA4, so paged tensor-core query tiling and
`cp.async` K/V staging are the next optimization milestone.

The direct GQA4 command passes fixed-address Graph correctness after two
replays and external owner teardown.
...
this shape. Token-parallel Graph coverage, mutable metadata, graph updates, and
real engine invocation remain open.
