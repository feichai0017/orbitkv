...
| --- | --- | --- |
| Opt-in SSD acceleration | cuFile restore leases, complete-group GPU writes, bounded staging and independent storage workers. | CPU compatibility correctness and native GDS qualification are separate gates. Keep io_uring as default until a matched native-NVMe comparison shows serving benefit. Multi-writer GPU assembly remains future work. See [GPU storage recovery](gds.md). |
