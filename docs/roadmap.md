...
| Storage representations | Experimental bounded Rust FP8 SSD quantization, checksum-checked reconstruction and raw fallback. Storage precision and SGLang external scale files enter cache identity. | Qwen3-8B encoded payloads halve in both engines; some vLLM text changes. Qualify model quality and latency before enablement. GPU codecs and lower-bit storage formats remain later work. See [storage formats](storage-formats.md). |
