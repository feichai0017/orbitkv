...
现有 P/D（见 [`pd.md`](./pd.md)）走的是 "P save→OrbitKVEngine→D load" 的 CPU 中转路径：

```
P GPU ──> Pinned CPU (OrbitKVEngine) ──> D GPU
```
...
## 3. RDMA 链路接管（orbitkv-transfer v2，verbs only）

### 3.1 v2 路线：常驻，verbs only，沿用 sideway/mummy 栈

`orbitkv-transfer` 有两条独立链路：
...
vLLM NIXL connector 的可参考部分主要是模块边界，而不是传输方向。`pd_connector` 按 NIXL 的文件组织和 hook 分层新写，走全新 module path `orbitkv.pd_connector`。
...
**显式**写在 `orbitkv.pd.rdma.rank_map` 里——不再依赖自动拓扑探测做隐式选择。
...
3. CPU 不允许在 `{0..15}`（默认 floor=16，`orbitkv.pd.rdma.reserved_cpu_floor` 可覆盖）。
...
当前 `orbitkv-router.rs` **尚未实现** PD 路由。开发阶段用 `pd_connector.proxy` 手动构造
...
# Proxy
cd python && uv run python -m orbitkv.pd_connector.proxy \
...
| **P1** connector skeleton | `orbitkv.pd_connector`；vLLM hook、状态机、metadata、layout、chunk tracker | ✅ MockRdmaPort 单测通过 |
