...
| [LMCache MP serialization](https://docs.lmcache.ai/mp/serde.html) and [FlexKV compression](https://github.com/taco-project/FlexKV/tree/738ddc141a198b4e20de6c5d1f0128e387f7fdb2/flexkv/transfer/compression) | Separate engine precision from cache encoding; bound codec workspace and qualify formats | `codec/` owns GPU ANS, FP8, TurboQuant kernels, CPU SIMD fallback and representation validation; `transfer/worker/codec` owns engine-page and workspace lifetimes. Encoded DRAM, SSD and Mooncake payloads share versioned metadata. Compressed GDS and broader model-quality qualification remain open. |
