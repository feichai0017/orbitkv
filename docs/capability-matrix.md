...
| Versioned runtime target binding | P2b static admission | A strict target contract and complete execution signature bind one unchanged manifest fingerprint to one exact executor-contract fingerprint. Admission checks topology, not the lossy capability-name set, and rejects unsupported storage, class order, address/retirement pairing, head/domain partition, layer cover, or fixed-state composition before SGLang pool allocation. Current closed shapes are whole-domain Full token KV, pure SWA, Full+SWA, Full MLA, Full+Mamba, and exact Full+GDN+convolution. Legacy loading remains compatible but carries no binding claim | `src/executor_capabilities.rs`, `bind-runtime-manifest`, `check-runtime-manifest`, packaged `executor_capabilities.v1.json`, cross-language binding and pre-allocation sentinel tests |
...
| Engine-neutral adapter SPI and structured engine bridge | L2 host/package | `orbitkv-runtime` defines the typed data-plane contract; `orbitkv-reference` implements flat external CPU/CUDA tensor arenas; the opt-in SGLang bridge maps per-layer K/V tensors to aggregate neutral token records and uses two-phase external append with distinct data-ready/last-use evidence | The `structured-data-plane` extra plus `ORBITKV_STRUCTURED_DATA_PLANE=1` enables host-tested eager non-overlap BF16/NHD `token_kv` Full, pure-SWA, and Full+SWA profiles with relocation disabled. MLA, relocation, graphs, overlap, distributed modes, performance, and hardware qualification remain outside this bridge |
| SGLang whole-domain pure-SWA executor | P2a host | One manifest-derived periodic class covering every model layer executes through the ABI8 periodic manager, request-private cache seam, pure-SWA allocator, SWA-leaf COW, semantic-retirement cleanup, and optional structured external append. Static layer/window/storage drift fails before native arena creation; dynamic tensor checks remain after allocation | Rust-manifest-to-Python-to-native lifecycle test plus SGLang generic all-SWA config, geometry, allocator, COW, cleanup, cache, and structured-arena tests. No released-checkpoint, GPU E2E, throughput, capacity, or general P2 claim |
