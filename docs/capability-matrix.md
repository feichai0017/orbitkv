...
The [model target matrix](model-targets.md) tracks Qwen, GLM, Kimi and DeepSeek
under a one-H20 budget with quantization and CPU offload allowed. Those planned
architectures are separate from the executable capabilities recorded here.
...
| Bucketed model runtime | Bounded L4 correctness for token KV and Qwen recurrent/convolution state | Token-KV and stateful graphs are searched once into decode/prefill executables. Dynamic inputs are preallocated, request segmentation is shared across attention and recurrent operators, and every state class has a stable arena |
| Decoder schedule artifact | L3 + narrow L4 correctness | Schema 11 persists selected decode/prefill schedules, explicit request/provider geometry and required generated CUDA module images; strict manifest/model/arena/bucket identity, LLIR fingerprints, required persistent-state aliases, and RMSNorm layout legality fail closed; older formats require regeneration |
...
| State preflight | L2 + bounded device regression | Required output/input storage aliases are validated before source generation and compilation on search, finalist and load paths; compiled bucket checks remain. Invalid loads preserve the current executable. State-compatible genome construction remains open |
...
geometries, not a parser restriction. Current schema 11 separates
...
Persistent K/V state now uses a required-alias contract. OrbitKV compiler rejects a
...
| Full latent KV | Host-tested component-aware core lifecycle | Rejected until a matching OrbitKV compiler kernel contract exists | Unqualified |
| Recurrent checkpoints | Host-tested pool and joint RuntimeSession transaction | Geometry/compiler facts plus atomic prepare, submit, completion, abort, release, and reuse with token KV | Ragged packed operator, stable-arena transitions and bounded Qwen3.8-27B-FP8 text execution pass on H20 |
| Convolution state | Host-tested pool and joint RuntimeSession transaction | Minimal-history geometry plus the same atomic token-KV/fixed-state lifecycle | Ragged packed operator and bounded Qwen3.8-27B-FP8 text execution pass on H20 |
