...
| Full + Sliding interleaving | Independent class lifetimes and joint transactions host-tested | Per-class lowering implemented | `DecoderGraph` currently rejects multiple classes | Unproven |
...
The native dense decoder currently requires one token-KV class covering every
layer. Therefore host compilation of a hybrid manifest is not evidence that the
complete Luminal model executes that hybrid architecture. Unknown or incomplete
contracts fail closed instead of silently becoming Full attention.
