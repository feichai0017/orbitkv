...
def convert_sharegpt_dataset(
    ...
    vllm_bench_dir = (
        Path(__file__).resolve().parents[1] / "third-party" / "vllm" / "benchmarks" / "multi_turn"
    )
    convert_script = vllm_bench_dir / "convert_sharegpt_to_openai.py"

    if not convert_script.is_file():
        raise FileNotFoundError("Initialize third-party/vllm to run the multi-turn benchmark")
...
def run_benchmark(args, converted_dataset_path: Path, output_file: Path | None = None):
