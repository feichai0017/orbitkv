...
echo -e "${BLUE}=== Running OrbitKV CI Checks ===${NC}\n"
...
cargo clippy -p orbitkv-core -p orbitkv-server --all-targets -- -D warnings
echo -e "${GREEN}✓ Clippy check passed${NC}\n"

# Check 6: Cargo check
echo -e "${YELLOW}[6/6] Running cargo check...${NC}"
cargo check -p orbitkv-core
cargo check -p orbitkv-server
