...
fn has_initial_layout_boundary(item: &PreparedStep) -> bool {
    let lowering = item.class_lowerings[0];
    (
...
fn batch_lifecycle_preserves_order_offsets_and_shared_completion() {
    ...
    assert!(prepared.iter().all(|item| {
        item.class_lowerings.len() == 1
            ...
            && has_initial_layout_boundary(item)
...
#[test]
fn completion_domains_advance_independently() {
    let mut manager = manager_with(18, 8, 64, 8);
    let requests = manager
        .acquire_request_leases_for_test(3)
        .expect("requests");
    for (request, domain, value) in [
        (requests[0], 41, 7),
        (requests[1], 41, 8),
        (requests[2], 42, 1),
    ] {
        ...
        complete(&mut manager, &submitted, domain, value);
    }
    assert_eq!(manager.completion_high_water.get(&41), Some(&8));
    assert_eq!(manager.completion_high_water.get(&42), Some(&1));
}
