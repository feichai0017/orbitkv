use super::{
    Arc, AttachedPrefix, BTreeMap, BTreeSet, CancelAttachedRequestItem, CanonicalKvManager,
    DetachedReason, EvictedPrefix, KvManagerError, MaterializedRequestView, PageLease, PagePhase,
    PrefixAttachItem, PrefixEvictionBatch, PrefixLease, PrefixPublishItem, PrefixPublishRelease,
    PrefixSemanticKey, PrefixState, PublishedPrefix, ReclamationState, ReleaseCompletion,
    RequestSnapshot, RequestView, RootEntry, SnapshotLease, ViewVersion,
};
...
impl CanonicalKvManager {
    ...
    /// Atomically cancels attached requests before the materialized view is
    /// observed outside the native manager.
    ///
    /// This path is restricted to already-attached request heads whose
    /// snapshot roots exactly alias the supplied resident prefix. It reverses
    /// the attach-time request page references, drops the attached snapshot,
    /// and marks the request released without producing detached bindings or
    /// reclamation certificates.
    ///
    /// # Errors
    ///
    /// Rejects empty or duplicate batches, stale request or Prefix identities,
    /// busy requests, non-aliasing snapshots, and pages that are not live,
    /// quiescent, or still owned by the resident Prefix.
    ///
    /// # Panics
    ///
    /// Panics only if manager state changes after complete preflight, which
    /// indicates an internal invariant violation.
    pub fn cancel_attached_request_batch(
        &mut self,
        items: &[CancelAttachedRequestItem],
        ...
        let mut decrements = BTreeMap::<PageLease, u32>::new();
        let mut plans = Vec::with_capacity(items.len());
        for item in items {
            if !seen_requests.insert(item.request) {
                return Err(KvManagerError::DuplicateRequest);
            }
            if !seen_prefixes.insert(item.prefix) {
                return Err(KvManagerError::DuplicatePrefix);
            }
            self.check_prefix_epoch(item.prefix)?;
            let prefix_state = self
                .prefixes
                .get(item.prefix.slot, item.prefix.generation)?;
            if prefix_state.evicted
                || self.prefix_index.get(&prefix_state.key) != Some(&item.prefix)
            {
                return Err(KvManagerError::PrefixMiss);
            }
            let request_state = self.request(item.request)?;
            if request_state.released || request_state.quarantined {
                return Err(KvManagerError::RequestUnavailable);
            }
            if request_state.head != item.expected_head {
                return Err(KvManagerError::StaleView);
            }
            if request_state.busy() {
                return Err(KvManagerError::RequestBusy);
            ...
            let snapshot = self.request_snapshot(item.request)?;
            if snapshot.boundary != prefix_state.key.boundary
                || !Arc::ptr_eq(&snapshot.roots, &prefix_state.roots)
            {
                return Err(KvManagerError::PrefixHintStale);
            }
            let entries = Self::root_entries(&snapshot.roots, snapshot.boundary, self.page_tokens);
            for entry in &entries {
                *decrements.entry(entry.page).or_default() = decrements
                    .get(&entry.page)
                    .copied()
                    .unwrap_or(0)
                    .checked_add(1)
            ...
            plans.push((item.request, request_state.head));
        }
        for (lease, decrement) in &decrements {
            let page = self.page(lease.page_id)?;
            if page.generation != lease.generation
                || page.phase != PagePhase::Live
                || page.reader_pins != 0
        ...
        for (lease, decrement) in decrements {
            self.page_mut(lease.page_id)
                .expect("attached request cancel preflight retained page")
                .request_refs -= decrement;
        }
        for (request, snapshot) in plans {
            self.snapshots
                .remove(snapshot.slot, snapshot.generation)
                .expect("attached request cancel preflight retained snapshot");
            self.request_mut(request)
                .expect("attached request cancel preflight retained request")
                .released = true;
        }
        Ok(())
    }
    ...
    fn validate_prefix_bundle(
        ...
        if self
            .classes
            .iter()
            .any(|class| class.retention == super::RetentionKind::Chunked)
        {
            return Err(KvManagerError::UnsupportedProfile(
                "chunked prefix publication is not implemented",
            ));
        }
