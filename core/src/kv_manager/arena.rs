...
pub(super) struct RuntimeClass {
    ...
    pub(super) blocks_per_epoch: Option<u64>,
...
impl RuntimeClass {
    pub(super) fn candidate_start(self, previous_boundary: u64) -> u64 {
        match self.retention {
            ...
            RetentionKind::Chunked => self.epoch_start_for_boundary(previous_boundary),
        }
    }
    ...
    pub(super) fn epoch_start_for_boundary(self, boundary: u64) -> u64 {
        boundary / self.validated_chunk_tokens() * self.validated_chunk_tokens()
    }

    pub(super) fn append_crosses_epoch(self, previous_boundary: u64, target_boundary: u64) -> bool {
        debug_assert!(target_boundary > previous_boundary);
        previous_boundary / self.validated_chunk_tokens()
            != (target_boundary - 1) / self.validated_chunk_tokens()
    }

    pub(super) fn is_epoch_reset(self, previous_boundary: u64, target_boundary: u64) -> bool {
        if self.retention != RetentionKind::Chunked {
            return false;
        }
        previous_boundary > 0
            && previous_boundary.is_multiple_of(self.validated_chunk_tokens())
            && target_boundary > previous_boundary
    }
    ...
    pub(super) fn temporal_address(self, ordinal: u64) -> (u64, u64) {
        match self.retention {
            ...
            RetentionKind::Chunked => {
                let blocks = self
                    .blocks_per_epoch
                    .expect("validated chunked class has blocks per epoch");
                (ordinal % blocks, ordinal / blocks)
            }
        }
    }

    fn validated_chunk_tokens(self) -> u64 {
        self.chunk_tokens
