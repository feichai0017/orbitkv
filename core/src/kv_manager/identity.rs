...
///
/// Snapshots are first-class: request state contains only a head lease, while
/// the snapshot payload lives in a bounded arena. A recycled slot can therefore
/// never make an old request or prefix hint name a new view.
#[derive(Clone, Copy, Debug, Eq, Ord, PartialEq, PartialOrd, Serialize)]
#[repr(C)]
