...
def semantic_live_tokens_for_request(
    *,
    ...
    by the next query.  Chunked retention keeps only the current epoch; a
    committed boundary exactly at the end of an epoch has an empty root.
    """

    boundary = _nonnegative("request boundary", boundary)
    if retention == "chunked":
        if window_tokens is not None:
            raise ValueError("Chunked retention cannot carry a window")
        chunk = _positive("chunk tokens", chunk_tokens)
        if active_kv_length is not None:
            raise ValueError(
                "Chunked retention cannot carry an active KV length"
            )
