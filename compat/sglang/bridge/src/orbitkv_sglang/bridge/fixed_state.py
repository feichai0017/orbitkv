...
class FixedStateCoordinator:
    ...
    def _request_owner(self, request: Any) -> int:
        request_id = getattr(request, "_orbitkv_engine_request_id", None)
        if (
            ...
            raise ManagerError(
                "fixed-state owner lacks a native session request identity"
            )
        return request_id
    ...
    def shutdown(self) -> None:
        from . import state as bridge_state

        try:
            self.close()
        finally:
            if bridge_state._FIXED_STATE is self:
                bridge_state._FIXED_STATE = None
...
def _execute_fixed_state_deferred(
    ...
    coordinator = bridge_state._FIXED_STATE
...
def _attach_fixed_state_forward_batch(
    ...
    if bridge_state._FIXED_STATE is not None:
