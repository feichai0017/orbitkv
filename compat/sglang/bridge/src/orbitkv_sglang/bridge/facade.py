...
class _NativeAuthorityForbidden:
    ...
    def free_group_begin(self) -> None:
        group_state = getattr(self, "_orbitkv_free_group_state", None)
        if group_state == "retry_pending":
            raise RuntimeError(
                "SGLang cannot begin a new release group while recycle is pending"
            )
        if group_state != "idle":
            runtime = _runtime()
    ...
    def free_group_end(self) -> None:
        group_state = getattr(self, "_orbitkv_free_group_state", None)
        if group_state == "retry_pending":
            pending_work = getattr(
                self, "_orbitkv_pending_release_work", None
            )
            if pending_work is not None:
                if self.free_group != []:
                    runtime = _runtime()
                    runtime.fail_stop(
                        "SGLang retained conflicting pending release work"
                    )
                    self._orbitkv_free_group_state = "flushing"
                    self.free_group = []
                    raise FailStopped(
                        runtime.failure_reason or "pending release work conflicted"
                    )
                from . import session_lowering

                session_lowering.retry_pending_release()
                return
            candidates = self.free_group
            if not isinstance(candidates, tuple) or not candidates:
                runtime = _runtime()
                runtime.fail_stop(
                    "SGLang lost its pending OrbitKV release group"
                )
                self._orbitkv_free_group_state = "flushing"
                self.free_group = []
                raise FailStopped(
                    runtime.failure_reason or "pending release group was lost"
                )
        elif group_state == "collecting":
        ...
        try:
            ...
            self.free_group = candidates
            self._orbitkv_free_group_state = "retry_pending"
            self.is_not_in_free_group = True
            raise
...
def _initialize_facade(
    ...
    *,
    ...
    facade._orbitkv_pending_release_work = None
...
def _validate_chunked_pool_floor(
    class_config: Any, size: int, page_size: int
) -> None:
    """Reserve one complete resettable epoch for every live request."""

    blocks = getattr(class_config, "blocks_per_epoch", None)
    ...
    if (
        ...
        or chunk_tokens != blocks * page_size
    ):
        raise RuntimeError(
            "compiled chunked KV geometry differs from the SGLang page size"
        )
    minimum_pages = blocks * _limits().maximum_running_requests
    actual_pages = size // page_size
    if actual_pages < minimum_pages:
        raise RuntimeError(
            "SGLang chunked KV capacity is below the compiled per-request "
            f"epoch floor: pages={actual_pages} minimum={minimum_pages}"
        )
...
def _build_token_to_kv_pool_allocator(
    ...
    *,
    ...
    _state._admit_product_runtime_profile()
    ...
    if retentions in (("full",), ("chunked",)):
        if bool(getattr(configurator, "is_hybrid_swa", False)):
            raise RuntimeError(
                "single-class paged plan cannot use SGLang hybrid storage"
            )
        size = _pool_tokens(
            "paged KV capacity", int(sizes.max_total_num_tokens), page_size
        )
        class_config = _config().classes[0]
        if class_config.retention == "chunked":
            _validate_chunked_pool_floor(class_config, size, page_size)
        allocator = full_type(
            size,
            page_size,
            dtype,
    ...
    try:
        _mirror_cleanup_coordinator(req_to_token_pool, allocator)
    except Exception as error:
        _state._rollback_initialization(error)
        raise
    return allocator
