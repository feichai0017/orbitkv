...
SOURCE_ROOT = REPOSITORY_ROOT / "integrations/sglang/src"
...
def _strict_json(path: Path) -> dict[str, Any]:
    try:
        ...
        if str(error) == "strict JSON value must be an object":
            raise RuntimeError(f"JSON value is not an object: {path}") from error
        ...
        raise RuntimeError(f"cannot load strict JSON {path}: {message}") from error
    return value
