...
def _validate_certificate_mirrors(
    mirror: Any,
    allocator: Any,
    ...
    *,
    ...
    for certificate in certificates:
        ...
        full_location_parts.append(mirror[begin:end].to(dtype=torch.int64))
    ...
    full_locations = (
        full_location_parts[0]
        if len(full_location_parts) == 1
        else torch.cat(full_location_parts)
    )
    expected = (
        expected_parts[0] if len(expected_parts) == 1 else torch.cat(expected_parts)
    )

    if hybrid:
        mapping = allocator.full_to_swa_index_mapping
        mapping_size = int(mapping.shape[0])
        if mapping_size <= 1:
            raise RuntimeError("SGLang Full-to-SWA mapping has no usable locations")
        safe_locations = full_locations.clamp(min=0, max=mapping_size - 1)
        mapped = mapping[safe_locations].to(dtype=torch.int64)
        actual = torch.where(
            (full_locations > 0) & (full_locations < mapping_size),
            mapped,
            torch.full_like(mapped, -1),
        )
    else:
        actual = full_locations

    # torch.equal is deliberately the only device-to-host decision in this
    # validation. No mirror is mutated until the entire certificate batch has
    # passed, so a forged middle certificate cannot partially clear state.
...
def _clear_reclamation_mirrors(
    ...
    for certificate in certificates:
        ...
        if class_config.retention == "sliding":
            sliding_certificates.append(certificate)

    full_locations = _validate_certificate_mirrors(
        mirror,
        allocator,
        sliding_certificates,
        hybrid=hybrid,
    ...
    if not releasing:
        if hybrid and full_locations is not None:
            _mapping_clear(
                allocator.full_to_swa_index_mapping,
                full_locations,
            )
        elif not hybrid:
            for certificate in sliding_certificates:
                begin = int(certificate.token_begin)
                end = int(certificate.token_end_exclusive)
    ...
    if not releasing and certificates:
        ...
        frontier = max(
            ...
            for item in sliding_certificates
        )
