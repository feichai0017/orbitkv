...
RECORD_SCHEMA = "orbitkv.sglang-v0517-full-hybrid-single-run.v5"
...
def _validate_batch_counter_contract(
    ...
    *,
    ...
    expected_request_counters = {
        ...
        # SGLang brackets every decode result with free_group_begin/end.  The
        # manager must flush every finished engine batch as one release and
        # recycle transaction, never as per-request scalar calls.
        "release_batch_calls": completed_iterations,
        "released_items": expected_requests,
        "recycle_requests_calls": completed_iterations,
    }
    mismatches = {
    ...
    if hybrid:
        ...
        raise RuntimeError(
            f"OrbitKV Full release acknowledgements are not grouped at {stage}"
        )
