...
class CanonicalRuntime:
    ...
    def unbind_request_rows(
        self, assignments: Sequence[tuple[Hashable, int]]
    ) -> None:
        """Forget a proven batch only after SGLang returned every row."""

        with self._lock:
            self._healthy()
            values = tuple(assignments)
            keys = {key for key, _row in values}
            rows = {row for _key, row in values}
            if (
                not values
    ...
    def release_batch(self, keys: Sequence[Hashable]) -> None:
        with self._lock:
            ...
            if not values or len(set(values)) != len(values):
                raise ManagerError("release batch keys must be nonempty and unique")
            try:
                records = tuple(self._requests[key] for key in values)
            except KeyError as error:
                raise ManagerError("release batch names an unknown request") from error

            prepared = tuple(
                record.pending
                for record in records
                if record.pending is not None
                and record.pending.phase is StepPhase.PREPARED
            )
            if prepared:
                self.abort_unobserved(
                    BatchRecord(
                        tuple(pending.key for pending in prepared),
                        prepared,
                    )
                )
            ...
            event_keys = tuple(
                ...
                if record.pending is not None
                and record.pending.phase is StepPhase.EVENT
            )
            if event_keys:
                self.wait_batch(event_keys)

            uncertain = tuple(
                record.pending for record in records if record.pending is not None
            )
            if uncertain:
                lowered = tuple(
                    pending
                    for pending in uncertain
                    if pending.phase is StepPhase.LOWERED
                )
                if lowered:
                    self.lowering_failed(
                        BatchRecord(
                            tuple(pending.key for pending in lowered), lowered
                        ),
                        RuntimeError("release raced lowering"),
                    )
                else:
                    self._quarantine_submitted(uncertain)
                    self.fail_stop("request release raced an unproven submission")
                raise FailStopped(self._failure or "release raced submission")

            try:
                completions = tuple(
                    self.manager.release_batch(
                        tuple(record.lease for record in records)
                    )
                )
                if len(completions) != len(records):
                    raise ManagerError("manager returned the wrong release item count")
                released_pages = tuple(
                    self._validate_release(record, completion)
                    for record, completion in zip(
                        records, completions, strict=True
                    )
                )
                receipts = tuple(
                    receipt
                    for completion in completions
                    for receipt in reclamation_receipts(completion.retirements)
                )
