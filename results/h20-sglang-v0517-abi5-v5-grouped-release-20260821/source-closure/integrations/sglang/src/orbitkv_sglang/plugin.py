...
_ALLOCATOR: Any = None
_MIRROR_CLEANUP: _MirrorCleanupCoordinator | None = None
...
class _NativeAuthorityForbidden:
    ...
    def free_group_end(self) -> None:
        ...
        try:
            if candidates:
                _flush_release_group(candidates)
...
@dataclass(frozen=True, slots=True)
class _MirrorCleanupOperation:
    zero_tensors: tuple[Any, ...]
    frontier_state: Any | None
    swa_frontier: int | None


@dataclass(frozen=True, slots=True)
class _MirrorCleanupPlan:
    mapping: Any | None
    mapping_indices: Any | None
    operations: tuple[_MirrorCleanupOperation, ...]


class _MirrorCleanupCoordinator:
    """Own one transactional ReqToToken/LUT cleanup boundary."""
    ...
    def matches(self, req_to_token_pool: Any, allocator: Any) -> bool:
        return (
            self.req_to_token_pool is req_to_token_pool
            and self.allocator is allocator
        )

    def preflight(self, items: Sequence[MirrorCleanupItem]) -> _MirrorCleanupPlan:
        import torch

        values = tuple(items)
        if not values:
            raise RuntimeError("mirror cleanup batch must be nonempty")
        ...
        row_capacity = int(table.shape[0])
        try:
        ...
        if hybrid and (
            ...
            or int(mapping.shape[0]) <= 1
        ):
            raise RuntimeError("SGLang Full-to-SWA mapping changed")
        mapping_size = int(mapping.shape[0]) if mapping is not None else 0
        operations: list[_MirrorCleanupOperation] = []
        actual_parts = []
        expected_parts = []
        mapping_clear_parts = []
        rows: set[int] = set()
        ...
        for item in values:
            if not isinstance(item.context, _MirrorCleanupContext):
                raise RuntimeError("reclamation cleanup lost its request context")
            req = item.context.req
            ...
            if isinstance(raw_row, bool) or not isinstance(raw_row, Integral):
                raise RuntimeError("ReqToToken reclamation row is not an integer")
            row = int(raw_row)
            if (
                row != item.context.request_row
                or not 0 < row < row_capacity
                or row in rows
            ):
                raise RuntimeError(
                    "ReqToToken reclamation names a dummy or foreign row"
                )
            rows.add(row)
            mirror = table[row]
            if isinstance(item.boundary, bool) or not isinstance(
                item.boundary, Integral
            ):
                raise RuntimeError("manager cleanup boundary is not an integer")
            boundary = int(item.boundary)
            if not 0 <= boundary <= maximum:
                raise RuntimeError("manager cleanup boundary exceeds ReqToToken")

            sliding_spans: list[tuple[int, int]] = []
            sliding_frontiers = []
            for certificate in item.certificates:
                try:
                    ...
                    raise RuntimeError(
                        "reclamation names an unknown KV class"
                    ) from error
                ...
                if (
                    ...
                    or (not item.releasing and end % _config().page_tokens)
                ):
                    raise RuntimeError(
                        "manager reclamation exceeds a whole mirror page"
                    )
                if not item.releasing and class_config.retention != "sliding":
                ...
                if hybrid and class_config.retention == "sliding":
                    if mapping_size <= 1:
                        raise RuntimeError(
                            "SGLang Full-to-SWA mapping has no usable locations"
                        )
                    safe_locations = full_locations.clamp(
                        min=0, max=mapping_size - 1
                    )
                    mapped = mapping[safe_locations].to(dtype=torch.int64)
                    actual_parts.append(
                        torch.where(
                            (full_locations > 0)
                            & (full_locations < mapping_size),
                            mapped,
                            torch.full_like(mapped, -1),
                        )
                    )
                    if not item.releasing:
                        mapping_clear_parts.append(safe_locations.reshape(-1))
                else:
                    actual_parts.append(full_locations)

                if class_config.retention == "sliding":
                    sliding_spans.append((begin, end))
                    sliding_frontiers.append(end)

            prefix = getattr(req, "prefix_indices", None)
            prefix_count = None
            prefix_views = []
            if item.releasing or (not hybrid and sliding_spans):
                if (
                    type(prefix) is not torch.Tensor
                    or prefix.ndim != 1
                    or prefix.dtype is not torch.int64
                    or prefix.device != mirror.device
                    or not callable(getattr(prefix, "zero_", None))
                ):
                    raise RuntimeError(
                        "SGLang prefix mirror is not a writable integer tensor"
                    )
                ...
                if item.releasing:
                    if prefix_count:
                        actual_parts.append(prefix.to(dtype=torch.int64))
                        expected_parts.append(
                            mirror[:prefix_count].to(dtype=torch.int64)
                        )
                    prefix_views.append(prefix)
                else:
                    for begin, end in sliding_spans:
                        ...
                        if prefix_begin < prefix_end:
                            ...
                            actual_parts.append(prefix_view.to(dtype=torch.int64))
                            expected_parts.append(
                                mirror[prefix_begin:prefix_end].to(dtype=torch.int64)
                            )
                            prefix_views.append(prefix_view)

            zero_tensors = []
            if item.releasing:
                zero_tensors.extend((mirror, *prefix_views))
                if hybrid and boundary:
                    release_locations = mirror[:boundary].to(dtype=torch.int64)
                    safe_locations = release_locations.clamp(
                        min=0, max=mapping_size - 1
                    )
                    actual_parts.append(
                        torch.where(
                            (release_locations > 0)
                            & (release_locations < mapping_size),
                            release_locations,
                            torch.full_like(release_locations, -1),
                        )
                    )
                    expected_parts.append(release_locations)
                    mapping_clear_parts.append(safe_locations.reshape(-1))
            elif not hybrid:
                for begin, end in sliding_spans:
                    zero_tensors.append(mirror[begin:end])
                zero_tensors.extend(prefix_views)

            frontier_state = None
            swa_frontier = None
            if not item.releasing and item.certificates:
                if req.kv is None or sliding is None or not sliding_frontiers:
                    raise RuntimeError(
                        "SWA reclamation lost its request KV metadata"
                    )
                from sglang.srt.managers.schedule_batch import ReqKvInfo

                frontier_state = req.kv
                if type(frontier_state) is not ReqKvInfo:
                    raise RuntimeError("SWA reclamation KV metadata is not writable")
                allocated = getattr(frontier_state, "kv_allocated_len", None)
                current_frontier = getattr(
                    frontier_state, "swa_evicted_seqlen", None
                )
                if (
                    isinstance(allocated, bool)
                    or not isinstance(allocated, Integral)
                    or isinstance(current_frontier, bool)
                    or not isinstance(current_frontier, Integral)
                    or int(allocated) != boundary
                    or not 0 <= int(current_frontier) <= boundary
                ):
                    raise RuntimeError("SWA reclamation KV frontier is invalid")
                swa_frontier = max(
                    int(current_frontier), max(sliding_frontiers)
                )
                if swa_frontier > int(allocated):
                    raise RuntimeError("SWA reclamation exceeds allocated KV")
                if prefix_count is not None and prefix_count > int(allocated):
                    raise RuntimeError("SGLang prefix mirror exceeds allocated KV")
            operations.append(
                _MirrorCleanupOperation(
                    zero_tensors=tuple(zero_tensors),
                    frontier_state=frontier_state,
                    swa_frontier=swa_frontier,
                )
            )
        ...
        if actual_parts:
            ...
            # This is the sole device-to-host decision for the entire
            # EventGroup.  No authoritative mirror has changed yet.
            if not torch.equal(actual, expected):
                raise RuntimeError(
                    "reclamation certificate disagrees with the SGLang mirror"
                )
        mapping_indices = None
        if mapping_clear_parts:
            mapping_indices = (
                mapping_clear_parts[0]
                if len(mapping_clear_parts) == 1
                else torch.cat(tuple(mapping_clear_parts))
            )
        return _MirrorCleanupPlan(mapping, mapping_indices, tuple(operations))

    def commit(self, plan: _MirrorCleanupPlan) -> None:
        if plan.mapping_indices is not None:
            plan.mapping[plan.mapping_indices] = 0
        for operation in plan.operations:
            for target in operation.zero_tensors:
                target.zero_()
    ...
    def finalize(plan: _MirrorCleanupPlan) -> None:
        for operation in plan.operations:
            if operation.swa_frontier is not None:
                operation.frontier_state.swa_evicted_seqlen = operation.swa_frontier


def _mirror_cleanup_coordinator(
    req_to_token_pool: Any, allocator: Any
) -> _MirrorCleanupCoordinator:
    global _MIRROR_CLEANUP
    if _MIRROR_CLEANUP is None:
        _MIRROR_CLEANUP = _MirrorCleanupCoordinator(req_to_token_pool, allocator)
    elif not _MIRROR_CLEANUP.matches(req_to_token_pool, allocator):
        raise RuntimeError("SGLang reclamation mirror authority changed")
    return _MIRROR_CLEANUP
...
def _prepare_batch(
    ...
    try:
        ...
        for req, key, pending, is_new in zip(
            ...
            if pending.key != key or pending.prepared.request != manager_record.lease:
                raise RuntimeError("manager batch order differs from SGLang request order")
            req._orbitkv_request_key = key
            req._orbitkv_request_lease = manager_record.lease
            if is_new:
                # New requests do not exist in the runtime until the explicit
                # acquire_batch succeeds, so install their cleanup immediately
                # after the atomic prepare result has been validated.
                runtime.bind_reclamation_cleanup(
                    key,
                    MirrorCleanupBinding(
                        coordinator=_mirror_cleanup_coordinator(
                            batch.req_to_token_pool, _ALLOCATOR
                        ),
        ...
        if runtime.failure_reason is None:
            ...
            if new_requests:
                runtime.release_batch(
                    tuple(_request_key(req) for req in new_requests)
                )
...
def _release_candidate(req: Any, tree_cache: Any) -> _ReleaseCandidate | None:
    runtime = _runtime()
    key = _request_key(req)
    if req.req_pool_idx is None:
...
def _flush_release_group(candidates: Sequence[_ReleaseCandidate]) -> None:
    ...
    for candidate in values:
        ...
        if (
            ...
            or candidate.tree_cache.token_to_kv_pool_allocator is not _ALLOCATOR
        ):
...
def _release_kv_cache(req: Any, tree_cache: Any, is_insert: bool = True) -> None:
    del is_insert
    candidate = _release_candidate(req, tree_cache)
