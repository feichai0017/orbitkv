...
readonly QUAL_RECORD_ROOT="${QUAL_ROOT}/.qualification/records/abi5-v3-current"
...
readonly QUAL_SOURCE_CLOSURE_SHA256="57a22d229b8ce426aeabe6864e6a438438bb9e7f06d0f6dd66fb654e2452f871"
...
assert bench.RECORD_SCHEMA == "orbitkv.sglang-v0517-full-hybrid-single-run.v3"
...
print("ABI5 v3 Python/source/plan/checkpoint contracts passed")
PY
}
...
host_preflight() {
  ...
  verify_python_contracts
  verify_cuda_hidden_imports
  printf 'ABI5 H20 host preflight passed; no GPU was enumerated or initialized.\n'
}

require_execution_gate() {
  local expected_gate
  expected_gate="GO:${QUAL_SOURCE_CLOSURE_SHA256}:${QUAL_LIBRARY_SHA256}"
  [[ "${ORBITKV_H20_EXECUTE-}" == "ABI5_V3_H20_QUALIFICATION" ]] || \
    die "GPU phase requires ORBITKV_H20_EXECUTE=ABI5_V3_H20_QUALIFICATION"
...
print(f"paired ABI5 v3 records passed: {stock_path.name} / {manager_path.name}")
PY
}
