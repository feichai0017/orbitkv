...
def validate_arguments(args: argparse.Namespace) -> dict[str, Path | None]:
    ...
    if args.requests * aligned_prompt_tokens > args.chunked_prefill_size:
        raise ValueError(
            "--chunked-prefill-size must admit the page-aligned complete "
            "B1/B4 prompt batch"
        )
...
def _validate_batch_counter_contract(
    ...
    *,
    ...
    batch_calls = counters["prepare_batch_calls"]
    if batch_calls <= 0:
        raise RuntimeError(
            f"OrbitKV ABI5 workload executed no manager batches at {stage}"
        )
