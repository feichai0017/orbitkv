...
readonly QUAL_RECORD_ROOT="${QUAL_ROOT}/.qualification/records/abi5-v4-current"
...
readonly QUAL_SOURCE_CLOSURE_SHA256="e61045f6fe3731f2bf76fb141f385c2898b3254b056a6818acdc216ea555b567"
...
assert bench.RECORD_SCHEMA == "orbitkv.sglang-v0517-full-hybrid-single-run.v4"
...
print("ABI5 v4 Python/source/plan/checkpoint contracts passed")
PY
}
...
host_preflight() {
  ...
  verify_python_contracts
  verify_cuda_hidden_imports
  printf 'ABI5 v4 H20 host preflight passed; no GPU was enumerated or initialized.\n'
}

require_execution_gate() {
  local expected_gate
  expected_gate="GO:${QUAL_SOURCE_CLOSURE_SHA256}:${QUAL_LIBRARY_SHA256}"
  [[ "${ORBITKV_H20_EXECUTE-}" == "ABI5_V4_H20_QUALIFICATION" ]] || \
    die "GPU phase requires ORBITKV_H20_EXECUTE=ABI5_V4_H20_QUALIFICATION"
...
print(f"paired ABI5 v4 records passed: {stock_path.name} / {manager_path.name}")
PY
}
